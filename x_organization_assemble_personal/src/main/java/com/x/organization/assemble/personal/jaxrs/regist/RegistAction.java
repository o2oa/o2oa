...
public class RegistAction extends StandardJaxrsAction {
	...
	@HttpMethodDescribe(value = "当前允许的注册模式,disable,captcha,code", response = WrapOutString.class)
	@GET
	@Path("mode")
	...
	@HttpMethodDescribe(value = "生成一个code", response = WrapOutBoolean.class)
	@GET
	...
	@HttpMethodDescribe(value = "校验mobile是否已经存在", response = WrapOutBoolean.class)
	@GET
	...
	@HttpMethodDescribe(value = "校验name是否已经存在", response = WrapOutBoolean.class)
	@GET
	...
	@HttpMethodDescribe(value = "校验password密码等级.", response = WrapOutInteger.class)
	@GET
	...
	@HttpMethodDescribe(value = "注册人员", request = WrapInRegist.class, response = WrapOutBoolean.class)
	@POST
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
