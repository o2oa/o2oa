...
@Path("companyduty")
public class CompanyDutyAction extends StandardJaxrsAction {

	BeanCopyTools<CompanyDuty, WrapOutCompanyDuty> outCopier = BeanCopyToolsBuilder.create(CompanyDuty.class,
			WrapOutCompanyDuty.class, null, WrapOutCompanyDuty.Excludes);

	BeanCopyTools<WrapInCompanyDuty, CompanyDuty> inCopier = BeanCopyToolsBuilder.create(WrapInCompanyDuty.class,
			CompanyDuty.class, null, WrapInCompanyDuty.Excludes);

	@HttpMethodDescribe(value = "根据ID获取指定的CompanyDuty.", response = WrapOutCompanyDuty.class)
	@GET
	@Path("{id}")
	...
	@HttpMethodDescribe(value = "创建CompanyDuty.", request = WrapInCompanyDuty.class, response = WrapOutId.class)
	@POST
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response create(@Context HttpServletRequest request, WrapInCompanyDuty wrapIn) {
	...
	@HttpMethodDescribe(value = "更新CompanyDuty.", request = WrapInCompanyDuty.class, response = WrapOutId.class)
	@PUT
	@Path("{id}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response update(@Context HttpServletRequest request, @PathParam("id") String id, WrapInCompanyDuty wrapIn) {
		ActionResult<WrapOutId> result = new ActionResult<>();
		WrapOutId wrap = null;
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			EffectivePerson effectivePerson = this.effectivePerson(request);
	...
	@HttpMethodDescribe(value = "删除CompanyDuty.", response = WrapOutId.class)
	@DELETE
	@Path("{id}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	...
	@HttpMethodDescribe(value = "列示CompanyDuty对象,下一页.", response = WrapOutCompanyDuty.class)
	@GET
	...
	@HttpMethodDescribe(value = "列示CompanyDuty对象,上一页.", response = WrapOutCompanyDuty.class)
	@GET
	...
	@HttpMethodDescribe(value = "根据给定的Company ID,获取指定Company所属的CompanyDuty.", response = WrapOutCompanyDuty.class)
	@GET
	...
	@HttpMethodDescribe(value = "根据给定的Identity ID,获取所拥有的CompanyDuty.", response = WrapOutCompanyDuty.class)
	@GET
	...
	@HttpMethodDescribe(value = "根据给定的公司职务名称列示公司职务.", response = WrapOutCompanyDuty.class)
	@GET
	@Path("list/name/{name}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
