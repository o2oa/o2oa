function departmentAttribute_init() {
    $('#content').html('');
    $('#result').html('');
...
function departmentAttribute_get() {
    $('#result').html('');
    var url = departmentAttribute_getUrl();
    $('#url').html(url);
    $.ajax({
	type : 'get',
	dataType : 'json',
...
function departmentAttribute_getUrl() {
    var url = '../jaxrs/departmentattribute';
    switch ($('#function').val()) {
