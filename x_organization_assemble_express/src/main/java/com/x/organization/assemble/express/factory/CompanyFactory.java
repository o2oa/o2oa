...
public class CompanyFactory extends AbstractFactory {
	...
	public String getWithName(String name) throws Exception {
		...
		cq.select(root.get(Company_.id)).where(p).distinct(true);
	...
	public String getWithDepartment(String id) throws Exception {
		...
		cq.select(root.get(Department_.company)).where(p).distinct(true);
		List<String> list = em.createQuery(cq).setMaxResults(1).getResultList();
		if (!list.isEmpty()) {
			return list.get(0);
		} else {
	...
	public List<String> listAll() throws Exception {
		...
		CriteriaQuery<String> cq = cb.createQuery(String.class);
		Root<Company> root = cq.from(Company.class);
		cq.select(root.get(Company_.id)).distinct(true);
		return em.createQuery(cq).getResultList();
	}
	...
	public String getSupDirect(String id) throws Exception {
		...
		cq.select(root.get(Company_.superior)).where(p).distinct(true);
		List<String> list = em.createQuery(cq).setMaxResults(1).getResultList();
		if (!list.isEmpty()) {
			return list.get(0);
		} else {
	...
	public List<String> listWithCompanyAttribute(String name, String attribute) throws Exception {
		...
		cq.select(root.get(CompanyAttribute_.company)).where(p).distinct(true);
		return em.createQuery(cq).getResultList();
	}
