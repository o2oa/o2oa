...
@Path("editor")
public class EditorAction extends StandardJaxrsAction {

	@HttpMethodDescribe(value = "获取他人共享给我编辑的文件.", response = NameValueCountPair.class)
	@GET
	@Path("list")
	...
	public Response listShare(@Context HttpServletRequest request) {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (String str : business.attachment().listPersonWithEditor(effectivePerson.getName())) {
				NameValueCountPair nvcp = new NameValueCountPair();
				nvcp.setName(str);
				nvcp.setValue(str);
				nvcp.setCount(business.attachment().countWithPersonWithEditor(str, effectivePerson.getName()));
				wraps.add(nvcp);
			}
