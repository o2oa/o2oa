package o2.collect.console.server.application;
...
public class ApplicationServerTools extends JettySeverTools {
	...
	private static String project_class_prefix = "o2.base.core.project.";
	...
	public static Server start(ApplicationServer applicationServer) throws Exception {
		...
		if (BooleanUtils.isTrue(applicationServer.getRedeploy())) {
			...
			List<Class<?>> classes = calculateProjectToDepoly(applicationServer);
	...
	private static List<Class<?>> calculateProjectToDepoly(ApplicationServer applicationServer) throws Exception {
		...
		if (ListTools.isNotEmpty(applicationServer.getIncludes())) {
			for (String str : applicationServer.getIncludes()) {
		...
		if (ListTools.isNotEmpty(applicationServer.getExcludes())) {
			for (String str : applicationServer.getExcludes()) {
				if (!StringUtils.startsWith(str, project_class_prefix)) {
					str = project_class_prefix + str;
				}
				excludes.add(str);
