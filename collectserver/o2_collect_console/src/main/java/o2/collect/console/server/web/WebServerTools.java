package o2.collect.console.server.web;
...
public class WebServerTools extends JettySeverTools {
	...
	// "servers/webServer/x_desktop/res/config/config.json");
	...
	// }
	// String webHost = host;
	// if (StringUtils.isNotEmpty(Config.currentNode().getWeb().getHost())) {
	// webHost = Config.currentNode().getWeb().getHost();
	// }
	// center.put("host", host);
	// center.put("port", Config.centerServer().getPort().toString());
	// center.put("webHost", webHost);
	// centers.add(center);
	// /** 写入proxy地址 */
	// if (StringUtils.isNotEmpty(Config.centerServer().getProxyHost())) {
	// Map<String, String> proxyCenter = new HashMap<String, String>();
	// String proxyWebHost = Config.centerServer().getProxyHost();
	// if (StringUtils.isNotEmpty(Config.currentNode().getWeb().getProxyHost()))
	// {
	// proxyWebHost = Config.currentNode().getWeb().getProxyHost();
	// }
	// proxyCenter.put("host", Config.centerServer().getProxyHost());
	// proxyCenter.put("port", Config.centerServer().getProxyPort().toString());
	// proxyCenter.put("webHost", proxyWebHost);
	...
	// if (StringUtils.equals(Config.token().initialManagerInstance().getName(),
	// Token.defaultInitialManager)) {
	...
	// }
	// /** 写入systemName */
	// map.put("systemName", Config.collect().getName());
	// map.put("systemTitle", Config.collect().getTitle());
	// FileUtils.writeStringToFile(file, gson.toJson(map),
	...
	// buffer.append(" </head>");
	// buffer.append("<body>");
	// for (Entry<String, WebServer> en :
	// Config.nodes().webServers().entrySet()) {
	// WebServer o = en.getValue();
	// if (BooleanUtils.isTrue(o.getEnable())) {
	// String url = BooleanUtils.isTrue(o.getSslEnable()) ? "https://" :
	// "http://";
	// url += en.getKey();
