...
class ManageListNext extends ActionBase {
	...
	ActionResult<List<WrapOutWorkCompleted>> execute(EffectivePerson effectivePerson, String id, Integer count,
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (effectivePerson.isManager()
				...
				result = this.standardListNext(workCompletedOutCopier, id, count, "sequence", equalsTerms, null, null,
				...
				if (ListTools.isNotEmpty(ids)) {
					...
					result = this.standardListNext(workCompletedOutCopier, id, count, "sequence", null, null, null,
