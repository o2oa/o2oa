	...
	ActionResult<List<WrapOutWork>> execute(EffectivePerson effectivePerson, String id, Integer count,
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (effectivePerson.isManager()
				...
				result = this.standardListNext(workOutCopier, id, count, "sequence", equalsTerms, null, null, null,
				...
				if (ListTools.isNotEmpty(ids)) {
					...
					result = this.standardListNext(workOutCopier, id, count, "sequence", null, null, null, inTerms,
							null, null, null, true, DESC);
				}
			}
			/* 添加权限 */
