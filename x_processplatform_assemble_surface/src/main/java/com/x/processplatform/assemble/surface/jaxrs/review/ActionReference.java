...
public class ActionReference extends ActionBase {

	ActionResult<WrapOutMap> execute(EffectivePerson effectivePerson, String id) throws Exception {
		ActionResult<WrapOutMap> result = new ActionResult<>();
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			Business business = new Business(emc);
			Review readCompleted = emc.find(id, Review.class, ExceptionWhen.not_found);
			WrapOutMap wrap = new WrapOutMap();
			wrap.put("readCompleted", reviewOutCopier.copy(readCompleted));
	...
	private List<WrapOutWorkLog> listWorkLog(Business business, Review readCompleted) throws Exception {
