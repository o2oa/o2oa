...
public class ApplicationDictHelper {
	...
	private ItemConverter<ApplicationDictItem> converter;
	...
	public ApplicationDictHelper(EntityManagerContainer emc, String application) {
		...
		this.converter = new ItemConverter<>(ApplicationDictItem.class);
	}
	...
	public String select(String applicationDict, String path) throws Exception {
		...
		if (null == dict) {
			throw new Exception("applicationDict name or alias or id :" + applicationDict + " on applicaiton{id:" + application + "} not existed.");
		}
	...
	public void insert(String applicationDict, String json, String path) throws Exception {
		try {
			...
			ApplicationDictItem parent = this.getWithApplicationDictWithPath(dict.getId(), parentPaths[0], parentPaths[1], parentPaths[2], parentPaths[3], parentPaths[4],
					parentPaths[5], parentPaths[6], parentPaths[7]);
			if (null == parent) {
				throw new Exception("parent not existed.");
			}
			ApplicationDictItem cursor = this.getWithApplicationDictWithPath(dict.getId(), cursorPaths[0], cursorPaths[1], cursorPaths[2], cursorPaths[3], cursorPaths[4],
			...
			if ((null != cursor) && cursor.getItemType().equals(ItemType.a)) {
				...
				throw new Exception("unexpected post with applicationDict{id:" + applicationDict + "} path:" + StringUtils.join(paths, ".") + "json:" + jsonElement);
			}
		} catch (Exception e) {
	...
	public void update(String applicationDict, String json, String path) throws Exception {
		try {
			...
			if (exists.isEmpty()) {
				throw new Exception("applicationDict{id:" + applicationDict + "} on path:" + StringUtils.join(paths, ".") + " is not existed.");
			}
	...
	private List<ApplicationDictItem> listWithApplicationDictWithPath(String applicationDict, String... paths) throws Exception {
		...
		Predicate p = cb.equal(root.get(ApplicationDictItem_.applicationDict), applicationDict);
	...
	private ApplicationDictItem getWithApplicationDictWithPath(String applicationDict, String path0, String path1, String path2, String path3, String path4, String path5,
		...
		throw new Exception("error mulit applicationDictItem{id:" + applicationDict + ", path0:" + path0 + ", path1:" + path1 + ", path2:" + path2 + ", path3:" + path3
	...
	private Integer getArrayLastIndexWithApplicationDictWithPath(String applicationDict, String... paths) throws Exception {
		EntityManager em = emc.get(ApplicationDictItem.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<ApplicationDictItem> cq = cb.createQuery(ApplicationDictItem.class);
		Root<ApplicationDictItem> root = cq.from(ApplicationDictItem.class);
