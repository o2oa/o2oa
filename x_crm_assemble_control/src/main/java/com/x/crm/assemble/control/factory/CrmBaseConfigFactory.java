...
public class CrmBaseConfigFactory extends AbstractFactory {
	...
	public List<String> getIdListByBaseconfigtype(String _configtype) throws Exception {
		...
		//Predicate p = cb.and(cb.equal(root.get(CrmBaseConfig_.baseconfigtype), _configtype), cb.isNull(root.get(CrmBaseConfig_.parentconfigid)));
		Predicate p = cb.or(cb.equal(root.get(CrmBaseConfig_.parentconfigid), ""), cb.isNull(root.get(CrmBaseConfig_.parentconfigid)));
		...
		if (null != _tmpList) {
			logger.error("" + _tmpList.size());
		}
		return _tmpList;
	}

	//查询parentconfigid为指定值得ids
	public List<String> getIdListByBaseconfigtypeByParentId(String _configtype, String _parentconfigid) throws Exception {
		EntityManager em = this.entityManagerContainer().get(CrmBaseConfig.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<String> cq = cb.createQuery(String.class);
		Root<CrmBaseConfig> root = cq.from(CrmBaseConfig.class);
		Predicate p = cb.and(cb.equal(root.get(CrmBaseConfig_.baseconfigtype), _configtype), cb.equal(root.get(CrmBaseConfig_.parentconfigid), _parentconfigid));
		cq.select(root.get(CrmBaseConfig_.id)).where(p);
		List<String> _tmpList = new ArrayList<>();
	...
	/**
	 * _configtype:类型
	 * _parentconfigid：上一级id
	 * */
	public List<CrmBaseConfig> getConfigListByTypByParentIdOrderByOrdernumber(String _configtype, String _parentconfigid) throws Exception {
		EntityManager em = this.entityManagerContainer().get(CrmBaseConfig.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
	...
	public List<WrapOutCrmBaseConfig> rootNodeListByType(String _configtype) throws Exception {
		logger.error("rootNodeListByType:run!!!!:" + _configtype);
		EntityManager em = this.entityManagerContainer().get(CrmBaseConfig.class);
		...
		List<WrapOutCrmBaseConfig> rootNodeList = WrapCrmTools.CrmBaseConfigOutCopier.copy(emc.list(CrmBaseConfig.class, _tmpList));
		return rootNodeList;
	}

	public WrapOutCrmBaseConfig recursiveTree(String _configtype, String _parentconfigid, WrapOutCrmBaseConfig wrapnode) throws Exception {
		EntityManager em = this.entityManagerContainer().get(CrmBaseConfig.class);
		//当前节点
		//WrapOutCrmBaseConfig parentNode = WrapCrmTools.CrmBaseConfigOutCopier.copy(this.entityManagerContainer().find(_parentconfigid, CrmBaseConfig.class));
		...
		logger.error("typedQuery.getResultList:" + typedQuery.getResultList().size());
		...
		for (WrapOutCrmBaseConfig childnode : childNodes) {
			...
			//WrapOutCrmBaseConfig node = recursiveTree(_configtype, childnode.getId(), wrapnode); //递归
