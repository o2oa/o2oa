...
import com.x.common.core.application.component.Assemble;
import com.x.common.core.application.component.Service;
...
public class ApplicationCache extends AbstractApplicationCache {
	...
	public static <T extends JpaObject> void notify(Class<T> clz, List<Object> keys) throws Exception {
		...
		clearCacheRequest.setClassName(clz.getCanonicalName());
	...
	private ApplicationCache() {
		try {
			...
			scanner.scan();
			classes.addAll(scanner.getNamesOfSubclassesOf(Assemble.class));
			classes.addAll(scanner.getNamesOfSubclassesOf(Service.class));
				...
				try {
					...
					if (clearCacheRequest instanceof StopReceiveThreadSignal) {
						...
						for (String str : INSTANCE.manager.getCacheNames()) {
							if (StringUtils.contains(str, SPLIT)) {
								/* 是个复合缓存 */
								if (ArrayUtils.contains(StringUtils.split(str, SPLIT),
										clearCacheRequest.getClassName())) {
