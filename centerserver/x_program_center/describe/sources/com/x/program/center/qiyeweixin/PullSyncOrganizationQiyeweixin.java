...
public class PullSyncOrganizationQiyeweixin {

	private static Logger logger = LoggerFactory.getLogger(PullSyncOrganizationQiyeweixin.class);
	...
	public PullResult execute(Business business) throws Exception {
		...
		String accessToken = Config.qiyeweixin().corp_access_token();
		List<Unit> units = new ArrayList<>();
		List<Person> people = new ArrayList<>();
		List<Identity> identities = new ArrayList<>();
		DepartmentFactory departmentFactory = new DepartmentFactory(
				QiyeweixinHelper.listDepartment(accessToken));
		...
		logger.print("从企业微信同步人员结束,结果:{}.", result);
		return result;
	}
	...
	private void check(Business business, PullResult result, List<Unit> units, List<Person> people,
		...
		units.add(unit);
		for (User o : QiyeweixinHelper.listDepartmentUser(accessToken, department)) {
	...
	private Unit checkUnit(Business business, PullResult result, Unit sup, Department department) throws Exception {
		Unit unit = business.unit().getWithQiyeweixinIdObject(department.getId().toString());
		if (null != unit) {
		...
		if (null == unit) {
			...
			if (!StringUtils.equals(unit.getQiyeweixinHash(), DigestUtils.sha256Hex(XGsonBuilder.toJson(department)))) {
	...
	private Unit createUnit(Business business, PullResult result, Unit sup, Department department) throws Exception {
		...
		unit.setQiyeweixinId(department.getId().toString());
	...
	private Person createOrLinkPerson(Business business, PullResult result, User user) throws Exception {
		...
		if (null != person) {
			person.setQiyeweixinId(user.getUserid());
	...
	private void clean(Business business, PullResult result, List<Unit> units, List<Person> people,
		...
		emc.commit();
		/* 组织单独方法删除 */
		List<Unit> allUnit = this.listQiyeweixinUnit(business);
		...
		List<Person> allPeople = this.listQiyeweixinPerson(business);
		/* 删除个人 */
	...
	private List<Unit> listQiyeweixinUnit(Business business) throws Exception {
		EntityManagerContainer emc = business.entityManagerContainer();
	...
	private List<Person> listQiyeweixinPerson(Business business) throws Exception {
		EntityManagerContainer emc = business.entityManagerContainer();
