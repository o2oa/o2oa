package com.x.mail.assemble.control.jaxrs.uuid;
...
public class UUIDAction extends AbstractJaxrsAction {
	...
	private LogUtil logger = new LogUtil( UUIDAction.class );
	...
	public Response getUUID( @Context HttpServletRequest request ) {
		...
		try {
			...
			logger.info( "user["+ currentPerson.getName() +"] get a new UUID：" + uuid );
			data.add( uuid );
			result.setData( data );
		} catch (Exception e) {
