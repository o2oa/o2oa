...
class ActionBind extends ActionBase {
	...
	ActionResult<WrapOutBind> execute() throws Exception {

		ActionResult<WrapOutBind> result = new ActionResult<>();
		WrapOutBind wrap = new WrapOutBind();
		String meta = StringTools.uniqueToken();
		String url = "http://www.pgyer.com/ZhiHe_android?meta=" + meta; // 二维码内容
		...
		try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
			...
			wrap.setImage(Base64.encodeBase64String(out.toByteArray()));
		}
		wrap.setMeta(meta);
		result.setData(wrap);
		return result;
