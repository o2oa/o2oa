...
@Path("source")
public class SourceAction extends StandardJaxrsAction {

	private static Logger logger = LoggerFactory.getLogger(SourceAction.class);

	@HttpMethodDescribe(value = "获取Source.", response = JsonElement.class)
	@GET
	@Path("{id}")
	...
	@HttpMethodDescribe(value = "创建新的Source.", response = WrapOutId.class)
	@POST
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	...
	@HttpMethodDescribe(value = "列示指定Portal下的Source.", response = WrapOutPortal.class)
	@GET
