...
MWF.xApplication.process.workcenter.Main = new Class({
    ...
    loadList: function(type){
        ...
        if(type=='read') this.batchAction.show();
        else this.batchAction.hide();
    ...
    Test: function(){
        ...
        action.ReadAction.listMyFilterPaging(//平台封装好的方法
            1,1000,//uri的参数
            data,//body请求参数
        ...
        for (var i=0;i<data.length;i++){
            //resut1.push({data[i].title,data[i].id,data[i].work,data[i].updateTime,0})
            resut1.push(data[i].id);
        }
        var list = [];
        for (var i=0;i<resut1.length;i++){
            ...
            var action = o2.Actions.load("x_processplatform_assemble_surface").ReadAction.processing(//平台封装好的方法
                resut1[i],//uri的参数
                data//body请求参数
            );
            list.push(action)
...
MWF.xApplication.process.workcenter.TaskCompletedList = new Class({
    ...
    loadData: function(){
        var _self = this;
        // {"relateWork":true,"latest":true}
        this.filterList.relateWork = true;
        ...
        return this.action.TaskCompletedAction.V2ListPaging(this.page, 20, this.filterList||{}).then(function(json){
            _self.fireEvent("loadData");
            _self.total = json.count;
            return json.data;
        }.bind(this));
...
MWF.xApplication.process.workcenter.ReadCompletedList = new Class({
    ...
    loadData: function(){
        ...
        return this.action.ReadCompletedAction.listMyFilterPaging(this.page, this.size, this.filterList||{}).then(function(json){
            // return this.action.ReadCompletedAction.V2ListPaging(this.page, this.size, {"relateWork":true}).then(function(json){
            _self.fireEvent("loadData");
            _self.total = json.count;
            return json.data;
        }.bind(this));
