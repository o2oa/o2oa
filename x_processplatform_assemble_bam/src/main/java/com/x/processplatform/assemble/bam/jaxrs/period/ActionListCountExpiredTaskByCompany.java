...
class ActionListCountExpiredTaskByCompany extends ActionListCountExpiredTask {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (CompanyStub stub : this.listStub()) {
					...
					p.put("count", this.count(business, o, applicationId, processId, activityId, stub.getValue(),
	...
	private List<CompanyStub> listStub() throws Exception {
		...
		for (CompanyStub o : ThisApplication.period.getExpiredTaskCompanyStubs()) {
			list.add(o);
		}
		return list;
	}
