...
class ActionListCountCompletedTaskByCompany extends ActionListCountCompletedTask {

	ActionResult<WrapOutMap> execute(String applicationId, String processId, String activityId) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (CompanyStub stub : this.listStub()) {
					...
					Long count = this.count(business, o, applicationId, processId, activityId, stub.getValue(),
							StandardJaxrsAction.EMPTY_SYMBOL, StandardJaxrsAction.EMPTY_SYMBOL);
					pair.put("count", count);
					Long duration = this.duration(business, o, applicationId, processId, activityId, stub.getValue(),
							StandardJaxrsAction.EMPTY_SYMBOL, StandardJaxrsAction.EMPTY_SYMBOL);
