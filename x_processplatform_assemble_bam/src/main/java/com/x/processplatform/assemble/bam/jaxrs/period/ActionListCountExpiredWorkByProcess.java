...
class ActionListCountExpiredWorkByProcess extends ActionListCountExpiredWork {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ProcessStub stub : this.listStub(applicationId)) {
					...
					p.put("count", this.count(business, o, applicationId, stub.getValue(), companyName, departmentName,
							personName));
					list.add(p);
				}
			}
