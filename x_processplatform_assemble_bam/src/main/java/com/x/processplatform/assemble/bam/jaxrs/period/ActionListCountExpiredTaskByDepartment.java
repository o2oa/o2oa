...
class ActionListCountExpiredTaskByDepartment extends ActionListCountExpiredTask {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (DepartmentStub stub : this.listStub(companyName)) {
					...
					p.put("count", this.count(business, o, applicationId, processId, activityId, companyName,
							stub.getValue(), HttpAttribute.x_empty_symbol));
					list.add(p);
				}
			}
			result.setData(wrap);
