...
public class DownloadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				...
				if (fileInfo != null) {
					...
					try {
						...
						logger.error("[downloadFile]数据文件流向浏览器输出时发生异常！", e);
					} finally {
					    if( output != null ){
					    	output.close();
					    }
			...
			logger.error("[downloadFile]用户下载数据文件发生未知异常！", e);
			ActionResult<Object> result = new ActionResult<>();
			result.error(e);
			response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			FileUploadServletTools.result(response, result);
	...
	private void setResponseHeader(HttpServletResponse response, boolean streamContentType,
		...
		if (streamContentType) {
			...
			response.setHeader("Content-Type", Config.mimeTypes().getContentType(fileInfo.getFileName()));
			response.setHeader("Content-Disposition",
