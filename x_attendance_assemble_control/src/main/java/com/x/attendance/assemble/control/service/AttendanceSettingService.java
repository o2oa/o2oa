...
public class AttendanceSettingService {
	...
	private Logger logger = LoggerFactory.getLogger( AttendanceSettingService.class );
	...
	public AttendanceSetting listIdsByCode( EntityManagerContainer emc, String code ) throws Exception {
	 ...
	 *  APPEALABLE	员工申诉及申诉审批	true|false
		APPEAL_AUDITOR_TYPE	        考勤结果申诉审核人确定方式	       所属部门职位|所属部门属性|人员属性(默认)|指定人|指定角色
		APPEAL_AUDITOR_VALUE	考勤结果申诉审核人确定内容	    直属领导(默认)    
		APPEAL_CHECKER_TYPE	        考勤结果申诉复核人确定方式	       无(默认)|所属部门职位|所属部门属性|人员属性|指定人|指定角色
	...
	public void initAllSystemConfig() {
		...
		selectContent = "所属部门职位|所属部门属性|人员属性|指定人|指定角色";
		isMultiple = false;
		description = "考勤结果申诉复核人确定方式：可选值：无|所属部门职位|所属部门属性|人员属性|指定人|指定角色。此配置控制考勤结果申诉审核人的确定方式,无-表示不需要复核。";
		try {
