...
public class DataItemFactory extends AbstractFactory {
	...
	public List<String> listWithApplication(String id) throws Exception {
		...
		Predicate p = cb.equal(root.get(DataItem_.application), id);
		cq.select(root.get(DataItem_.id)).where(p);
	...
	public List<String> listWithApplicationWithCompleted(String id, Boolean completed) throws Exception {
		...
		Root<DataItem> root = cq.from(DataItem.class);
		Predicate p = cb.equal(root.get(DataItem_.application), id);
		p = cb.and(p, cb.equal(root.get(DataItem_.completed), completed));
		cq.select(root.get(DataItem_.id)).where(p);
		return em.createQuery(cq).getResultList();
	}
	...
	public List<String> listWithProcess(String id) throws Exception {
		...
		CriteriaQuery<String> cq = cb.createQuery(String.class);
		Root<DataItem> root = cq.from(DataItem.class);
		Predicate p = cb.equal(root.get(DataItem_.process), id);
		cq.select(root.get(DataItem_.id)).where(p);
		return em.createQuery(cq).getResultList();
	}
