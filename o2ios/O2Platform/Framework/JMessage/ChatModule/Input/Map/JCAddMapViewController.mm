...
@interface JCAddMapViewController () <BMKMapViewDelegate, BMKLocationManagerDelegate, BMKGeoCodeSearchDelegate>
{
    BMKMapView* mapView;//地图视图

    BMKLocationManager *locManager; //定位
...
- (void)loadMapView {
    ...
    // 定位
    locManager = [[BMKLocationManager alloc]init];
...
- (void)BMKLocationManager:(BMKLocationManager * _Nonnull)manager didUpdateLocation:(BMKLocation * _Nullable)location orError:(NSError * _Nullable)error
{
    ...
    NSLog(@"当前的坐标是: %f,%f",location.location.coordinate.latitude,location.location.coordinate.longitude);
    //注意：必须先初始化地理编码类
    _geoCodeSearch = [[BMKGeoCodeSearch alloc]init];
    _geoCodeSearch.delegate = self;
    BMKReverseGeoCodeSearchOption *reverseGeoCodeOption = [[BMKReverseGeoCodeSearchOption alloc] init];
    //需要反地理编码的坐标位置
    reverseGeoCodeOption.location = CLLocationCoordinate2DMake(location.location.coordinate.latitude, location.location.coordinate.longitude);
    // 调用反地址编码方法，让其在代理方法中输出
    BOOL flag = [_geoCodeSearch reverseGeoCode:reverseGeoCodeOption];
    if (!flag) {
        [locManager startUpdatingLocation];
