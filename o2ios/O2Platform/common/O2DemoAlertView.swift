...
class O2DemoAlertView: UIView {
    ...
    let backFrame = CGRect.init(x: 0, y: 0, w: SCREEN_WIDTH, h: SCREEN_HEIGHT)
    ...
    private func addBoardView() {
        ...
        self.boardView.frame = CGRect.init(x: x, y: -(SCREEN_HEIGHT), w: 315, h: 485)
        self.boardView.backgroundColor = UIColor.clear
        let boardBackImage = UIImageView.init(frame: CGRect.init(x: 0, y: 0, w: 315, h: 485))
        ...
        self.closeBtn.frame = CGRect.init(x: closeX, y: 5, w: 22, h: 22) // 关闭按钮在公告的右上角 right：5 top：5
