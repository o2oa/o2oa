...
public class Query extends GsonPropertyObject {
	...
	private List<String> listJob(EntityManagerContainer emc) throws Exception {
		...
		Predicate ps = cb.equal(root.get(DataItem_.completed), Objects.equals(this.scopeType, ScopeType.workCompleted));
		Predicate pd = cb.and(DateRangeEntryTools.toPredicate(cb, root, this.getRestrictDateRangeEntry()),
				DateRangeEntryTools.toPredicate(cb, root, this.getDateRangeEntry()));
		Predicate pw = cb.and(WhereEntryTools.toPredicate(cb, root, this.getRestrictWhereEntry()),
				WhereEntryTools.toPredicate(cb, root, this.getWhereEntry()));
		Predicate pf = cb.and(FilterEntryTools.toPredicate(cb, root, this.getRestrictFilterEntryList()),
				FilterEntryTools.toPredicate(cb, root, this.getFilterEntryList()));
		cq.select(root.get(DataItem_.job)).distinct(true).where(cb.and(ps, pd, pw, pf));
	...
	private void fillPathSelectEntries(EntityManagerContainer emc, List<String> jobs, List<SelectEntry> selectEntries,
		...
		for (SelectEntry selectEntry : selectEntries) {
			...
			Root<DataItem> root = cq.from(DataItem.class);
			Predicate p = root.get(DataItem_.job).in(jobs);
