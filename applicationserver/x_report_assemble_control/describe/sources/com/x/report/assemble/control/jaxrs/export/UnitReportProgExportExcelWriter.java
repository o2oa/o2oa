		...
		if (ListTools.isNotEmpty(expUnitInfoList)) {
			for (ExpUnitInfo expUnitInfo : expUnitInfoList) {
				...
				if (sheet != null) {
					...
					row.createCell(2).setCellValue("一、" + expUnitInfo.getThisYear() + "年" + expUnitInfo.getThisMonth() + "月份五项重点工作计划（要点）");
					row.createCell(3).setCellValue("一、" + expUnitInfo.getThisYear() + "年" + expUnitInfo.getThisMonth() + "月份五项重点工作总结（要点）");
					row.createCell(4).setCellValue("下月部门重点工作");
					row.createCell(5).setCellValue("二、" + expUnitInfo.getNextYear() + "年" + expUnitInfo.getNextMonth() + "月份五项重点工作计划（要点）");
					...
					if (ListTools.isNotEmpty(expUnitInfo.getExpWorkList())) {
						...
						for ( ExpWork expWork : expUnitInfo.getExpWorkList() ) {
							...
							if (ListTools.isNotEmpty(expWork.getExpMeasuresInfoList())) {
								...
								// 获取批注对象( int dx1, int dy1, int dx2, int dy2, short col1, int row1, short col2, int row2 )
								// 前四个参数是坐标点,后四个参数是编辑和显示批注时的大小.
								comment = drawing.createCellComment(new XSSFClientAnchor(0, 0, 0, 0, (short) 3, 3, (short) 4, 4));
					...
					cellStyle_sheet_title.setAlignment(CellStyle.ALIGN_CENTER); // 居中
					cellStyle_sheet_title.setVerticalAlignment(CellStyle.VERTICAL_CENTER); // 居中
					...
					cellStyle_top_title.setAlignment(CellStyle.ALIGN_CENTER); // 居中
					cellStyle_top_title.setVerticalAlignment(CellStyle.VERTICAL_CENTER); // 居中
					...
					cellStyle.setAlignment(CellStyle.ALIGN_LEFT); // 侧左
