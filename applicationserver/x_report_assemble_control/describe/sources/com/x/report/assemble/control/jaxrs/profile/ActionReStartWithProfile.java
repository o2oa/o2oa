 ...
 *
 */
public class ActionReStartWithProfile extends BaseAction {
	
	private Logger logger = LoggerFactory.getLogger( ActionReStartWithProfile.class );
	...
	protected ActionResult<WrapOutBoolean> execute( HttpServletRequest request, EffectivePerson effectivePerson, String profileId ) throws Exception {
		...
		if( check ){
			try {
				check = report_P_ProfileOperationServiceAdv.restart( profile );
