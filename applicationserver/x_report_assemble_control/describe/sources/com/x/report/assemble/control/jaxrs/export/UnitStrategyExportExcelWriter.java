	...
	public byte[] writeExcel( Map<String, ExpUnitInfo> expUnitInfoMap ) {	
		...
		if( expUnitInfoMap != null ){
			...
			while( expUnitInfo_iterator.hasNext() ) {
				...
				//logger.info( "|===="+ currentRowNumber + " --> " + ( currentRowNumber + expUnitInfo.getRowCount() -1 )+ ": 组织名称["+ expUnitInfo.getTitle() +"]" );
				...
				sheet.addMergedRegion( new CellRangeAddress( currentRowNumber, ( currentRowNumber + expUnitInfo.getRowCount() -1 ), 0, 0 ));
				...
				if( expWorkMap != null && !expWorkMap.isEmpty()) {
					...
					while( expWork_iterator.hasNext() ) {
						...
						//logger.info( "====|===="+ currentRowNumber + " --> " + ( currentRowNumber + expWork.getRowCount() -1 )+ ": 工作标题["+ expWork.getTitle() +"]" );
						...
						sheet.addMergedRegion( new CellRangeAddress( currentRowNumber, ( currentRowNumber + expWork.getRowCount() -1 ), 1, 1 ));
						...
						if( expMeasuresInfoMap != null && !expMeasuresInfoMap.isEmpty() ) {
							...
							while( expMeasuresInfo_iterator.hasNext() ) {
								...
								//logger.info( "========|===="+ currentRowNumber + " --> " + ( currentRowNumber + expMeasuresInfo.getRowCount() -1 )+ ": 举措标题["+ expMeasuresInfo.getTitle() +"]" );
								logger.info( ">>>>>>>>>>>>>>>>>>举措："+expMeasuresInfo.getTitle()+" , rowcount=" + expMeasuresInfo.getRowCount());
