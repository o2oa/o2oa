...
public class ActionListDayByYearMonth extends BaseAction {

	private Logger logger = LoggerFactory.getLogger(ActionListDayByYearMonth.class);
	private DateOperation dateOperation = new DateOperation();

	protected ActionResult<List<Wo>> execute( HttpServletRequest request, String year, String month, EffectivePerson effectivePerson ) {
		...
		//查询自己有汇报内容的所有日期列表，再将日期分页
		if (check) {
			try {
				dateStringList = report_P_PermissionServiceAdv.listAllCreateDateString( permissionObjectCodes, year, month );
			} catch (Exception e) {
				check = false;
				Exception exception = new ExceptionReportInfoProcess(e, "系统在查询用户有汇报的所有日期列表时发生异常。");
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
					...
					if (check) {
						try {
							...
							viewAbleReportIds = report_I_PermissionServiceAdv.lisViewableReportIdsWithFilter(
									...
									permissionObjectCodes, null, 100 );
						} catch (Exception e) {
							check = false;
