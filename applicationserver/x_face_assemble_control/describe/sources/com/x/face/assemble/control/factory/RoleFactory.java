...
public class RoleFactory extends AbstractFactory {
	...
	private Role pickObject(String flag) throws Exception {
		Role o = this.entityManagerContainer().flag(flag, Role.class, ExceptionWhen.none, false, Role.FLA  GS);
		if (o != null) {
			...
			if (matcher.find()) {
				...
				o = this.entityManagerContainer().flag(unique, Role.class, ExceptionWhen.none, false, Role.FLA  GS);
				if (null != o) {
					this.entityManagerContainer().get(Role.class).detach(o);
				}
			}
	...
	public List<Role> pick(List<String> FLA  GS) throws Exception {
