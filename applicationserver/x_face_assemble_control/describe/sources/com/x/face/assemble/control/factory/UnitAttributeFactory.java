...
public class UnitAttributeFactory extends AbstractFactory {
	...
	private UnitAttribute pickObject(String flag) throws Exception {
		UnitAttribute o = this.entityManagerContainer().flag(flag, UnitAttribute.class, ExceptionWhen.none, false,
				UnitAttribute.FLA  GS);
		if (o != null) {
			...
			if (matcher.find()) {
				...
				o = this.entityManagerContainer().flag(unique, UnitAttribute.class, ExceptionWhen.none, false,
						UnitAttribute.FLA  GS);
				if (null != o) {
					this.entityManagerContainer().get(UnitAttribute.class).detach(o);
				}
	...
	public List<UnitAttribute> pick(List<String> FLA  GS) throws Exception {
		...
		list = list.stream()
				.sorted(Comparator.comparing(UnitAttribute::getOrderNumber, Comparator.nullsLast(Integer::compareTo))
						.thenComparing(Comparator
								.comparing(UnitAttribute::getName, Comparator.nullsFirst(String::compareTo)).reversed()))
