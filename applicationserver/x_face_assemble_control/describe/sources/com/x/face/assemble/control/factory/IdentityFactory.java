...
public class IdentityFactory extends AbstractFactory {
	...
	private Identity pickObject(String flag) throws Exception {
		Identity o = this.entityManagerContainer().flag(flag, Identity.class, ExceptionWhen.none, false,
				Identity.FLA  GS);
		if (o != null) {
			...
			if (matcher.find()) {
				...
				o = this.entityManagerContainer().flag(unique, Identity.class, ExceptionWhen.none, false,
						Identity.FLA  GS);
				if (null != o) {
					this.entityManagerContainer().get(Identity.class).detach(o);
				}
	...
	public List<Identity> pick(List<String> FLA  GS) throws Exception {
