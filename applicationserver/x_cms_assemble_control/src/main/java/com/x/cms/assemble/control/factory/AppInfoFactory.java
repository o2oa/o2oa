...
public class AppInfoFactory extends AbstractFactory {
	...
	public AppInfo getWithFlag(String flag) throws Exception {
		...
		CriteriaQuery<AppInfo> cq = cb.createQuery(AppInfo.class);
		Root<AppInfo> root = cq.from(AppInfo.class);
		Predicate p = cb.equal(root.get(AppInfo_.id), flag);
		p = cb.or(p, cb.equal(root.get(AppInfo_.appName), flag));
		p = cb.or(p, cb.equal(root.get(AppInfo_.appAlias), flag));
		List<AppInfo> appInfos = em.createQuery(cq.where(p)).getResultList();
		if (ListTools.isNotEmpty(appInfos)) {
