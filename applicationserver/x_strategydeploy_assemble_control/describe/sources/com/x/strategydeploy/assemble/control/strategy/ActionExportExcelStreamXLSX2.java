...
public class ActionExportExcelStreamXLSX2 extends BaseAction {
	...
	ActionResult<WoCacheFileId> execute(HttpServletRequest request, EffectivePerson effectivePerson, String year, Boolean stream) throws Exception {
		...
		for (StrategyDeploy strategydeploy : strategydeploy_list) {
			XSSFSheet sheet = workbook.createSheet(strategydeploy.getSequencenumber() + "." + strategydeploy.getStrategydeploytitle());
			...
			titlstyle.setAlignment(XSSFCellStyle.ALIGN_CENTER);
			...
			//cell1.setCellValue("工作路径" + strategydeploy.getSequencenumber() + ":" + strategydeploy.getStrategydeploytitle());
			...
			for (MeasuresInfo measures : measures_list) {
				...
				//logger.info("有举措：" + list.get(i).getSequencenumber() + ":" + list.get(i).getStrategydeploytitle());
		...
		//		WoExcel wo = new WoExcel(bytes, this.contentType(stream, excelfilename), this.contentDisposition(stream, excelfilename));
		//		result.setData(wo);
	...
	//根据年份获得工作纲要列表
	protected static List<StrategyDeploy> listStrategyDeploysByYear(EffectivePerson effectivePerson, String year) throws Exception {
