	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, String workId) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			CompletableFuture<Void> future_task = CompletableFuture.runAsync(() -> {
				try {
					List<Task> os = emc.listEqual(Task.class, Task.work_FIELDNAME, workId, Task.person_FIELDNAME,
			...
			CompletableFuture<Void> future_taskCompleted = CompletableFuture.runAsync(() -> {
				try {
					List<TaskCompleted> os = emc.listEqual(TaskCompleted.class, TaskCompleted.work_FIELDNAME, workId,
			...
			CompletableFuture<Void> future_read = CompletableFuture.runAsync(() -> {
				try {
					List<Read> os = emc.listEqual(Read.class, Read.work_FIELDNAME, workId, Read.person_FIELDNAME,
							effectivePerson.getDistinguishedName());
			...
			CompletableFuture<Void> future_readCompleted = CompletableFuture.runAsync(() -> {
				try {
					List<ReadCompleted> os = emc.listEqual(ReadCompleted.class, ReadCompleted.work_FIELDNAME, workId,
							ReadCompleted.person_FIELDNAME, effectivePerson.getDistinguishedName());
			...
			CompletableFuture<Void> future_review = CompletableFuture.runAsync(() -> {
				try {
					List<Review> os = emc.listEqual(Review.class, Review.work_FIELDNAME, workId,
