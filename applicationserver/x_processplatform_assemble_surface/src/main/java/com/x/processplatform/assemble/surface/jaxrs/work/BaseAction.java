...
//				.filter(o -> Objects.equals(o.getFromActivityType(), ActivityType.end))
//				.sorted(Comparator.comparing(WorkLog::getFromTime, Comparator.nullsFirst(Date::compareTo)).reversed())
//				.findFirst().orElse(null);
//		if (null != latestEnd) {
...
//		}
		...
		// List<WoWorkLog> os =
		// WoWorkLog.copier.copy(business.workLog().listWithJobObject(work.getJob()));
		// List<WoTaskCompleted> _taskCompleteds = WoTaskCompleted.copier
		// .copy(business.taskCompleted().listWithJobObject(work.getJob()));
		// List<WoTask> _tasks =
		// WoTask.copier.copy(business.task().listWithJobObject(work.getJob()));
		// os = business.workLog().sort(os);
		//
		// Map<String, List<WoTaskCompleted>> _map_taskCompleteds =
		// _taskCompleteds.stream()
		...
		// for (WoWorkLog o : os) {
		// List<WoTaskCompleted> _parts_taskCompleted =
		// _map_taskCompleteds.get(o.getFromActivityToken());
		// o.setTaskCompletedList(new ArrayList<WoTaskCompleted>());
		// if (!ListTools.isEmpty(_parts_taskCompleted)) {
		// for (WoTaskCompleted _taskCompleted :
		// business.taskCompleted().sort(_parts_taskCompleted)) {
