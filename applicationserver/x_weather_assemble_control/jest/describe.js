...
Describe.prototype = {
	...
	$.getJSON('../describe/describe.json?rd=' + Math.random(), function(json) {
	    ...
	    $.each(json.jaxrs, function(ji, j) {
		$.each(j.methods, function(mi, m) {
		    $('#' + j.name + '_' + m.name).click(function() {
			$('#result').html('');
			...
			if (m.pathParameters && m.pathParameters.length > 0) {
				...
				if (m.name == 'listNext' || m.name == 'listPrev') {
				    ...
				    case 'flag':
				    case 'id':
					txt += '<tr><td><input type="text" id="' + p.name + '" style="width:600px; padding:1px; border:1px #000000 solid" value="(0)"/></td><td>' + p.name + ':' + p.description + '</td></tr>';
					break;
				    case 'count':
					txt += '<tr><td><input type="text" id="' + p.name + '" style="width:600px; padding:1px; border:1px #000000 solid" value="20"/></td><td>' + p.name + ':' + p.description + '</td></tr>';
					break;
				    default:
					txt += '<tr><td><input type="text" id="' + p.name + '" style="width:600px; padding:1px; border:1px #000000 solid"/></td><td>' + p.name + ':' + p.description + '</td></tr>';
					break
				    }
				} else {
				    txt += '<tr><td><input type="text" id="' + p.name + '" style="width:600px; padding:1px; border:1px #000000 solid"/></td><td>' + p.name + ':' + p.description + '</td></tr>';
			...
			if (m.formParameters && m.formParameters.length > 0) {
				...
				if (p.type == "File") {
				    txt += '<tr><td><input type="file" name="' + p.name + '" id="' + p.name + '" style="width:600px; padding:1px; border:1px #000000 solid"/></td><td>' + p.name + ':' + p.description + '</td></tr>';
				} else {
				    txt += '<tr><td><input type="text" id="' + p.name + '" style="width:600px; padding:1px; border:1px #000000 solid"/></td><td>' + p.name + ':' + p.description + '</td></tr>';
				}
			    });
			...
			if (m.ins && m.ins.length > 0) {
				...
				if (i.isCollection) {
				    ...
				    txt += '<tr><td><input type="text" id="' + i.name + '" style="width:600px; padding:1px; border:1px #000000 solid"/></td><td>' + i.name + ':' + i.description
				    '</td></tr>';
				}
			    });
			    txt += '</table>';
			...
			if (m.outs && m.outs.length > 0) {
				...
				txt += '<tr><td>' + o.name + '</td><td>' + o.type + '</td><td>' + (o.isCollection ? 'multi' : 'single') + '</td><td>' + o.description + '</td><td id="out_' + o.name + '_out">&nbsp;</td></tr>';
			    });
			    txt += '</table>';
			    txt += '</fieldset>';
			}
