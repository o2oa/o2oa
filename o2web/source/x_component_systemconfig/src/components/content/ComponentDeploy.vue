<template>
  <div class="title">{{lp.componentDeploy}}</div>
  <div class="componentsArea">
    <div class="componentItem" v-for="(componentItem, index) in components" :key="componentItem.id">
      ...
      <div>
        <button class="grayColor_bg" v-if="componentItem.visible" @click="openApplication(componentItem)">{{lp.component.open}}</button>
        <button class="grayColor_bg" @click="()=>{editComponent(componentItem, index)}">{{lp.component.edit}}</button>
        <button class="grayColor_bg" v-if="componentItem.type!=='system'" @click="(e)=>{uninstallApplication(e, componentItem)}">{{lp.component.uninstall}}</button>
      </div>
    </div>
    ...
    <div class="componentItem" style="background-color: #fcfcfc" @mouseenter="(e)=>{e.target.addClass('deployOver')}" @mouseleave="(e)=>{e.target.removeClass('deployOver')}" @click="createComponent">
      <div class="componentItemTitle">{{lp.component.deploy}}</div>
    ...
    <div class="componentEditorArea" ref="componentEditorNode">
      <BaseInput :label="lp.component.name" v-model:value="currentComponent.name"/>
      <BaseInput :label="lp.component.title" v-model:value="currentComponent.title"/>
      <BaseInput :label="lp.component.path" v-model:value="currentComponent.path" @change="changePath"/>
      <div class="editorPathInfo">{{lp.component.urlPathInfo}}</div>
      <BaseBoolean :label="lp.component.visible" v-model:value="currentComponent.visible"/>
      <BasePerson :label="lp.component.allowList" v-model:value="currentComponent.allowList"/>
      <BasePerson :label="lp.component.denyList" v-model:value="currentComponent.denyList"/>
      <BaseIcon :label="lp.component.icon" v-model:value="currentComponent"
                :icon-style="{marginLeft:'10px'}"
                :can-change="!!(currentComponent.path && currentComponent.path.startsWith('@url'))"
                :upload-text="lp.component.selectIcon"
                :clear-text="lp.component.clearIcon"/>
      <BaseUpload v-if="currentComponent.type!=='system' &&  (!currentComponent.path || !currentComponent.path.startsWith('@url'))"
                  :label="lp.component.upload"
                  :warn="lp.component.uploadWarn"
...
function uninstallApplication(e, cmpt){
  var text = lp.component.removeComponent.replace(/{name}/, cmpt.title);
  component.confirm("warn", e, lp.component.removeComponentTitle, text, 500, 170, function(){
...
async function uninstallComponent(cmpt) {
  ...
  component.notice(lp.component.removeComponentOk, "success");
  ...
  o2.DL.open({
    ...
    buttonList: [{
      ...
      action: async (dlg) => {
        ...
        component.notice( lp.component.deploySuccess, "success");
        if (index){
