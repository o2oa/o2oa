<template>
    <div v-if="filters" ref="componentNode">
      ...
      <div class="item_info">
        <div class="item_message_item" @click="addFilter">
          <div class="item_message_area">
            ...
            <div style="margin-left: 0; font-size: 16px; color: #666666">{{lp._messageConfig.addFilter}}</div>
          </div>
        </div>
      </div>
      ...
      <div class="item_message_module">
        <el-collapse>
          <div class="item_info" v-for="type in Object.keys(filters)" :key="type">
            <el-collapse-item :name="type">
              <template #title>
                <div style="display: flex; align-items: center; width: 100%;">
                  <div class="item_server_item_slot" style="width:80%">{{type}}</div>
                  <div class="item_module_store_del o2icon-del" @click="deleteFilter($event, type)"></div>
                </div>
              </template>
              <div class="item_info">
                <BaseScript :value="filters[type]" :inputType="{type: 'service'}" @blur="(v)=>{filters[type]=v; saveFilter();}" @save="(v)=>{filters[type]=v; saveFilter();}"></BaseScript>
              </div>
      ...
      <div class="item_consumer_editorArea" ref="filterEditorArea">
        <BaseInput :label="lp._messageConfig.filterKey" v-model:value="newFilterData.key"/>
      </div>
...
const filterEditorArea = ref();
const newFilterData = ref({});
const filters = ref({});

const saveFilter = ()=>{
  saveConfig('messages', 'filters', filters.value);
}
...
const addFilter = ()=>{
  newFilterData.value={key: ''};
  openEditDlg(filterEditorArea, (dlg)=>{
    if (!newFilterData.value.key){
      component.notice(lp._messageConfig.inputFilterKey, 'error', dlg.node, {x: 'left', y: 'top'}, {x: 10, y: 10});
      return false
    }
    const k = newFilterData.value.key;
    if (filters.value.hasOwnProperty(k)){
...
const deleteFilter = (e, key)=>{
  e.stopPropagation();
  const text = lp._messageConfig.deleteFilterInfo.replace('{name}', key)
  component.confirm("warn", e, lp._messageConfig.deleteFilterTitle, text, 350, 170, (dlg)=>{
    delete filters.value[key];
    delConfig('messages', 'filters.'+key);
    dlg.close();
  }, (dlg)=>{
    dlg.close();
...
const load = ()=>{
  if (props.message){
    filters.value = props.message.filters || {};
  }
}
load();
