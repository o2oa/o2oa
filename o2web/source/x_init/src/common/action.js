...
//title
const title_set = '/jaxrs/server/set/title';
const title_get = '/jaxrs/server/get/title';
...
const server_license = '/jaxrs/server/license';
...
async function post(url, body, contentType) {
    try {
        ...
        if (res.ok) {
            ...
            const o = await res.json();
            const e = new Error(o.message);
            notice.error(`Post ${url} Status Code: ${res.status} (${res.statusText})`, e.message, {
                err: e,
            });
            throw e;
        }
...
async function setTitle(title) {
    return (await post(title_set, `{"title": "${title}"}`, 'application/json')).value;
}
async function getTitle() {
    return (await get(title_get)).title;
}
