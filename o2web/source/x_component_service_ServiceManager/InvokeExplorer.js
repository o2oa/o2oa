...
MWF.xApplication.service.ServiceManager.InvokeExplorer = new Class({
    ...
    keyCopy: function(e){
        if (this.selectMarkItems.length){
            ...
            this.selectMarkItems.each(function(item){
                this.app.restActions.getInvoke(item.data.id, function(json){
                    json.data.elementType = "invoke";
    ...
    saveItemAs: function(data, success, failure, cancel){
        this.app.restActions.listInvoke( function(dJson){
            var i=1;
...
//                html += "<>"
    ...
    saveItemAsUpdate: function(someItem, data, success, failure){
        ...
        this.app.restActions.updateInvoke(someItem.id, data, function(){
            if (success) success();
        }.bind(this), function(){
            if (failure) failure();
        }.bind(this));
    ...
    saveItemAsNew: function(dJson, data, success, failure){
        ...
        this.app.restActions.createInvoke(data, function(){
            if (success) success();
        }.bind(this), function(){
            if (failure) failure();
        }.bind(this));
