...
(function(){
    ...
    var _loadSingle = function(url, callback, reload, doc){
        ...
        var head = (curdoc.head || curdoc.getElementsByTagName("head")[0] || curdoc.documentElement);
    ...
    var _loadSingleCss = function(url, dom, callback, reload, sourceDoc){
        var key = encodeURIComponent(url);
        if (!reload){
            if (_loadedCss[key]){
                if (dom) _parseDom(dom, function(node){ node.className += ((node.className) ? " css"+_loadedCss[key] : "css"+_loadedCss[key])}, sourceDoc);
        ...
        var success = function(xhr){
            ...
            try{
                if (cssText){
                    ...
                    if (dom){
                        uuid = _uuid();
                        _parseDom(dom, function(node){ node.className += ((node.className) ? " css"+uuid : "css"+uuid)}, sourceDoc);
                    }
    ...
    var _loadCss = function(urls, dom, callback, reload, sourceDoc){
        ...
        if (type==="array"){
            ...
            var styleList = [];
            for (var i=0; i<modules.length; i++){
                _loadSingleCss(modules[i], dom, function(style){
                    thisLoaded.push(modules[i]);
        ...
        if (type==="string"){
            _loadSingleCss(modules, dom, callback, reload, sourceDoc);
        }
    };
    ...
    var _loadO2 = function(){
        o2.core = true;
        o2.more = true;
        // o2.load("o2.core", _dom.check);
        // o2.load("o2.more", _dom.check);
    };
