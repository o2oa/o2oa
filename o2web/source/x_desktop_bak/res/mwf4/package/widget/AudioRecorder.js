...
MWF.widget.AudioRecorder = new Class({
	...
	initialize: function(content, options){
		...
		if (this.options.visible){
            this.path = MWF.defaultPath+"/widget/$AudioRecorder/";
            this.cssPath = MWF.defaultPath+"/widget/$AudioRecorder/"+this.options.style+"/css.wcss";
            this._loadCss();
        }
    ...
    createToolbar: function(){
        MWF.require("MWF.widget.Toolbar", function(){
            ...
            this.toolbar = new MWF.widget.Toolbar(this.actionNode, {"style": "audio"}, this);

            new Element("div", {
                "MWFnodeid": "record",
                "MWFnodetype": "MWFToolBarOnOffButton",
                "MWFButtonImage": this.path+""+this.options.style+"/icon/record.png",
                "title": MWF.LP.widget.record,
                "MWFButtonAction": "recordAction",
                "MWFButtonText": MWF.LP.widget.record
            ...
            new Element("div", {
                ...
                "title": MWF.LP.widget.stop,
                "MWFButtonAction": "stopAction",
                "MWFButtonText": MWF.LP.widget.stop
            }).inject(this.actionNode);
            ...
            new Element("div", {
                ...
                "title": MWF.LP.widget.play,
                "MWFButtonAction": "playAction",
                "MWFButtonText": MWF.LP.widget.play
            }).inject(this.actionNode);
            ...
            new Element("div", {
                ...
                "title": MWF.LP.widget.save,
                "MWFButtonAction": "saveAction",
                "MWFButtonText": MWF.LP.widget.save
            }).inject(this.actionNode);
            ...
            new Element("div", {
                ...
                "title": MWF.LP.widget.cancel,
                "MWFButtonAction": "cancelAction",
                "MWFButtonText": MWF.LP.widget.cancel
            }).inject(this.actionNode);
                ...
                , function (error) {
                    switch (error.code || error.name) {
                        ...
                        case 'PermissionDeniedError':
                            MWF.xDesktop.notice("error", {"x": "right", "y": "top"}, '用户拒绝提供信息。', this.node);
                            //this.throwError('用户拒绝提供信息。');
                            break;
                        case 'NOT_SUPPORTED_ERROR':
                        case 'NotSupportedError':
                            MWF.xDesktop.notice("error", {"x": "right", "y": "top"}, '浏览器不支持硬件设备。', this.node);
                            //this.throwError('<a href="http://www.it165.net/edu/ewl/" target="_blank" class="keylink">浏览器</a>不支持硬件设备。');
                            break;
                        case 'MANDATORY_UNSATISFIED_ERROR':
                        case 'MandatoryUnsatisfiedError':
                            MWF.xDesktop.notice("error", {"x": "right", "y": "top"}, '无法发现指定的硬件设备。', this.node);
                            //this.throwError('无法发现指定的硬件设备。');
                            break;
                        default:
                            MWF.xDesktop.notice("error", {"x": "right", "y": "top"}, '无法打开麦克风。异常信息:' + (error.code || error.name), this.node);
                            //this.throwError('无法打开麦克风。异常信息:' + (error.code || error.name));
                            break;
                    }
                }.bind(this));
        } else {
            MWF.xDesktop.notice("error", {"x": "right", "y": "top"}, '浏览器不支持录音功能。', this.node);
