...
MWF.xApplication.Execution.ReportAttachment = new Class({
    ...
    listAttachment: function( callback ){
        this.actions.listReportAttachment(this.options.documentId, function(json){
            ...
            if( this.isQueryUploadSuccess ){
                ...
                if (files.length) {
                    for (var i = 0; i < files.length; i++) {
                        ...
                        this.actions.uploadReportAttachment(this.options.documentId, function (json) {
                            ...
                            this.attachmentController.checkActions();
                        }.bind(this), function(){}.bind(this), formData, file,this.options.documentId);

                        //this.actions.uploadReportAttachment(this.options.documentId, function (o, text) {
                        //    j = JSON.decode(text);
                        //    if (j.data.length>0 && j.data[0] && j.data[0].id) {
                        //        this.actions.getAttachment(j.data[0].id, this.options.documentId, function (json) {
                        //            json = this.transportData(json);
                        //            if (json.data) {
                        //                this.attachmentController.addAttachment(json.data);
                        //                //this.attachmentList.push(json.data);
    ...
    deleteAttachment: function (attachment) {
        ...
        this.actions.deleteReportAttachment(attachment.data.id, this.documentId, function (json) {
    ...
    downloadAttachment: function (e, node, attachments) {
        attachments.each(function (att) {
            this.actions.getReportAttachmentStream(att.data.id, this.options.documentId);
        }.bind(this));
    },
