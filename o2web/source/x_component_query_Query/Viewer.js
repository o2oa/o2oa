...
//     },
        ...
        _exportViewAll: function (filename, dlg){
            ...
            action.exportViewWithQuery(this.json.viewName, this.json.application, data, function(json){
                ...
                // a.click();
                // a.destroy();
                MWF.ExcelUtilsV2.openDownloadDialog(uri, data.excelName, function (){
                    mask.hide()
                    if(dlg)dlg.close();
                })
            }.bind(this));
...
MWF.xApplication.query.Query.Viewer.Exporter = new Class({
    Implements: [Options, Events],
    options: {
        queryFlag: '',
        viewFlag: '',
    ...
    _getView: function () {
        ...
        if(this.options.viewId){
            ...
            this.getViewRes = o2.Actions.load("x_query_assemble_surface").ViewAction.getWithQuery(
                this.options.viewFlag,
                this.options.queryFlag,
                function (json) {
                    this.json = json.data;
                    this.viewJson = JSON.decode(json.data.data);
                }.bind(this)
    ...
    loadExportData: function(start, end, callback){
        ...
        var list = [];
        do{
            var promise = o2.Actions.load('x_query_assemble_surface').ViewAction.executeV2(this.json.id, page, count, {
                filterList: this.options.filterList,
                parameter: this.options.parameter,
                searchKey: this.options.searchKey,
                orderList: this.orderList,
