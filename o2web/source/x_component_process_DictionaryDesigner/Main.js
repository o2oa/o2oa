...
MWF.xApplication.process.DictionaryDesigner.Main = new Class({
	Extends: MWF.xApplication.Common.Main,
	Implements: [Options, Events],
	options: {
		"style": "default",
		"name": "process.DictionaryDesigner",
		"icon": "icon.png",
	...
	onQueryLoad: function(){
		...
		if (!this.actions){
            ...
            //     MWF.xDesktop.requireApp("process."+this.options.action, "Actions.RestActions", function(){
            //         this.actions = new MWF.xApplication.process[this.options.action].Actions.RestActions();
            //     }.bind(this), false);
            // }else{
...
//		this.processData = this.options.processData;
                    ...
                    if (this.tab.showPage) {
                        ...
                        if (dictionary) {
                            if (dictionary.currentSelectedItem) {
                                ...
                                var item = new MWF.xApplication.process.DictionaryDesigner.Dictionary.item(key, value, parent, level, this.dictionary, true, nextSibling);
    ...
    createListTitleNodes: function (){
        this.dictionaryListTitleNode.setStyle("display", 'flex');

        this.titleActionArea = new Element("div", {
            styles: this.css.titleActionArea
        }).inject(this.dictionaryListTitleNode);
...
//				var y = e.event.y;
    ...
    createListDictionaryItem: function(dictionary, isNew){
        ...
        if( dictionary.id === this.options.id ){
            listDictionaryItem.setStyles(this.css.listDictionaryItem_current);
            this.currentListDictionaryItem = listDictionaryItem;
        }

        var itemObj = {
            node: listDictionaryItem,
            data: dictionary
        };
    ...
    loadDictionaryByData: function(node, e){
        ...
        if (openNew){
            this.loadDictionaryData(dictionary.id, function(data){
                var dictionary = new MWF.xApplication.process.DictionaryDesigner.Dictionary(this, data);
                dictionary.load();
            }.bind(this), true);
        }
    },
    ...
    loadDictionary: function(){
		...
		this.getDictionaryData(this.options.id, function(ddata){
            ...
            if (this.options.readMode){
                this.dictionary = new MWF.xApplication.process.DictionaryDesigner.DictionaryReader(this, ddata);
            }else{
                this.dictionary = new MWF.xApplication.process.DictionaryDesigner.Dictionary(this, ddata);
            }
                ...
                if (this.status.openDictionarys){
                    this.status.openDictionarys.each(function(id){
                        this.loadDictionaryData(id, function(data){
                            ...
                            if (this.options.readMode){
                                var dictionary = new MWF.xApplication.process.DictionaryDesigner.DictionaryReader(this, data, {"showTab": showTab});
                            }else{
                                var dictionary = new MWF.xApplication.process.DictionaryDesigner.Dictionary(this, data, {"showTab": showTab});
                            }
        ...
        if (this.tab.showPage){
            ...
            dictionary.save(function(){
                if (dictionary==this.dictionary){
                    ...
                    this.setTitle(MWF.APPDD.LP.title + "-"+name);
