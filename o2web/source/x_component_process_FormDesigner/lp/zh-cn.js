MWF.xApplication.process.FormDesigner.LP = {
    ...
    importO2_infor:
        '请将O2格式的表单数据拷贝到以下编辑器中。（使用表单或页面设计器工具条上的“导出”按钮，获取表单数据）<br/>按下Ctrl+Alt+I可格式化数据',
    ...
    propertyTemplate: {
        ...
        checkOtherFlowNote:
            '当加签、重置、退回时执行该校验脚本,通过this.form.currentRouteName获取流转方式（加签为：addTask,重置为: reset,退回为：goBack），通过this.form.opinion获取意见，通过this.form.flowData获取流转的其他参数',
        ...
        allowDropNote:
            "this.event 可以获取数据 { draggingNode, dropNode, type }。type 参数有三种情况：'prev'、'inner' 和 'next'，分别表示放置在目标节点前、插入至目标节点和放置在目标节点后。'return true'表示允许插入。",
        loadFun: '加载子树数据脚本',
        ...
        openAtt: '打开',
        previewFirst: '优先预览',
        previewAttachmentNote:
            '下列文件格式可直接预览：zip, pdf, ofd, png, jpg, bmp, jpeg, gif, js, css, java, json, xml, php, html, htm, xhtml, log, md, txt。下列格式需安装liberoffice: doc, docx, xls, xlsx, ppt, pptx。不在上述格式中的双击直接打开附件。',
        ...
        attachmentValidationNote:
            '可以通过this.target.uploadingFiles获取当前正在上传的附件。返回true表示通过，返回文本系统提示文本后中断上传。',
        ...
        selectView: '选择视图',
        calculationView: '计算视图',
        viewScript: '视图脚本',
        nameAliasID: '名称，别名和id',
        calculation: '计算',
        filter: '过滤',
        title: '标题',
        path: '路径',
        ...
        dataScriptNote2:
            "脚本返回Promise对象或数据，如：return this.Actions.load('x_hotpic_assemble_control').HotPictureInfoAction.listForPage(1, 6);",
        dataType: '数据类型',
        text: '文本',
        number: '数字',
        currencySymbol: '货币符号',
        ...
        associatedDocumentField: '可选字段：targetTitle,targetStartTime,targetCreatorPersonCn,targetType,targetCategory,通过属性data-o2-action="delete"标识删除节点',
        ...
        tinymceInfo3:
            'queryLoad,postLoad,load和afterLoad事件为o2平台事件。其他事件为tinymce编辑器事件，可以通过this.target获取脚本环境，this.event获取事件对象，请查看',
        ...
        datatemplateOuterNote:
            '注: 位于当前数据模板外部的相关组件标识。数据模板可以有多层，外部组件标识填写有下列规则：<br/>' +
