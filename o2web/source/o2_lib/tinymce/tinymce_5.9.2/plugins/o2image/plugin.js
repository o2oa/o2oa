...
(function () {
    ...
    var read = function (normalizeCss, image) {
      ...
      return {
        ...
        width: getAttrib(image, 'data-width') || getSize(image, 'width'),
        height: getAttrib(image, 'data-height') || getSize(image, 'height'),
        ...
        isDecorative: getIsDecorative(image),
          "data-id": getAttrib(image, 'data-id'),
            "data-orgid": getAttrib(image, 'data-orgid'),
    ...
    var write = function (normalizeCss, newData, image) {
      ...
      updateAlt(image, oldData, newData);
        updateProp(image, oldData, newData, 'data-id', updateAttrib);
        updateProp(image, oldData, newData, 'data-orgid', updateAttrib);
        updateProp(image, oldData, newData, 'data-width', updateAttrib);
        updateProp(image, oldData, newData, 'data-height', updateAttrib);
        updateProp(image, oldData, newData, 'onerror', updateAttrib);
        updateProp(image, oldData, newData, 'data-prv', updateAttrib);
    };
    ...
    var toImageData = function (data, removeEmptyAlt) {
      return {
        ...
        isDecorative: data.isDecorative,
          ...
          "data-width": data["data-width"],
          "data-height": data["data-height"],
          "onerror": data["onerror"],
          "data-prv": data["data-prv"]
      };
    };
    ...
    var formFillFromMeta2 = function (info, data, meta) {
      ...
      if (info.hasAdvTab) {
        ...
        if (isString(meta["data-id"])) {
            data["data-id"] = meta["data-id"];
        }
        if (isString(meta["data-orgid"])) {
            data["data-orgid"] = meta["data-orgid"];
        }
        if (isString(meta["data-width"])) {
            data["data-width"] = meta["data-width"];
        }
        if (isString(meta["data-height"])) {
            data["data-height"] = meta["data-height"];
        }
        ...
        return {
          ...
          onClose: closeHandler(state),
            onTabChange: function(api, details){
                tinymce.dom.DomQuery(".tox-lock").css("display","none");
            },
    ...
    var submitHandler = function (editor) {
      return function (info) {
        return function (api) {
          ...
          if( apiData.src && apiData.src.meta ){
              var meta = apiData.src.meta;
              for( var key in meta ){
    ...
    var Dialog = function (editor) {
      ...
      var open = function () {
        collect(editor).then(makeDialog(helpers)).then(function (arg) {
            tinymce.activeEditor.activeO2ImageDialog = editor.windowManager.open(arg);
            tinymce.dom.DomQuery(".tox-lock").css("display","none");
        });
      };
      return { open: open };
    ...
    var register = function (editor) {
      editor.ui.registry.addToggleButton('o2image', {
        icon: 'image',
      ...
      editor.ui.registry.addMenuItem('o2image', {
        icon: 'image',
        text: 'Image...',
        onAction: Dialog(editor).open
      });
      editor.ui.registry.addContextMenu('o2image', {
    ...
    function Plugin () {
      global$5.add('o2image', function (editor) {
        setup(editor);
        register(editor);
        register$1(editor);
      });
