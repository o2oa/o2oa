        ...
        return ir.toBlob().then(function (blob) {
            ...
            editor.undoManager.transact(function () {
                ...
                if( ( base64enable && isBlob ) || images_upload_handler ) {
                    ...
                    var prv = selectedImage && selectedImage.dataset ? selectedImage.dataset.prv : "true";
                    var name = selectedImage && selectedImage.dataset ? selectedImage.dataset.filename : "";
                    if( !name )name = editor.$(selectedImage).attr("alt") || blobInfo.name();
                    var names = name.split(".");

                    // var suffix = names.length > 1 ? names[names.length-1] : "";
                    // suffix = suffix.toLowerCase();
                    // if( suffix === "jpg" )suffix = "jpeg"
                    // var fileType = suffix ? ("image/"+suffix) : blob.type;

                    var type = (blob.type||"").split("/")[1];
                    names[names.length-1] = type || "png";
                    name = names.join(".");
                    ...
                    var file = blobInfo.blob();
                    // }else{
                    //     var src=window.atob(ir.toBase64());
                    //     var ia = new Uint8Array(src.length);
                    //     for (var i = 0; i < src.length; i++) {
                    //         ia[i] = src.charCodeAt(i);
                    //     }
                    //     file = new Blob([ia], {type: blob.type });
                    // }
                    file.name = name;
                    var formData = new FormData();
                    formData.append('file', file, name );

                    // new Element("img.test111", {
                    //     src: ir.toDataURL()
