...
(function() {
    ...
    this.insertMatch = function(data, options) {
        ...
        if (data.completer && data.completer.insertMatch) {
            ...
            if (completions.filterText || data.offset) {
                var ranges = this.editor.selection.getAllRanges();
                for (var i = 0, range; range = ranges[i]; i++) {
                    if (completions.filterText) range.start.column -= completions.filterText.length;
                    if (data.offset) range.start.column += data.offset;
...
(function(){
    this.setFilter = function(str) {
        ...
        this.filterText = str;
        if (this.filterText) matches = this.filterCompletions(matches, this.filterText);
