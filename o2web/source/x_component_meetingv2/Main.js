...
MWF.xDesktop.requireApp("meetingv2", "MeetingView", null, false);
MWF.xDesktop.requireApp("meetingv2", "Common", null, false);
MWF.xDesktop.requireApp("Template", "MDomItem", null, false);
MWF.xApplication.meetingv2.options.multitask = false;
MWF.xApplication.meetingv2.Main = new Class({
    ...
    options: {
        ...
        "sideBarEnable" : true,
        "settingEnable" : true,
        "title": MWF.xApplication.meetingv2.LP.title
    },
    onQueryLoad: function(){
        this.lp = MWF.xApplication.meetingv2.LP;
        this.menuMode="show";
        ...
        //if (!this.actions) this.actions = new MWF.xApplication.meetingv2.Actions.RestActions();
    ...
    createTopMenu_right : function(text, icon, action){
        ...
        //actionIconNode.setStyle("background", "url(../x_component_meetingv2/$Main/default/icon/"+icon+".png) no-repeat center center");
        ...
        actionNode.addEvents({
            "mouseover": function(){
                ...
                //this.node.retrieve("iconNode").setStyle( "background","url(../x_component_meetingv2/$Main/default/icon/"+this.node.retrieve("icon")+"_click.png) no-repeat center center" );
            ...
            "mouseout": function(){
                ...
                //this.node.retrieve("iconNode").setStyle( "background","url(../x_component_meetingv2/$Main/default/icon/"+this.node.retrieve("icon")+".png) no-repeat center center" );
            }.bind({ node:actionNode }),
            "click": function(){
        ...
        actionNode.addEvents({
            ...
            "click": function(){
                ...
                if( _self.currentTopMenuNode && this.node != _self.currentTopMenuNode){
                    ...
                    //_self.currentTopMenuNode.retrieve("iconNode").setStyle( "background","url(../x_component_meetingv2/$Main/default/icon/"+_self.currentTopMenuNode.retrieve("icon")+".png) no-repeat center center" );
                }
    ...
    getMyMeetingView: function(callback){
        if (!this.myMeetingView){
            MWF.xDesktop.requireApp("meetingv2", "MeetingView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }
                this.myMeetingView = new MWF.xApplication.meetingv2.MeetingView(this.contentNode, this, options);
    ...
    getListView: function(callback){
        if (!this.listView){
            MWF.xDesktop.requireApp("meetingv2", "ListView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }
                this.listView = new MWF.xApplication.meetingv2.ListView(this.contentNode, this, options);
                if( options )this.status.options = null;
                if (callback) callback();
            }.bind(this));
        }else{
    ...
    getMonthView: function(callback){
        if (!this.monthView){
            MWF.xDesktop.requireApp("meetingv2", "MonthView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }
                this.monthView = new MWF.xApplication.meetingv2.MonthView(this.contentNode, this, options);
                if( options )this.status.options = null;
                if (callback) callback();
            }.bind(this));
        }else{
    ...
    getWeekView: function(callback){
        if (!this.weekView){
            MWF.xDesktop.requireApp("meetingv2", "WeekView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }
                this.weekView = new MWF.xApplication.meetingv2.WeekView(this.contentNode, this, options);
                if( options )this.status.options = null;
                if (callback) callback();
            }.bind(this));
        }else{
    ...
    getDayView: function(callback, d){
        if (!this.dayView){
            MWF.xDesktop.requireApp("meetingv2", "DayView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }
                this.dayView = new MWF.xApplication.meetingv2.DayView(this.contentNode, this, options || {"date": d});
    ...
    getRoomView: function(callback){
        if (!this.roomView){
            MWF.xDesktop.requireApp("meetingv2", "RoomView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }
                this.roomView = new MWF.xApplication.meetingv2.RoomView(this.contentNode, this, options);
                if( options )this.status.options = null;
                if (callback) callback();
            }.bind(this));
        }else{
    ...
    addMeeting: function(date, hour, minute, room, processData, latest){
        o2.Actions.load("x_meeting_assemble_control").ConfigAction.getSystemConfig(function(json){
            ...
            if (process){
                ...
                //new MWF.xApplication.meetingv2.Creator(this, date, hour, room);
                var form = new MWF.xApplication.meetingv2.MeetingForm(this,{}, {
                    date : date,
                    hour : hour,
                    minute : minute,
                    room : room
    ...
    config: function(){
        new MWF.xApplication.meetingv2.Config(this, this.meetingConfig);
        //this.hideMenu();
    },
    addRoom : function(){
        var form = new MWF.xApplication.meetingv2.RoomForm(this,{}, {}, {app:this});
        form.view = this;
        form.create();
        //this.hideMenu();
    },
    ...
    loadSideBar : function(){
        this.sideBar = new MWF.xApplication.meetingv2.SideBar(this.node, this);
        this.sideBar.show();
    },
...
MWF.xApplication.meetingv2.Config = new Class({
    Implements: [Events],
    ...
    load: function(){
        ...
        if( !d.disableViewList.contains( "toMyMeeting" )  ){
            html +=
                "   <div style='"+ viewStyle +"'>" +
                "<input type='radio' name='configSelectDefaultView' "+(((!d.defaultView) || d.defaultView=="toMyMeeting") ? "checked" : "")+" value='toMyMeeting'>"+ o2.txt( d.toMyMeetingViewName || this.lp.myMeeting )+
                "   </div>";
        }
        ...
        if( !d.disableViewList.contains( "toRoom" )  ){
            html +=
                "   <div style='"+ viewStyle +"'>" +
                "<input type='radio' name='configSelectDefaultView' "+((d.defaultView=="toRoom") ? "checked" : "")+" value='toRoom'>"+ o2.txt(d.toRoomViewName || this.lp.room)+
                "   </div>";
        }
