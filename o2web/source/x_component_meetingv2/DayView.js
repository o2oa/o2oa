MWF.xApplication.meetingv2.DayView = new Class({
    ...
    initialize: function(node, app, options){
        ...
        this.path = "../x_component_meetingv2/$DayView/";
        this.cssPath = "../x_component_meetingv2/$DayView/"+this.options.style+"/css.wcss";
        this._loadCss();
        this.app = app;
        this.container = $(node);
    ...
    incrementDay : function(){
        if( this.days.length > 1 ) {
            ...
            var day = new MWF.xApplication.meetingv2.DayView.Day(this, node, "after", date, false );
            this.days.push(day);
        }else if( this.days.length == 1 ){
            var date = this.date = this.days[this.days.length-1].date.clone().increment();
            this.days[0].destroy();
            this.days.splice(0, 1);
            var day = new MWF.xApplication.meetingv2.DayView.Day(this, this.dayContainerNode, null, date, false );
            this.days.push(day);
        }
    },
    decrementDay : function( node ){
        if( this.days.length > 1 ){
            ...
            var day = new MWF.xApplication.meetingv2.DayView.Day(this, node, "before", date, true);
            ...
            this.days[this.days.length-1].destroy();
            this.days.splice(this.days.length-1, 1);
            var day = new MWF.xApplication.meetingv2.DayView.Day(this, this.dayContainerNode, null, date, true);
            this.days.unshift( day )
        }
    },
    loadDay: function( count, setFirst ){
        ...
        for( var i = 0; i< ( count || this.dayCount ); i++ ){
            var day = new MWF.xApplication.meetingv2.DayView.Day(this, this.dayContainerNode, null,date, setFirst && i==0);
            this.days.push( day );
            date.increment();
        }
...
MWF.xApplication.meetingv2.DayView.Day = new Class({
    Implements: [Events],
        ...
        this.app.actions[ isAll ? "listMeetingDayAll" : "listMeetingDay" ](y, m, d, function(json){
            ...
            if( !json.data || json.data.length == 0 ){
                ...
                json.data.each(function(meeting, i){
                    if (!meeting.myReject){
                        flag = false;
                        this.meetings.push(new MWF.xApplication.meetingv2.DayView.meetingv2(this.dayContentNode, this, meeting));
                    }
                }.bind(this));
            }
            if( flag ){
...
MWF.xApplication.meetingv2.DayView.meetingv2 = new Class({
