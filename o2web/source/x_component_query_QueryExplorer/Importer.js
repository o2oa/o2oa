MWF.xApplication.query.QueryExplorer.Importer = new Class({
    ...
    readyProrressBar: function(){
        ...
        this.status = {
            "count": this.applicationJson.viewList.length+this.applicationJson.statList.length+1,
            "complete": 0
        }
    },
    doImportApplication: function(){
        ...
        this.actions.action.invoke({"name": "addApplication","data": this.applicationJson.application,"success": function(){
            ...
            this.importOverwriteViewList();
            //this.importOverwriteDictionaryList();
            this.importOverwriteStatList();
    ...
    // importOverwritePageList: function(){
    //     this.actions.listPage(this.applicationJson.application.id, function(json){
    //         var formList = json.data || [];
    //         this.applicationJson.pageList.each(function(form){
    ...
    //             if (id) {
    //                 form.id = id;
    //                 this.actions.action.invoke({"name": "updatePage","data": form,"parameter": {"id": form.id},"success": function(){
    //                     this.progressBarTextNode.set("text", "Import Page \""+form.name+"\" ...");
    //                     this.checkExport();
    //                 }.bind(this)});
    //             }else{
    //                 this.actions.action.invoke({"name": "addPage","data": form,"parameter": {"id": form.id},"success": function(){
    //                     //this.progressBarTextNode.set("text", "Import Page \""+form.name+"\" ...");
    //                     this.checkExport();
    //                 }.bind(this)});
    //             }
    ...
    //    }.bind(this));
    //},
    importOverwriteViewList: function(){
        this.actions.listView(this.applicationJson.application.id, function(json){
            var viewList = json.data || [];
            this.applicationJson.viewList.each(function(view){
                view.application = this.applicationJson.application.id;
                var id = "";
                for (var i=0; i<viewList.length; i++){
                    if (viewList[i].name==view.name){
                        id = viewList[i].id;
                        break;
                    }
                }
                var data = JSON.decode(view.data);
                view.data = data;
                if (id){
                    view.id = id;
                    this.actions.updateView(view, function(){
                        this.progressBarTextNode.set("text", "Import View \""+view.name+"\" ...");
                        this.checkExport();
                    }.bind(this));
                }else{
                    this.actions.addView(view, function(){
                        this.progressBarTextNode.set("text", "Import View \""+view.name+"\" ...");
                        this.checkExport();
                    }.bind(this));
                }
    ...
    importOverwriteStatList: function(){
        this.actions.listStat(this.applicationJson.application.id, function(json){
            var statList = json.data || [];
            this.applicationJson.statList.each(function(stat){
                stat.application = this.applicationJson.application.id;
                var id = "";
                for (var i=0; i<statList.length; i++){
                    if (statList[i].name==stat.name){
                        id = statList[i].id;
                        break;
                    }
                }
                var data = JSON.decode(stat.data);
                stat.data = data;
                if (id){
                    stat.id = id;
                    this.actions.updateStat(stat, function(){
                        this.progressBarTextNode.set("text", "Import Stat \""+stat.name+"\" ...");
                        this.checkExport();
                    }.bind(this));
                }else{
                    this.actions.addStat(stat, function(){
                        this.progressBarTextNode.set("text", "Import Stat \""+stat.name+"\" ...");
                        this.checkExport();
                    }.bind(this));
                }
