...
MWF.xApplication.orggrid.PersonExplorer.PersonContent.BaseInfor = new Class({
    ...
    save: function(){
        ...
        return o2.Actions.load("x_general_assemble_control").SecurityClearanceAction.enable().then(function(json){
            ...
            var propertyContentScrollNode = this.content && this.content.propertyContentScrollNode;

            if( propertyContentScrollNode )propertyContentScrollNode.mask({
                "style": {
                    "opacity": 0.7,
            ...
            this.savePerson(function(){
                this.cancel();
                if( propertyContentScrollNode )propertyContentScrollNode.unmask();
            }.bind(this), function(xhr, text, error){
                var errorText = error;
                if (xhr){
    ...
    savePerson: function(callback, cancel){
        ...
        this._savePerson(data, callback, cancel);
    },
    _savePerson: function (data, callback, cancel){
...
MWF.xApplication.orggrid.PersonExplorer.PersonContent.BaseInforDialog = new Class({
    ...
    openDialog: function ( mode ){
        ...
        var _self = this;
        var title;
        this.mode = mode === 'create' ? 'edit' : 'read';
        switch(mode){
            case 'edit': title = this.explorer.app.lp.editPerson; break;
            case 'read': title = this.explorer.app.lp.viewPerson; break;
            default: title = this.explorer.app.lp.addPerson; break;
        }
        this.dialog = o2.DL.open({
            ...
            "onQueryClose": function (){
                if( _self.tooltip ){
                    _self.tooltip.destroy();
                    _self.tooltip = null;
                }
            },
            "onPostLoad": function (){
                _self.contentNode = this.content;
                _self.load();
                _self.editContentNode.setStyle('width', '94%');
                if( _self.mode === 'edit' ){
                    _self.edit();
                }
            }
    ...
    _savePerson: function (data){
        this.actions.savePerson(data, function(json){
            this.actions.getPerson(function(json){
                Object.merge(this.data, json.data);
