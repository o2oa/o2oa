var MWFCalendarv2MonthView = MWF.xApplication.calendarv2.MonthView = new Class({
    ...
    initialize: function(node, app, options){
        ...
        this.path = "../x_component_calendarv2/$MonthView/";
        this.cssPath = "../x_component_calendarv2/$MonthView/"+this.options.style+"/css.wcss";
        this._loadCss();
        this.app = app;
    ...
    loadCalendar: function(){
        ...
        this.calendar = new MWFCalendarv2MonthView.Calendar(this, date  );
...
MWFCalendarv2MonthView.Calendar = new Class({
    Implements: [Events],
    ...
    resetBodySize: function(){
        ...
        MWFCalendarv2MonthView.WeekWidth = tableSize.x;
        MWFCalendarv2MonthView.DayWidth = tableSize.x / 7;
        this.dataTdList.each( function( td ){
            td.setStyle("width", MWFCalendarv2MonthView.WeekWidth)
        });
        ...
        var top = 30; //MWFCalendarv2MonthView.THHeight + 30;
        ...
        for( var key in this.usedYIndex ){
            ...
            if( maxLength > 4 ){
                ...
                this.dataTableList[key].setStyle("top", top );
                top =  top + MWFCalendarv2MonthView.WeekHeight + 1;
                trs[ parseInt(key) ].getElements("td").each( function(td){
                    td.setStyle("height", MWFCalendarv2MonthView.WeekHeight )
                });
                this.calendarTrHeight.push( MWFCalendarv2MonthView.WeekHeight );
            }
        }
    ...
    createMonthSelector: function(){
        this.monthSelector = new MWFCalendarv2MonthView.MonthSelector(this.date, this);
    },
    ...
    loadDataContainer : function(){
        ...
        [0,1,2,3,4,5,6].each( function( i){
            ...
            dataTable.setStyles({
                //"display" : "none",
                "position":"absolute",
                "top" : (MWFCalendarv2MonthView.WeekHeight + 1) * i + MWFCalendarv2MonthView.THHeight + 30,
                "left" : "0px",
    ...
    loadOneDay: function( data ){
        ...
        data.each( function( d, i ){
            d.inOneDayEvents.each( function( event, i ){
                this.oneDayDocumentList.push( new MWFCalendarv2MonthView.Calendar.InOnDayDocument( this, event, d.eventDate ) );
            }.bind(this))
        }.bind(this))
    },
    loadWholeDay: function( data ){
        ...
        this.wholeDayRange.each( function( r ){
            this.wholeDayDocumentList.push(  new MWFCalendarv2MonthView.Calendar.WholeDayDocument( this, r ) );
        }.bind(this))
    },
    loadDay: function(td, date, isCreate){
        ...
        if( isCreate ){
            ...
            td.addEvent("dblclick", function(ev){
                ...
                var form = new MWF.xApplication.calendarv2.EventForm(_self,{}, {
        ...
        if( type === "today" ){
            titleDayNode.addClass("mainColor_bg");
        }
    ...
    getIndexByPage: function( page ){
        ...
        var col = (page.x - pos.x ) / MWFCalendarv2MonthView.DayWidth;
        if( col < 0 || col > 7 )return null;

        this.pageOffsetHeight = page.y - pos.y;
        var y = page.y - pos.y - MWFCalendarv2MonthView.THHeight;
        if( y < 0 )return null;
        var row = null;
    ...
    cellDrag: function(td, e){
        ...
        if(( this.pageOffsetHeight + MWFCalendarv2MonthView.WeekHeight * 1.5) > ( this.scrollNodeHeight + scrollNodeTop )){
            window.setTimeout( function(){
                this.scrollNode.scrollTo(0, scrollNodeTop + MWFCalendarv2MonthView.WeekHeight )
            }.bind(this), 200)
        }else if( this.pageOffsetHeight - MWFCalendarv2MonthView.WeekHeight * 1.5 < scrollNodeTop ){
            window.setTimeout( function(){
                this.scrollNode.scrollTo(0, scrollNodeTop - MWFCalendarv2MonthView.WeekHeight )
            }.bind(this), 200)
        }
...
MWFCalendarv2MonthView.THHeight = 50;
MWFCalendarv2MonthView.WeekHeight = 140;
MWFCalendarv2MonthView.WeekWidth;
MWFCalendarv2MonthView.DayWidth;

MWFCalendarv2MonthView.Calendar.WholeDayDocument = new Class({
    Implements: [Events],
    ...
    load: function(){
        ...
        Object.each( this.range.weekInforList, function( weekInfor ){
            this.weekList.push( new MWFCalendarv2MonthView.Calendar.WholeDayWeek(this, weekInfor, this.range.data ) )
        }.bind(this))
    },
...
MWFCalendarv2MonthView.Calendar.WholeDayWeek = new Class({
    Implements: [Events],
    ...
    createNode : function(){
        ...
        if( this.weekInfor.isEventStart ){
            this.timeNode = new Element("div",{
                ...
                text : this.timeStart.format("%m-%d %H:%M") + MWF.xApplication.calendarv2.LP.to + this.timeEnd.format("%m-%d %H:%M")
            }).inject( node );
        }
    ...
    getCoordinate : function(){
        ...
        var width = ( infor.daysIndex.length / 7 ) * MWFCalendarv2MonthView.WeekWidth - 3;
        var left = ( infor.daysIndex[0] / 7 ) * MWFCalendarv2MonthView.WeekWidth;

        //var width = ( infor.diff / MWFCalendarv2MonthView.WeekMsec  ) * MWFCalendarv2MonthView.WeekWidth - 2;
        //var left = ( infor.left / MWFCalendarv2MonthView.WeekMsec ) * MWFCalendarv2MonthView.WeekWidth + 3;
        return {
            top : top + 2,
            left : left,
            width : width
...
MWFCalendarv2MonthView.Calendar.InOnDayDocument = new Class({
    Implements: [Events],
    ...
    createNode : function(){
        ...
        this.timeNode = new Element("div",{
            ...
            text : this.timeStart.format("%H:%M") + MWF.xApplication.calendarv2.LP.to + this.timeEnd.format("%H:%M")
        }).inject( node );
    ...
    getCoordinate : function(){
        ...
        var width = MWFCalendarv2MonthView.DayWidth - 3;
        var left = this.index.dayIndex * MWFCalendarv2MonthView.DayWidth;
...
MWFCalendarv2MonthView.Document = new Class({
    ...
    load: function(){
        ...
        //    this.iconNode.setStyle("background", "url(../x_component_calendarv2/$MonthView/"+this.app.options.style+"/icon/invite.png) no-repeat center center");
        //}
    ...
    openCalendar: function(){
        this.form = new MWF.xApplication.calendarv2.CalendarForm(this,this.data, {}, {app:this.app});
        this.form.view = this;
        this.form.open();
    },
    destroy: function(){
...
MWFCalendarv2MonthView.MonthSelector = new Class({
    Implements: [Events],
