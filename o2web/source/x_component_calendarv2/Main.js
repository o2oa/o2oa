var MWFCalendarv2 = MWF.xApplication.calendarv2  = MWF.xApplication.calendarv2 || {};
MWF.require("MWF.xDesktop.UserData", null, false);
MWF.require("MWF.xAction.org.express.RestActions", null,false);
//MWF.xDesktop.requireApp("calendarv2", "Actions.RestActions", null, false);
MWF.xDesktop.requireApp("calendarv2", "Common", null, false);
MWF.xDesktop.requireApp("Template", "MDomItem", null, false);
MWF.xApplication.calendarv2.options.multitask = false;
MWFCalendarv2.LeftNaviWidth = 250;
MWF.xApplication.calendarv2.Main = new Class({
    ...
    options: {
        ...
        "isMax": true,
        "eventId" : "",
        "title": MWF.xApplication.calendarv2.LP.title
    },
    onQueryLoad: function(){
        this.lp = MWF.xApplication.calendarv2.LP;
        this.menuMode="show";
        this.isManager = MWF.AC.isAdministrator();
        this.userName = ( layout.desktop.session.user || layout.user ).distinguishedName;
        if (!this.actions) this.actions = MWF.Actions.get("x_calendar_assemble_control"); //new MWF.xApplication.calendarv2.Actions.RestActions();
        //if (!this.personActions) this.personActions = MWF.Actions.get("x_organization_assemble_express");
    },
    loadApplication: function(callback) {
        this.canlendarData = null;

        this.content.loadCss("../x_component_calendarv2/$Main/default/style.css");
    ...
    loadNaviTitleNode: function(){
        ...
        //    "src" : "../x_component_calendarv2/$Main/appicon.png"
        //}).inject(iconAreaNode);
        //
    ...
    loadLeftNavi : function(){
        ...
        this.leftNavi = new MWF.xApplication.calendarv2.Navi(this, this.naviNode, {});
    },
    ...
    createTopMenu_right : function(text, icon, action){
        ...
        // actionIconNode.setStyle("background", "url(../x_component_calendarv2/$Main/default/icon/"+icon+".png) no-repeat center center");
        ...
        actionNode.addEvents({
            "mouseover": function(){
                ...
                // this.node.retrieve("iconNode").setStyle( "background","url(../x_component_calendarv2/$Main/default/icon/"+this.node.retrieve("icon")+"_click.png) no-repeat center center" );
            ...
            "mouseout": function(){
                ...
                // this.node.retrieve("iconNode").setStyle( "background","url(../x_component_calendarv2/$Main/default/icon/"+this.node.retrieve("icon")+".png) no-repeat center center" );
            }.bind({ node:actionNode }),
            "click": function(){
    ...
    getListView: function(callback){
        if (!this.listView){
            MWF.xDesktop.requireApp("calendarv2", "ListView", function(){
                var options;
                ...
                this.listView = new MWF.xApplication.calendarv2.ListView(this.contentNode, this, options);
    ...
    getMonthView: function(callback){
        if (!this.monthView){
            MWF.xDesktop.requireApp("calendarv2", "MonthView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }else if( this.currentDate  ){
                ...
                this.monthView = new MWF.xApplication.calendarv2.MonthView(this.contentNode, this, options);
                if( options && this.status)this.status.options = null;
                if (callback) callback();
            }.bind(this));
        }else{
    ...
    getWeekView: function(callback){
        if (!this.weekView){
            MWF.xDesktop.requireApp("calendarv2", "WeekView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }else if( this.currentDate  ){
                ...
                this.weekView = new MWF.xApplication.calendarv2.WeekView(this.contentNode, this, options);
                if( options && this.status)this.status.options = null;
                if (callback) callback();
            }.bind(this));
        }else{
    ...
    getDayView: function(callback, d){
        if (!this.dayView){
            MWF.xDesktop.requireApp("calendarv2", "DayView", function(){
                var options;
                if( this.status && this.status.options ){
                    options = this.status.options
                }else if( d ){
                ...
                this.dayView = new MWF.xApplication.calendarv2.DayView(this.contentNode, this, options);
    ...
    addCalendarEvent: function(date, hour, minute, calendarId){
        MWF.UD.getPublicData("calendarConfig", function(json){
            ...
            if (process){
                ...
                //new MWF.xApplication.calendarv2.Creator(this, date, hour, room);
    ...
    addCalendar: function(){
        var form = new MWF.xApplication.calendarv2.CalendarForm(this,{}, {
        }, {app:this});
        form.view = this.leftNavi;
        form.create();
    },
    editCalendar: function( data ){
        var form = new MWF.xApplication.calendarv2.CalendarForm(this, data, {
        }, {app:this});
        form.view = this.leftNavi;
    ...
    config: function(){
        new MWF.xApplication.calendarv2.Config(this, this.calendarConfig);
        //this.hideMenu();
    },
    ...
    loadSideBar : function(){
        //this.sideBar = new MWF.xApplication.calendarv2.SideBar(this.node, this);
        //this.sideBar.show();
    },
    resizeNodes : function(){
        ...
        if( this.showLeftNavi ){
            this.rightContentNode.setStyle("width",size.x - MWFCalendarv2.LeftNaviWidth );
    ...
    openEvent : function (id) {
        ...
        this.actions.getEvent( id, function (json) {
            var form = new MWF.xApplication.calendarv2.EventForm(this, json.data, {
                isFull : true
            }, {app:this});
...
MWF.xApplication.calendarv2.Navi = new Class({
    Implements: [Options, Events],
    options : {
    ...
    loadMoreCalendarNode : function(){
        this.seeMore = new Element("div.seeMore", {
            ...
            "events" : {
                ...
                click : function(ev){
                    var form = new MWF.xApplication.calendarv2.CalendarMarket(this.app,{}, {
                    }, {app:this.app});
                    form.view = this.leftNavi;
                    form.create();
                }.bind(this)
    ...
    loadMyCalendar : function(){
        ...
        this.myCalendars.each( function( d ){
            this.myCalendarNaviItem.push( new MWF.xApplication.calendarv2.NaviItem( this, listNode, d, {
                isSelected : true
            } ) );
        }.bind(this))
    },
    loadUnitCalendar : function(){
        ...
        this.unitCalendars.each( function( d ){
            this.unitCalendarNaviItem.push( new MWF.xApplication.calendarv2.NaviItem( this, listNode, d, {
                isSelected : true
            } ));
        }.bind(this))
    },
    loadFollowCalendar : function(){
        ...
        this.followCalendars.each( function( d ){
            this.followCalendarNaviItem.push( new MWF.xApplication.calendarv2.NaviItem( this, listNode, d, {
                isSelected : true
            } ));
        }.bind(this));
...
MWF.xApplication.calendarv2.NaviItem = new Class({
    Implements: [Options, Events],
    options: {
        "style": "default",
        "index" : 0,
    ...
    loadActionsMenu : function(){
        this.menu = new MWF.xApplication.calendarv2.CalendarMenu(this.actionNode, {} , this.app, {}, this.app.node);
...
MWF.xApplication.calendarv2.CalendarMenu = new Class({
    Extends: MSelector,
    options : {
        "style": "arrow",
        "width": "150px",
        ...
        "defaultOptionLp" : MWF.xApplication.calendarv2.LP.font,
        "textField" : "name",
        "valueField" : "val",
        "event" : "mouseenter",
...
MWF.xApplication.calendarv2.CalendarMarket = new Class({
    Extends: MPopupForm,
    Implements: [Options, Events],
    options: {
        "style": "meeting",
        ...
        "maxAction" : true,
        "closeAction": true,
        "title" : MWF.xApplication.calendarv2.LP.calendarMarket
    },
        ...
        this.app.actions.listPublicCalendar( function( json ){
            if( !json.data  || json.data.length == 0 ){
                ...
                ( json.data || [] ).each( function(d){
                    ...
                    //}).inject( topNode );
                    //typeNode.setStyle( "background-color" , MWFCalendarv2.ColorOptions.getLightColor(d.color ) );
                }.bind(this))
            }
...
MWF.xApplication.calendarv2.Config = new Class({
    Implements: [Events],
