...
//		this.processData = this.options.processData;
    ...
    createListTitleNodes: function (){
        this.agentListTitleNode.setStyle("display", 'flex');

        this.titleActionArea = new Element("div", {
            styles: this.css.titleActionArea
        }).inject(this.agentListTitleNode);
...
//				var y = e.event.y;
    ...
    createListAgentItem: function(agent, isNew){
        ...
        if( agent.id === this.options.id ){
            listAgentItem.setStyles(this.css.listAgentItem_current);
            this.currentListAgentItem = listAgentItem;
        }
...
//				var y = e.event.y;
	...
	resizeNode: function(){
        if (!this.isMax){
            ...
            this.agentListAreaSccrollNode.setStyle("height", ""+(y-leftToolbarSize.y)+"px");
