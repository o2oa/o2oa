...
import com.x.okr.assemble.control.jaxrs.okrconfigsystem.WrapInOkrConfigSystem;
...
public class OkrConfigSystemService{
	private Logger logger = LoggerFactory.getLogger( OkrConfigSystemService.class );
	private BeanCopyTools<WrapInOkrConfigSystem, OkrConfigSystem> wrapin_copier = BeanCopyToolsBuilder.create( WrapInOkrConfigSystem.class, OkrConfigSystem.class, null, WrapInOkrConfigSystem.Excludes );
	/**
	 ...
	 * @param wrapIn
	 */
	public OkrConfigSystem save( WrapInOkrConfigSystem wrapIn ) throws Exception {
		...
		//根据ID查询信息是否存在，如果存在就update，如果不存在就create
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				...
				if( okrConfigSystem != null ){
					...
					wrapin_copier.copy( wrapIn, okrConfigSystem );
