...
import com.x.okr.assemble.control.jaxrs.okrconfigworklevel.WrapInOkrConfigWorkLevel;
...
public class OkrConfigWorkLevelService{
	private Logger logger = LoggerFactory.getLogger( OkrConfigWorkLevelService.class );
	private BeanCopyTools<WrapInOkrConfigWorkLevel, OkrConfigWorkLevel> wrapin_copier = BeanCopyToolsBuilder.create( WrapInOkrConfigWorkLevel.class, OkrConfigWorkLevel.class, null, WrapInOkrConfigWorkLevel.Excludes );
	/**
	 ...
	 * @param wrapIn
	 */
	public OkrConfigWorkLevel save( WrapInOkrConfigWorkLevel wrapIn ) throws Exception {
		...
		//根据ID查询信息是否存在，如果存在就update，如果不存在就create
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				...
				if( okrConfigWorkLevel != null ){
					...
					wrapin_copier.copy( wrapIn, okrConfigWorkLevel );
