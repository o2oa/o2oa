...
public class UUIDAction extends AbstractJaxrsAction {
	...
	public Response getUUID( @Context HttpServletRequest request ) {
		...
		try {
			proxyIndentity = okrUserInfoService.getLoginIdentity( currentPerson.getName() );
		} catch (Exception e1) {
			logger.error( "system get login indentity with person name got an exception", e1 );
		}
		try {
			uuid = UUID.randomUUID().toString();
			logger.info( "user["+ currentPerson.getName() +"][proxy:'"+ proxyIndentity +"'] get a new UUID：" + uuid );
			data.add( uuid );
			result.setData( data );
		} catch (Exception e) {
			logger.error( "user["+ currentPerson.getName() +"][proxy:'"+ proxyIndentity +"'] get a new UUID error！", e );
		}
