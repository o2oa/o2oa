...
import com.x.okr.assemble.control.jaxrs.okrconfigsercretary.WrapInOkrConfigSecretary;
import com.x.okr.entity.OkrConfigSecretary;

public class OkrConfigSecretaryService {
	
	private Logger logger = LoggerFactory.getLogger( OkrConfigSecretaryService.class );
	
	private BeanCopyTools<WrapInOkrConfigSecretary, OkrConfigSecretary> wrapin_copier = BeanCopyToolsBuilder.create( WrapInOkrConfigSecretary.class, OkrConfigSecretary.class, null, WrapInOkrConfigSecretary.Excludes );
	 ...
	 * @param wrapIn
	 */
	public OkrConfigSecretary save( WrapInOkrConfigSecretary wrapIn ) throws Exception {
		...
		if( wrapIn.getId() !=null && wrapIn.getId().trim().length() > 20 ){
			...
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {	
				...
				if( okrConfigSecretary != null ){
					...
					wrapin_copier.copy( wrapIn, okrConfigSecretary );
				...
				logger.error( "OkrConfigSecretary update/ got a error!" );
				throw e;
			}
		}else{//没有传入指定的ID
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {	
				...
				logger.error( "OkrConfigSecretary create got a error!", e);
				throw e;
			}
		}
