...
import com.x.okr.assemble.control.jaxrs.okrworkreportdetailinfo.WrapInOkrWorkReportDetailInfo;
...
public class OkrWorkReportDetailInfoService{
	private Logger logger = LoggerFactory.getLogger( OkrWorkReportDetailInfoService.class );
	private BeanCopyTools<WrapInOkrWorkReportDetailInfo, OkrWorkReportDetailInfo> wrapin_copier = BeanCopyToolsBuilder.create( WrapInOkrWorkReportDetailInfo.class, OkrWorkReportDetailInfo.class, null, WrapInOkrWorkReportDetailInfo.Excludes );
	/**
	 ...
	 * @param wrapIn
	 */
	public OkrWorkReportDetailInfo save( WrapInOkrWorkReportDetailInfo wrapIn ) throws Exception {
		...
		//根据ID查询信息是否存在，如果存在就update，如果不存在就create
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				...
				if( okrWorkReportDetailInfo != null ){
					...
					wrapin_copier.copy( wrapIn, okrWorkReportDetailInfo );
