...
@WebServlet(urlPatterns = "/servlet/download/centerattachment/*")
public class CenterWorkAttachmentDownloadServlet extends AbstractServletAction {

	private static final long serialVersionUID = -4314532091497625540L;
	private Logger logger = LoggerFactory.getLogger(CenterWorkAttachmentDownloadServlet.class);

	@HttpMethodDescribe(value = "下载附件 servlet/download/centerattachment/{id}/stream", response = Object.class)
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (check) {
			try {
				part = this.getURIPart(request.getRequestURI(), "centerattachment");
	...
	private void setResponseHeader(HttpServletResponse response, boolean streamContentType,
		...
		if (streamContentType) {
			...
			response.setHeader("Content-Disposition",
					"fileInfo; filename=" + URLEncoder.encode(okrAttachmentFileInfo.getName(), "utf-8"));
		} else {
			response.setHeader("Content-Type",
					Config.mimeTypes().getMimeByExtension("." + okrAttachmentFileInfo.getExtension()));
			response.setHeader("Content-Disposition",
					"inline; filename=" + URLEncoder.encode(okrAttachmentFileInfo.getName(), "utf-8"));
		}
