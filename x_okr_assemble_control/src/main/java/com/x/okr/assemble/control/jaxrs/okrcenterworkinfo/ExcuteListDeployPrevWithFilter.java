...
public class ExcuteListDeployPrevWithFilter extends ExcuteBase {

	private Logger logger = LoggerFactory.getLogger( ExcuteListDeployPrevWithFilter.class );
			...
			if(check){
				if( wraps != null && !wraps.isEmpty() ){
					for( WrapOutOkrCenterWorkViewInfo wrap : wraps ){
						...
						operations = new ExcuteListOperationWithId().execute( request, effectivePerson, okrUserCache, wrap.getId() );
