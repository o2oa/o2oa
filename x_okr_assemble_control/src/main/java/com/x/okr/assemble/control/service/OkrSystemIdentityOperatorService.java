...
public class OkrSystemIdentityOperatorService {
	private Logger logger = LoggerFactory.getLogger( OkrSystemIdentityOperatorService.class );
...
//		systemIdentityQueryService.checkIdentities( identities, identities_ok, identities_error );			
	...
	/**
	 * 将所有的数据中相关的身份修改为指定的身份，并且修改相应的姓名，组织以及公司信息，如果需要的话
	 * 
	...
	public Boolean changeUserIdentity( String fromIdentityName, String toIdentityName, String recordType, String tableName, String recordId ) throws Exception{
		...
		WrapPerson person = okrUserManagerService.getUserByIdentity( toIdentityName );
		
		OkrUserCache toUserCache = new OkrUserCache();
		toUserCache.setLoginUserName( person.getName() );
		toUserCache.setLoginIdentityName( toIdentityName );
		toUserCache.setLoginUserOrganizationName( okrUserManagerService.getDepartmentNameByIdentity( toIdentityName ) );
		toUserCache.setLoginUserCompanyName( okrUserManagerService.getCompanyNameByIdentity( toIdentityName ) );
		
		//判断是不是由于组织调整导致身份无效，如果是同一个人，那么是所有记录的身份都可以转过来的，除了处理日志
		//如果不是同一个人，那么部分身份是不应该转到新的员工身份的，如已办
		if( fromIdentityName.split("\\(") != null && fromIdentityName.split("\\(").length > 0 && fromIdentityName.split("\\(")[0].trim().equalsIgnoreCase( person.getName() )){
			isSameUser = true;
		}
