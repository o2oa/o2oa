...
public class OkrStatisticReportStatusService{
	private Logger logger = LoggerFactory.getLogger( OkrStatisticReportStatusService.class );
	...
	public List<String> listIds(String centerId, String workId, String organization, String cycleType, String status ) throws Exception {
		...
		try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create() ) {
			business = new Business( emc );
			return business.okrStatisticReportStatusFactory().listIds( centerId, workId, organization, cycleType, status );
		}catch( Exception e ){
			throw e;
		}
	}
