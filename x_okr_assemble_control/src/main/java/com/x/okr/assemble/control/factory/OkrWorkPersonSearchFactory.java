...
public class OkrWorkPersonSearchFactory extends AbstractFactory {
	 ...
	 * @throws Exception 
	 */
	public List<OkrWorkPerson> listNextForCenterInfoWithFilter(String id, Integer count, Object sequence,
			com.x.okr.assemble.control.jaxrs.okrcenterworkinfo.WrapInFilter wrapIn) throws Exception {
	 ...
	 * @throws Exception
	 */
	public long getCountForCenterInfoWithFilter( com.x.okr.assemble.control.jaxrs.okrcenterworkinfo.WrapInFilter wrapIn ) throws Exception {
		//先获取上一页最后一条的sequence值，如果有值的话，以此sequence值作为依据取后续的count条数据
		EntityManager em = this.entityManagerContainer().get( OkrWorkPerson.class );
		List<Object> vs = new ArrayList<>();
		StringBuffer sql_stringBuffer = new StringBuffer();
