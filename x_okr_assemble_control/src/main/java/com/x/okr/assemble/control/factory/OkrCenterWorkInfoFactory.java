...
public class OkrCenterWorkInfoFactory extends AbstractFactory {
	...
	public List<String> listAllDistinctAuditLeaderIdentity(List<String> identities_ok, List<String> identities_error) throws Exception {
		...
		if( identities_ok != null && identities_ok.size() > 0 ){
			p = cb.and( p, cb.not(root.get( OkrCenterWorkInfo_.auditLeaderIdentity ).in( identities_ok )) );
		}
		if( identities_error != null && identities_error.size() > 0 ){
			p = cb.and( p, cb.not(root.get( OkrCenterWorkInfo_.auditLeaderIdentity ).in( identities_error )) );
		}
		cq.distinct(true).select(root.get( OkrCenterWorkInfo_.auditLeaderIdentity ));
		return em.createQuery(cq.where(p)).getResultList();
	}
	/**
	 * 查询中心工作创建者身份列表（去重复）
	...
	public List<String> listAllDistinctReportAuditLeaderIdentity(List<String> identities_ok, List<String> identities_error) throws Exception {
		...
		Predicate p = cb.isNotNull( root.get( OkrCenterWorkInfo_.id ) );
		if( identities_ok != null && identities_ok.size() > 0 ){
			p = cb.and( p, cb.not(root.get( OkrCenterWorkInfo_.reportAuditLeaderIdentity ).in( identities_ok )) );
		}
		if( identities_error != null && identities_error.size() > 0 ){
			p = cb.and( p, cb.not(root.get( OkrCenterWorkInfo_.reportAuditLeaderIdentity ).in( identities_error )) );
		}
		cq.distinct(true).select(root.get( OkrCenterWorkInfo_.reportAuditLeaderIdentity ));
		return em.createQuery(cq.where(p)).getResultList();
	}
	...
	public List<OkrCenterWorkInfo> listErrorIdentitiesInCenterInfo( String identity, String recordId ) throws Exception {
		...
		Predicate p_reportAuditLeaderIdentity = cb.isNotNull(root.get( OkrCenterWorkInfo_.reportAuditLeaderIdentity ));
		p_reportAuditLeaderIdentity = cb.and( p_reportAuditLeaderIdentity, cb.like( root.get( OkrCenterWorkInfo_.reportAuditLeaderIdentity ), "%"+identity+"%" ) );
		
		Predicate p_auditLeaderIdentity = cb.isNotNull(root.get( OkrCenterWorkInfo_.auditLeaderIdentity ));
		p_auditLeaderIdentity = cb.and( p_auditLeaderIdentity, cb.like( root.get( OkrCenterWorkInfo_.auditLeaderIdentity ), "%"+identity+"%" ) );
