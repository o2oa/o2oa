...
import com.x.okr.assemble.control.jaxrs.okrattachmentfileinfo.WrapInOkrAttachmentFileInfo;
...
public class OkrAttachmentFileInfoService{
	private Logger logger = LoggerFactory.getLogger( OkrAttachmentFileInfoService.class );
	private BeanCopyTools<WrapInOkrAttachmentFileInfo, OkrAttachmentFileInfo> wrapin_copier = BeanCopyToolsBuilder.create( WrapInOkrAttachmentFileInfo.class, OkrAttachmentFileInfo.class, null, WrapInOkrAttachmentFileInfo.Excludes );
	/**
	 ...
	 * @param wrapIn
	 */
	public OkrAttachmentFileInfo save( WrapInOkrAttachmentFileInfo wrapIn ) throws Exception {
		...
		//根据ID查询信息是否存在，如果存在就update，如果不存在就create
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				...
				if( okrAttachmentFileInfo != null ){
					...
					wrapin_copier.copy( wrapIn, okrAttachmentFileInfo );
