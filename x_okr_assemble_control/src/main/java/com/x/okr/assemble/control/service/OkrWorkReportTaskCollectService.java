...
public class OkrWorkReportTaskCollectService {
	private Logger logger = LoggerFactory.getLogger( OkrWorkReportTaskCollectService.class );
	...
	public boolean checkReportCollectTask( String userIdentity, List<String> workTypeList ) throws Exception {
		...
		if ( person != null ) {
				...
				try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create() ) {
					...
					if ( taskCount != null && taskCount > 0 ) {
						if ( okrTasks == null || okrTasks.isEmpty() ) {
							...
							okrTask.setTargetName( person.getName() );
							okrTask.setTargetOrganizationName( okrUserManagerService.getDepartmentNameByIdentity(userIdentity));
							okrTask.setTargetCompanyName( okrUserManagerService.getCompanyNameByIdentity(userIdentity));
			...
			throw new Exception( "OkrWorkReportTaskCollectService : person is not exists, identity:" + userIdentity);
		}
		return true;
	}
}
