...
import com.x.okr.assemble.control.jaxrs.okrworkreportprocesslog.WrapInOkrWorkReportProcessLog;
...
public class OkrWorkReportProcessLogService{
	private Logger logger = LoggerFactory.getLogger( OkrWorkReportProcessLogService.class );
	private BeanCopyTools<WrapInOkrWorkReportProcessLog, OkrWorkReportProcessLog> wrapin_copier = BeanCopyToolsBuilder.create( WrapInOkrWorkReportProcessLog.class, OkrWorkReportProcessLog.class, null, WrapInOkrWorkReportProcessLog.Excludes );
	/**
	 ...
	 * @param wrapIn
	 */
	public OkrWorkReportProcessLog save( WrapInOkrWorkReportProcessLog wrapIn ) throws Exception {
		...
		//根据ID查询信息是否存在，如果存在就update，如果不存在就create
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				...
				if( okrWorkReportProcessLog != null ){
					...
					wrapin_copier.copy( wrapIn, okrWorkReportProcessLog );
