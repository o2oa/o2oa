...
public class ReportAttachmentDownloadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( check ){
			try {
				part = FileUploadServletTools.getURIPart(request.getRequestURI(), "reportattachment" );
		...
		if( check ){
			try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create() ) {
				...
				if ( null == okrAttachmentFileInfo ) {
					check = false;
					result.error( new Exception( "文件信息不存在。" ) );
	...
	private void setResponseHeader( HttpServletResponse response, boolean streamContentType, OkrAttachmentFileInfo okrAttachmentFileInfo )
			throws Exception {
