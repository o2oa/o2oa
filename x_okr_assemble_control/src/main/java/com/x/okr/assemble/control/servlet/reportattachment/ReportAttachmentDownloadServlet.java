...
@WebServlet(urlPatterns = "/servlet/download/reportattachment/*")
public class ReportAttachmentDownloadServlet extends AbstractServletAction {

	private static final long serialVersionUID = -4314532091497625540L;
	private Logger logger = LoggerFactory.getLogger(ReportAttachmentDownloadServlet.class);

	@HttpMethodDescribe(value = "下载附件 servlet/download/reportattachment/{id}/stream", response = Object.class)
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (check) {
			try {
				part = this.getURIPart(request.getRequestURI(), "reportattachment");
