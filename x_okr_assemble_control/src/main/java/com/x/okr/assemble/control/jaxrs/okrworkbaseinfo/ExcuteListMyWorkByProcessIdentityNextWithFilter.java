...
public class ExcuteListMyWorkByProcessIdentityNextWithFilter extends ExcuteBase {

	private Logger logger = LoggerFactory.getLogger( ExcuteListMyWorkByProcessIdentityNextWithFilter.class );
	...
	protected ActionResult<List<WrapOutOkrWorkBaseSimpleInfo>> execute( HttpServletRequest request,EffectivePerson effectivePerson, String id, Integer count, com.x.okr.assemble.control.jaxrs.okrworkperson.WrapInFilter wrapIn ) throws Exception {
		...
		if( check ){
			try{
				...
				Exception exception = new WorkBaseInfoProcessException( e, "系统根据条件进行数据列表查询时发生异常!" );
				result.error( exception );
				logger.error( e, effectivePerson, request, null);
			}
		}else{
