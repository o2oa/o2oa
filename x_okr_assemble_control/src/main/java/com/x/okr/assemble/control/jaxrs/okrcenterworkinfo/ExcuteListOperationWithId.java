...
public class ExcuteListOperationWithId extends ExcuteBase {

	private Logger logger = LoggerFactory.getLogger( ExcuteListOperationWithId.class );
	...
	protected List<String> execute( HttpServletRequest request, EffectivePerson effectivePerson, OkrUserCache okrUserCache, String id ) {
		...
		Boolean isCompanyWorkAdmin = false;
		String status = null;
		...
		if( !"草稿".equals( processStatus ) && !"已归档".equals( status ) ){
			//如果用户是管理,或者是部署者
			if( okrUserCache.isOkrSystemAdmin() || isCompanyWorkAdmin || okrUserCache.getLoginIdentityName().equals( okrCenterWorkInfo.getDeployerIdentity() )){
		...
		if( "草稿".equals( processStatus )){
			...
			if( okrUserCache.isOkrSystemAdmin() || isCompanyWorkAdmin ){
				operation.add( "DELETE" );
			}
		}
		if( confirm ){
