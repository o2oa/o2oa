...
public class DepartmentFactory extends AbstractFactory {
	...
	public Long countSubNestedWithCompany(String id) throws Exception {
		return listSubNestedWithCompany(id).stream().count();
	}
	...
	public Long countSubDirectWithCompany(String id) throws Exception {
		return this.listSubDirectWithCompany(id).stream().count();
	}

	@MethodDescribe("获取指定公司的直接下属部门(Level=1).")
	public List<String> listSubDirectWithCompany(String id) throws Exception {
		EntityManager em = this.entityManagerContainer().get(Department.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<String> cq = cb.createQuery(String.class);
		Root<Department> root = cq.from(Department.class);
