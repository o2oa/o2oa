package com.x.organization.assemble.control.alpha.jaxrs.departmentduty;
...
import com.x.organization.assemble.control.alpha.wrapin.WrapInDepartmentDuty;
import com.x.organization.assemble.control.alpha.wrapout.WrapOutCompanyDuty;
import com.x.organization.assemble.control.alpha.wrapout.WrapOutDepartmentDuty;
...
public class DepartmentDutyAction extends StandardJaxrsAction {
	...
	@HttpMethodDescribe(value = "获取去重的部门职务名称.", response = WrapOutStringList.class)
	@GET
	@Path("distinct/name")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	...
	@HttpMethodDescribe(value = "模糊匹配部门职务的名称,匹配名称,拼音和首字母.", response = WrapOutStringList.class)
	@GET
