...
class MyAppActivity : BaseMVPActivity<MyAppContract.View,MyAppContract.Presenter>(), MyAppContract.View {

    override var mPresenter: MyAppContract.Presenter = MyAppPresenter()
    override fun layoutResId(): Int = R.layout.activity_my_app
    ...
    private val itemTouchHelper = ItemTouchHelper (object : ItemTouchHelper.Callback() {
        ...
        override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
            if (actionState != ItemTouchHelper.ACTION_STATE_IDLE) {
                ...
                // 侧滑或者拖动的时候背景设置为灰色
                viewHolder!!.itemView.setBackgroundColor(FancySkinManager.instance().getColor(this@MyAppActivity,R.color.z_color_meeting_text))
            }
        }
    ...
    private fun initAllApp(){
        ...
        allAppAdapter.setOnItemClickListener { _, position ->
            if (isEdit) {
                ...
                IndexFragment.go(appBeanList[position].appId!!,this, appBeanList[position].appTitle?:"")
            }
        }
    }
    ...
    private fun initMyApp(){
        ...
        myAppEditAdapter.setOnItemClickListener { _, position ->
            if (isEdit) {
                ...
                IndexFragment.go(myAppBeanList[position].appId!!,this, myAppBeanList[position].appTitle?:"")
            }
        }
    }
    ...
    override fun onBackPressed() {
        if (isEdit) {
            appBeanList.clear()
            appBeanList.addAll(oldAppBeanList)
            myAppBeanList.clear()
            myAppBeanList.addAll(oldMyAppBeanList)
            edit_ll_view.visibility = View.GONE
            my_app_rv.visibility = View.VISIBLE
            my_app_text_view.visibility = View.GONE
            allAppAdapter.notifyDataSetChanged()
    ...
    private val allAppAdapter: CommonRecycleViewAdapter<MyAppListObject> by lazy {
        object : CommonRecycleViewAdapter<MyAppListObject>(this, appBeanList, R.layout.item_all_app_list) {
...
//                    }
    ...
    private val myAppEditAdapter: CommonRecycleViewAdapter<MyAppListObject> by lazy {
        object : CommonRecycleViewAdapter<MyAppListObject>(this, myAppBeanList, R.layout.item_all_app_list) {
            override fun convert(holder: CommonRecyclerViewHolder?, t: MyAppListObject?) {
                val resId = ApplicationEnum.getApplicationByKey(t?.appId)?.iconResId
                if (resId!=null) {
                    holder?.setImageViewResource(R.id.app_id, resId)
    ...
    private val myAppAdapter: CommonRecycleViewAdapter<MyAppListObject> by lazy {
        object : CommonRecycleViewAdapter<MyAppListObject>(this, oldMyAppBeanList, R.layout.item_app_mini) {
            override fun convert(holder: CommonRecyclerViewHolder?, t: MyAppListObject?) {
                val resId = ApplicationEnum.getApplicationByKey(t?.appId)?.iconResId
                if (resId!=null) {
                    holder?.setImageViewResource(R.id.app_id, resId)
