...
MWF.xApplication.Deployment.UserComponent = new Class({
    ...
    checkAdministrator: function(){
        ...
        if (this.data.controllerList.indexOf(currentName)!=-1) return true;
        return false;
    },
    loadSystemFlag: function(){},
            ...
            var options = {
                ...
                "onComplete": function(items){
                    ...
                    items.each(function(item){
                        this.list.push(item.data.name);
                    }.bind(this));
                    this.listNode.empty();
                    this.list.each(function(personName){
                        if (personName) new MWF.widget.Person({"name": personName}, this.listNode, this.deployment.explorer, false, null, {"style": "application"});
                    }.bind(this));
                }.bind(this)
            };
            var selector = new MWF.OrgSelector(this.deployment.content, options);
        }.bind(this));
    },
