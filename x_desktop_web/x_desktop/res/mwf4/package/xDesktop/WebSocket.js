...
MWF.xDesktop.WebSocket = new Class({
    ...
    receiveMeetingInviteMessage: function(data){
        ...
        content = content.replace(/{person}/g, data.applicant);
        var date = Date.parse(data.startTime).format("%Y-%m-%d- %H:%M");
        content = content.replace(/{date}/g, date);
        content = content.replace(/{subject}/g, data.subject);
        content = content.replace(/{addr}/g, data.room+"("+data.building+")");
