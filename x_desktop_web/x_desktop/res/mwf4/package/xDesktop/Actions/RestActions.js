...
//		MWF.getJSON(url, callback);
    ...
    invokeFormDataWithProgress: function(xhr, method, uri, data, file, callback, async){
        ...
        messageItem = this.addFormDataMessage(file, false, xhr);
        xhr.send(data);
    },
    ...
    setMessageTitle: function(messageItem, text){
        messageItem.subjectNode.set("text", text);
    },
	...
	addFormDataMessage: function(file, noProgress, xhr){
        var contentHTML = "";
        if (noProgress){
		...
		var msg = {
            ...
            "content": file.name+"<br/>"+contentHTML
		};
