...
MWF.widget.Identity = new Class({
	...
	initialize: function(data, container, explorer, canRemove, removeAction, options){
		...
		this.path = MWF.defaultPath+"/widget/$Identity/";
		this.cssPath = MWF.defaultPath+"/widget/$Identity/"+this.options.style+"/css.wcss";
		this._loadCss();
            ...
            if (person.data){
                ...
                if (person.data.icon){
                    ...
                    if (person.genderType=="f"){
                        img = "<img width='50' height='50' border='0' src='"+MWF.defaultPath+"/xApplication/Organization/$PersonExplorer/default/icon/female.png'></img>";
                    }else{
                        img = "<img width='50' height='50' border='0' src='"+MWF.defaultPath+"/xApplication/Organization/$PersonExplorer/default/icon/man.png'></img>";
                    }
                }
...
MWF.widget.Department = new Class({
    Implements: [Options, Events],
    Extends: MWF.widget.Identity,
...
MWF.widget.Company = new Class({
    Extends: MWF.widget.Department
});
MWF.widget.Application = new Class({
    Extends: MWF.widget.Department
});
MWF.widget.Process = new Class({
    Extends: MWF.widget.Department
});
MWF.widget.FormField = new Class({
    Extends: MWF.widget.Department
});