...
//    },
//    getDepartmentByPerson : function( name, callback, fail, async ){
//        this.actions.listDepartmentByPerson( function( json ){
//            if( callback )callback(json.data);
//        }, null, async )
//    },
//    getCompanyByPerson : function( name, callback ){
//        this.actions.listCompanyByPerson( function( json ){
//            if( callback )callback(json.data);
//        }, null, name )
//    }
//})
...
MWF.xApplication.Attendance.Echarts = new Class({
    ...
    loadDepartmentPieChart : function() {
        //if( !this.options.date )this.options.date = new Date();
        //this.year = this.options.date.getFullYear()
        //var month = this.options.date.getMonth()+1
        //this.month = month.toString().length == 2 ? month : "0"+month;
        this.loadResource(function(){
            this._loadDepartmentPieChart();
        }.bind(this))
    },
    _loadDepartmentPieChart: function (  ) {
        var data = this.getDepartmentPieData();
        this.chart = echarts.init(this.node, 'shine');
        this.chart.setOption({
            title: {
                text: '考勤汇总'
    ...
    loadDepartmentBarChart : function(){
        this.loadResource(function(){
            this._loadDepartmentBarChart();
        }.bind(this))
    },
    _loadDepartmentBarChart : function(){

        var data = this.getDepartmentBarData();
        var option = {
            title: {
                text: '考勤趋势'
                //subtext: '纯属虚构'
