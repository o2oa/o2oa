...
MWF.xApplication.Attendance.CompanyDetail = new Class({
    ...
    initialize: function(node, app, actions, options){
        ...
        this.app = app;
        this.lp = app.lp;
        this.path = "/x_component_Attendance/$CompanyDetail/";
        this.cssPath = "/x_component_Attendance/$CompanyDetail/"+this.options.style+"/css.wcss";
        this._loadCss();
    ...
    loadTab : function(){
        ...
        MWF.require("MWF.widget.Tab", function(){
            ...
            this.detailPage = this.tabs.addTab(this.detailArea, this.lp.companyAttendanceDetail, false);
            this.detailPage.contentNodeArea.set("class","detailPage");
            this.detailPage.addEvent("show",function(){
                if( !this.detailExplorer ){
                    this.detailExplorer = new MWF.xApplication.Attendance.CompanyDetail.Explorer( this.detailArea, this );
                    this.detailExplorer.load();
                }
            }.bind(this))


            this.detailStaticPage = this.tabs.addTab(this.detailStaticArea, this.lp.companyAttendanceStatic, false);
            ...
            this.detailStaticPage.addEvent("show",function(){
                if( !this.detailStaticExplorer ){
                    this.detailStaticExplorer = new MWF.xApplication.Attendance.CompanyDetail.DetailStaticExplorer( this.detailStaticArea, this );
...
MWF.xApplication.Attendance.CompanyDetail.Explorer = new Class({
    ...
    createCompanyTd : function(tr){
        var _self = this;
        var td = new Element("td", {  "styles" : this.css.filterTableTitle, "text" : this.lp.company  }).inject(tr);
        var td = new Element("td", {  "styles" : this.css.filterTableValue }).inject(tr);
        this.q_companyName = new MDomItem( td, {
            "name" : "q_companyName",
            "event" : {
                "click" : function(el){ _self.selecePerson(); }
            }
        }, true, this.app );
        this.q_companyName.load();
    },
    ...
    createActionTd : function( tr ){
        ...
        input.addEvent("click", function(){
            if( this.q_companyName.getValue().trim() == "" ){
                this.app.notice( "请先选择公司", "error" );
                return;
            }
            var filterData = {
                q_companyName : this.q_companyName.getValue(),
    ...
    selecePerson: function(){
        var options = {
            ...
            "onComplete": function(items){
                ...
                this.q_companyName.setValue( names.join(",") )
            }.bind(this)
        };
    ...
    loadView : function( filterData ){
        ...
        this.view = new MWF.xApplication.Attendance.CompanyDetail.View(this.elementContentNode, this.app,this );
...
MWF.xApplication.Attendance.CompanyDetail.DetailStaticExplorer = new Class({
    Extends: MWF.xApplication.Attendance.CompanyDetail.Explorer,
    ...
    loadView : function( filterData ){
        this.elementContentNode.empty();
        if( this.view )delete this.view;
        this.view = new MWF.xApplication.Attendance.CompanyDetail.DetailStaticView(this.elementContentNode, this.app,this );
        this.view.filterData = filterData;
...
MWF.xApplication.Attendance.CompanyDetail.View = new Class({
    Extends: MWF.xApplication.Attendance.Explorer.View,
    _createItem: function(data){
        return new MWF.xApplication.Attendance.CompanyDetail.Document(this.table, data, this.explorer, this);
    },
    ...
    _getCurrentPageData: function(callback, count){
        ...
        //var filter = this.filterData || {};
        //this.actions.listCompanyDetailFilter( filter, function(json){
        //    if( callback )callback(json);
        //}.bind(this))
    },
...
MWF.xApplication.Attendance.CompanyDetail.DetailStaticView = new Class({
    Extends: MWF.xApplication.Attendance.Explorer.View,
    _createItem: function(data){
        return new MWF.xApplication.Attendance.CompanyDetail.DetailStaticDocument(this.table, data, this.explorer, this);
    },
    ...
    _getCurrentPageData: function(callback, count){
        ...
        if( filter.q_type == "day" ) {
            this.actions.listDepartmentDateStaticByCompany( filter.q_companyName, filter.q_date,  function(json){ //listStaticDayCompany
                //var data = json.data;
            ...
            this.actions.listDepartmentMonthStaticByCompany( filter.q_companyName, filter.cycleYear, filter.cycleMonth, function(json){ //listDepartmentMonthStaticByCompany
                //var data = json.data;
                //data.sort( function( a, b ){
...
MWF.xApplication.Attendance.CompanyDetail.Document = new Class({
    Extends: MWF.xApplication.Attendance.Explorer.Document

})


MWF.xApplication.Attendance.CompanyDetail.DetailStaticDocument = new Class({
    Extends: MWF.xApplication.Attendance.Explorer.Document
