...
MWF.xApplication.cms.Explorer.ViewExplorer = new Class({
	...
	initialize: function(node, actions, columnData, categoryData, options){
		...
		this.setTooltip();
		this.path = "/x_component_cms_Explorer/$ViewExplorer/";
		this.cssPath = "/x_component_cms_Explorer/$ViewExplorer/"+this.options.style+"/css.wcss";
		this._loadCss();
    ...
    load: function(){
        ...
        this.loadController(function(){
            this.getDefaultView( function(viewId){
                this.userDefaultViewId = viewId;
                this.loadViewListNode();
                this.setNodeScroll();
            }.bind(this))
        }.bind(this))
    ...
    loadViewList : function(){
        if( this.options.viewId && this.options.viewId != ""){
            this.openView = this.options.viewId;
        }else if( this.userDefaultViewId ){
            this.openView = this.userDefaultViewId;
        }
        this.actions.listCategoryViewByCatagory( this.categoryData.id, function (data) {
            for(var i=0;i<data.data.length;i++){
                if(data.data[i].viewId == "default" ){
                    this.createListViewItem({"isDefault":true});
                }
            }
            this.actions.listViewByCategory( this.categoryData.id, function (json) {
    ...
    createListViewItem : function(view){
        var _self = this;
        var viewNode = new Element("div", {
            "styles": this.css.viewNode,
            "text" : view.isDefault ? this.app.lp.defaultView : view.name
        }).inject(this.viewListNode);
        viewNode.store("viewData",view);

        var actionNode = new Element("div.setDefaultNode", {
            "styles": this.css.viewSetDefaultNode,
            "title" : this.app.lp.setDefaultView
        }).inject(viewNode );
        actionNode.store("viewId", view.isDefault ? "default" : view.id );
        viewNode.store("actionNode",actionNode);

        actionNode.addEvent("click", function(e){
            _self.setDefaultView(this.retrieve("viewId"));
            e.stopPropagation();
        });

        viewNode.addEvents({
            "mouseover": function(){ this.retrieve("actionNode").fade("in"); },
            "mouseout": function(){ this.retrieve("actionNode").fade("out"); },
            "click": function (el) {
               _self.setCurrentViewItem(this);
            }
        })
        if( this.openView == "default" && view.isDefault ){
            this.setCurrentViewItem( viewNode );
        }else if( this.openView == view.id ){
            this.setCurrentViewItem( viewNode );
        }else if( !this.openView && this.views.length == 0 ){
            this.setCurrentViewItem( viewNode );
        }
        this.views.push(viewNode);
    },

    setDefaultView : function(viewId){
        MWF.UD.putData("cms_defaultView_" + this.categoryData.id , { "id":viewId }, function(){
            this.app.notice(this.app.lp.setDefaultSuccess, "success");
        }.bind(this))
    },
    getDefaultView : function(callback){
        MWF.UD.getDataJson("cms_defaultView_" + this.categoryData.id, function( data ){
    ...
    setCurrentViewItem : function( viewItem ){
        if(this.currentViewNode)this.currentViewNode.setStyles( this.css.viewNode );
        viewItem.setStyles( this.css.viewNode_current );
        this.currentViewNode = viewItem;
        this.currentViewData = viewItem.retrieve("viewData");
        if( this.currentViewData.content && typeof(this.currentViewData.content)=="string"){
            this.currentViewData.content = JSON.parse(this.currentViewData.content);
        }
        this.loadView();
    },
    loadView : function( viewData ){
        if( this.view)this.view.clear();
        if( !viewData )viewData = this.currentViewData;
        if( viewData.isDefault ){
            this.view = new MWF.xApplication.cms.Explorer.ViewExplorer.DefaultView(this.elementContentNode, this.app,this, viewData );
        }else{
            this.view = new MWF.xApplication.cms.Explorer.ViewExplorer.View(this.elementContentNode, this.app,this, viewData );
        }
        this.view.load();
        this.setContentSize();
    },
...
MWF.xApplication.cms.Explorer.ViewExplorer.DefaultView = new Class({

    initialize: function( container, app,explorer, data ){
    ...
    createListHead : function(){
        ...
        MWF.getJSON(listItemUrl, function(json){
            ...
            json.each(function(cell){
                var isShow = true;
                if( cell.access ){
                    if( cell.access == "admin" && !this.explorer.isAdmin ){
                        isShow = false;
                    }
                }
                if(isShow) {
    ...
    _createItem: function(data){
        return new MWF.xApplication.cms.Explorer.ViewExplorer.DefaultDocument(this.table, data, this.explorer, this);
    }
})

MWF.xApplication.cms.Explorer.ViewExplorer.View = new Class({
    Extends: MWF.xApplication.cms.Explorer.ViewExplorer.DefaultView,
    ...
    _createItem: function(data){
        return new MWF.xApplication.cms.Explorer.ViewExplorer.Document(this.table, data, this.explorer, this);
    }

})


MWF.xApplication.cms.Explorer.ViewExplorer.Filter = new Class({
...
MWF.xApplication.cms.Explorer.ViewExplorer.DefaultDocument = new Class({
...
MWF.xApplication.cms.Explorer.ViewExplorer.Document = new Class({
    Extends: MWF.xApplication.cms.Explorer.ViewExplorer.DefaultDocument,
