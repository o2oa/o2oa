...
MWF.xApplication.cms.Explorer.Starter = new Class({
    ...
    initialize: function(columnData, categoryData, app, options){
        ...
        this.path = "/x_component_cms_Explorer/$Starter/";
        this.cssPath = "/x_component_cms_Explorer/$Starter/"+this.options.style+"/css.wcss";
        this._loadCss();

        MWF.xDesktop.requireApp("cms.Explorer", "$Starter."+MWF.language, null, false);
        this.lp = MWF.xApplication.cms.Explorer.Starter.lp;
        ...
        if (this.app.desktop.session.user.name){
            this.orgAction.listIdentityByPerson(function(json){
                ...
                json.data.each(function(id){
                    var departSel = new MWF.xApplication.cms.Explorer.Starter.DepartmentSel(id, this, this.departmentSelArea, this.identityArea);
    ...
    getDocumentAction: function(callback){
        if (!this.documentAction){
            MWF.xDesktop.requireApp("cms.Explorer", "Actions.RestActions", function(){
                this.documentAction = new MWF.xApplication.cms.Explorer.Actions.RestActions();
                if (callback) callback();
            }.bind(this));
        }else{
            if (callback) callback();
...
MWF.xApplication.cms.Explorer.Starter.DepartmentSel = new Class({
