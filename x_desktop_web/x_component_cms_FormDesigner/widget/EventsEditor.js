...
MWF.xApplication.cms.FormDesigner.widget.EventsEditor = new Class({
	...
	initialize: function(node, designer, options){
		...
		this.path = "/x_component_cms_FormDesigner/widget/$EventsEditor/";
		this.cssPath = "/x_component_cms_FormDesigner/widget/$EventsEditor/"+this.options.style+"/css.wcss";
		this._loadCss();
...
MWF.xApplication.cms.FormDesigner.widget.EventsEditor.Item = new Class({
	...
	checkCreate: function(){
		...
		if (this.editor.data[event]){
			...
			this.iconNode.title = MWF.LP.cms.repetitionsEvent;
			this.createInput.focus();
		}else{
			this.event = event;
		...
		if (this.editor.currentEditItem!=this){
			if (!this.codeEditor){
				this.codeEditor = new MWF.widget.ScriptArea(this.codeNode, {
					"style": "event",
					"helpStyle" : this.editor.options.helpStyle,
