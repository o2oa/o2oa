   ...
   //     html += "<tr><td class='formTitle'>"+this.app.lp.application.icon+"</td><td id='formApplicationIcon'></td></tr>";
    ...
    changeAdministrators: function(){
        ...
        var options = {
            "type": "person",
            "title": "设置应用管理者",
            "names": this.data.controllerList || [],
    ...
    setAvailableItems: function(){
        ...
        if (this.data.availableIdentityList){
            this.data.availableIdentityList.each(function(name){
                if (name) new MWF.widget.Identity({"name": name}, this.availableItemsContentNode, explorer, false, null, {"style": "application"});
            }.bind(this));
        }
    ...
    changeAvailableIdentitys: function(){
        ...
        var options = {
            "type": "identity",
            "title": "设置应用可用身份",
            "names": this.data.availableIdentityList || [],
            "onComplete": function(items){
                var availableIdentityList = [];
                items.each(function(item){
                    availableIdentityList.push(item.data.name);
                }.bind(this));
    ...
    changeAvailableDepartments: function(){
        ...
        var options = {
            "type": "department",
            "title": "设置应用可用部门",
            "names": this.data.availableDepartmentList || [],
            "onComplete": function(items){
                var availableDepartmentList = [];
                items.each(function(item){
                    availableDepartmentList.push(item.data.name);
                }.bind(this));
                this.data.availableDepartmentList = availableDepartmentList;
                this.app.restActions.saveApplication(this.data, function(json){
                    this.availableItemsContentNode.empty();
                    this.setAvailableItems();
                }.bind(this));
    ...
    changeAvailableCompanys: function(){
        ...
        var options = {
            "type": "company",
            "title": "设置应用可用公司",
            "names": this.data.availableCompanyList || [],
            "onComplete": function(items){
                var availableCompanyList = [];
                items.each(function(item){
                    availableCompanyList.push(item.data.name);
                }.bind(this));
                this.data.availableCompanyList = availableCompanyList;
                this.app.restActions.saveApplication(this.data, function(json){
                    this.availableItemsContentNode.empty();
                    this.setAvailableItems();
                }.bind(this));
