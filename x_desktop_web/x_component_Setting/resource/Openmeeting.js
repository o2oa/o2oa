...
MWF.xApplication.Setting.resource.Openmeeting = new Class({
    Extends: MWF.xApplication.Setting.servers.DataServer.Document,
    Implements: [Events],
    initialize: function(explorer){
        this.explorer = explorer;
        this.app = this.explorer.app;
        this.container = this.explorer.openmeetingContent;
        this.css = this.explorer.app.css;
        this.load();
    },
    load: function(){
        this.app.actions.getResOpenMeeting(function(json){
            this.json = json.data;
    ...
    createBaseInfo: function(){
        ...
        var html = "<table cellSpacing='8px' width='90%' align='center'>" +
            "<tr><td width='120px'>server</td><td><input value='"+(this.json.server || "")+"'/></td></tr>" +
            "<tr><td>port</td><td><input value='"+(this.json.port || "")+"'/></td></tr>" +
            "<tr><td>host</td><td><input value='"+(this.json.host || "")+"'/></td></tr>" +
            "<tr><td>user</td><td><input value='"+(this.json.user || "")+"'/></td></tr>" +
            "<tr><td>password</td><td><input value='"+(this.json.pass || "")+"'/></td></tr>" +
            "</table>";
    ...
    saveDocument: function(){
        var inputs = this.inforNode.getElements("input");
        this.json.server = inputs[0].get("value");
        this.json.port = inputs[1].get("value");
        this.json.host = inputs[2].get("value");
        this.json.user = inputs[3].get("value");
        this.json.pass = inputs[4].get("value");

        this.app.actions.updateResOpenMeeting(this.json, function(){
