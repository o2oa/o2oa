...
MWF.xDesktop.requireApp("cms.Xform", "widget.RestActions", null, false);
MWF.require("MWF.widget.Common", null, false);
MWF.require("MWF.widget.MWFRaphael", null, false);
MWF.xApplication.cms.Xform.widget.Monitor = new Class({
    ...
    initialize: function(container, worklog, processid, options){
        ...
        this.path = "/x_component_cms_Xform/widget/$Monitor/";
        this.cssPath = "/x_component_cms_Xform/widget/$Monitor/"+this.options.style+"/css.wcss";
        this._loadCss();

        this.container = $(container);
        this.worklog = worklog;
        this.cmsid = processid;
    ...
    playBegin: function(){
        ...
        this.playIcon = this.paper.image(src, this.cms.begin.center.x-16, this.cms.begin.center.y-16, 32, 32);
    ...
    playGetNextActivity: function(){
        ...
        var activity = (activityType.toLowerCase()=="begin") ? this.cms.begin : this.cms[activityType+"s"][log.fromActivity];
    ...
    playToActivityStop: function(activity, log, prevRoute){
        ...
        var route = this.cms.routes[log.route];
        if (prevRoute){
    ...
    cmsReturnStyle: function(){
        this.worklog.each(function(log){
            ...
            var route = this.cms.routes[log.route];
            if (route){
                route.line.attr(this.cms.css.route.line.normal);
                route.point.attr(this.cms.css.route.decision.normal);
                route.arrow.attr(this.cms.css.route.arrow.normal);
            }
    ...
    loadPaper: function(){
        MWFRaphael.load(function(){
            ...
            MWF.xDesktop.requireApp("cms.ProcessDesigner", "Process", function(){
                this.process = new MWF.CMSPD.Process(this.paper, this.processData, this, {"style":"flat", "isView": true,
                    "onPostLoad": function(){
                        this.loadWorkLog();
                    }.bind(this)
                });
    ...
    getProcess: function(callback){
        this.action = new MWF.xApplication.process.Xform.widget.RestActions("x_processplatform_assemble_surface_process");
        ...
        worklogs.each(function(log, idx){
            ...
            if (log.taskCompletedList.length+log.taskList.length<1){
                ...
                log.taskCompletedList.each(function(task){
                    var taskNode = new Element("div", {"styles": this.css.workLogTaskNode}).inject(workNode);
                    var html = "<div style='font-weight: bold'>"+task.person+": </div>";
                ...
                log.taskList.each(function(task){
                    var taskNode = new Element("div", {"styles": this.css.workLogTaskNode}).inject(workNode);
                    var html = "<div style='font-weight: bold; color: red'>"+task.person+" "+MWF.xApplication.process.Xform.LP.processing+" </div>";
                    taskNode.set("html", html);
                }.bind(this));
            }
