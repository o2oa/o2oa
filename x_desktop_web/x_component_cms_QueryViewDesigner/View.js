...
MWF.xApplication.cms.QueryViewDesigner.View = new Class({
    ...
    initialize: function(designer, data, options){
        ...
        this.path = "/x_component_cms_QueryViewDesigner/$View/";
        this.cssPath = "/x_component_cms_QueryViewDesigner/$View/"+this.options.style+"/css.wcss";
        ...
        if(this.designer.application){
            //this.data.applicationName = this.designer.application.name;
            this.data.appId = this.designer.application.id;
            this.data.appName = this.designer.application.appName;
            //this.data.application = this.designer.application.id;
        }
                                ...
                                groupAreaNode.addEvent("click", function(){
                                    ...
                                    if (subtrs[0]){
                                        if (subtrs[0].getStyle("display")=="none"){
                                            ...
                                            iconNode.setStyle("background", "url("+"/x_component_cms_QueryViewDesigner/$View/default/icon/down.png) center center no-repeat");
                                        }else{
                                            subtrs.each(function(subtr){ subtr.setStyle("display", "none"); });
                                            iconNode.setStyle("background", "url("+"/x_component_cms_QueryViewDesigner/$View/default/icon/right.png) center center no-repeat");
                                        }
                                    }
...
MWF.xApplication.cms.QueryViewDesigner.View.Column = new Class({
    ...
    _createIconAction: function(){
        if (!this.actionArea){
            ...
            this._createAction({
                ...
                "event": "mousedown",
                "action": "move",
                "title": MWF.CMSQVD.LP.action.move
            });
            this._createAction({
                "name": "add",
                "icon": "add.png",
                "event": "click",
                "action": "addColumn",
                "title": MWF.CMSQVD.LP.action.add
            });
            this._createAction({
                "name": "delete",
                "icon": "delete1.png",
                "event": "click",
                "action": "delete",
                "title": MWF.CMSQVD.LP.action["delete"]
            });
        }
    },
    ...
    "delete": function(e){
        var _self = this;
        if (!e) e = this.node;
        this.view.designer.confirm("warn", e, MWF.CMSQVD.LP.notice.deleteColumnTitle, MWF.CMSQVD.LP.notice.deleteColumn, 300, 120, function(){
            _self.destroy();
    ...
    addColumn: function(e, data){
        MWF.require("MWF.widget.UUID", function(){
            ...
            var column = new MWF.xApplication.cms.QueryViewDesigner.View.Column(json, this.view, this);
