MWF.APPPS = MWF.xApplication.portal.SourceDesigner;
MWF.APPPS.options = {
	"multitask": true,
	"executable": false
};
MWF.xDesktop.requireApp("portal.PortalManager", "Actions.RestActions", null, false);
MWF.xDesktop.requireApp("portal.SourceDesigner", "Module.Package", null, false);
MWF.xApplication.portal.SourceDesigner.Main = new Class({
	...
	options: {
        ...
        "templateId": "",
        "name": "portal.SourceDesigner",
        "icon": "icon.png",
        "title": MWF.APPPS.LP.title,
        "appTitle": MWF.APPPS.LP.title,
        "id": "",
        "actions": null,
        "category": null,
        "processData": null
	...
	onQueryLoad: function(){
		...
		if (!this.options.id){
			...
			this.options.title = this.options.title + "-"+MWF.APPPS.LP.newSource;
		}
		this.actions = new MWF.xApplication.portal.PortalManager.Actions.RestActions();
		
		this.lp = MWF.xApplication.portal.SourceDesigner.LP;
	},
    ...
    cutModule: function(){
        if (this.shortcut) {
            if (this.page) {
                if (this.page.currentSelectedModule) {
                    var module = this.page.currentSelectedModule;
                    if (module.moduleType != "source" && module.moduleName.indexOf("$") == -1) {
                        this.copyModule();
                        module.destroy();
        ...
        if (this.shortcut) {
            if (this.page) {
                if (MWF.clipboard.data) {
                    if (MWF.clipboard.data.type == "source") {
                        ...
                        if (this.page.currentSelectedModule) {
                            ...
                            if (toModule.moduleType != "container" && toModule.moduleType != "source") {
                                where = "after";
	...
	switchToolbarMode: function(){
		if (this.toolbarMode=="all"){
			...
			this.toolbarTitleNode.set("text", MWF.APPPS.LP.tools);
    ...
    changeDesignerModeToMobile: function(){
        ...
        if (!this.mobilePage){
            this.mobilePage = new MWF.PSCPage(this, this.designMobileNode, {"mode": "Mobile"});
...
//				var designer = this;
	...
	loadPage: function(){
		this.getPageData(function(){
			this.pcPage = new MWF.PSCPage(this, this.designNode);
    ...
    loadNewPageDataFromTemplate: function(callback){
        this.actions.getSourceTemplate(this.options.templateId, function(page){
            if (page){
	...
	loadPageData: function(callback){
		this.actions.getSource(this.options.id, function(page){
			if (page){
	...
	savePage: function(){
        if (!this.isSave){
            ...
            this.actions.saveSource(pcData, mobileData, fieldList, function(responseJSON){
                this.notice(MWF.APPPS.LP.notice["save_success"], "ok", null, {x: "left", y:"bottom"});
    ...
    saveTemplate: function(markNode, areaNode, iconNode, nameNode, categorySelect, newCategoryNode, descriptionNode){
        ...
        if (!name){
            this.notice(MWF.APPPS.LP.notice["saveTemplate_inputName"], "error", nameNode, {x: "left", y:"top"});
            return false;
        }
        if (categorySelect.options[categorySelect.selectedIndex].value=="$newCategory" && !newCategoryNode.get("value")){
            this.notice(MWF.APPPS.LP.notice["saveTemplate_inputCategory"], "error", categorySelect, {x: "left", y:"top"});
            return false;
        }
        ...
        this.actions.addPageTemplate(pcData, mobileData, data, function(){
            this.notice(MWF.APPPS.LP.notice["saveTemplate_success"], "ok", null, {x: "left", y:"bottom"});
            markNode.destroy();
            areaNode.destroy();
        }.bind(this), function(xhr, text, error){
            var errorText = error+":"+text;
