...
//				var y = e.event.y;
		...
		okButton.addEvent("click", function(){
			if (inputNode.get("value")){
				...
				if (item.type=="folder"){
					...
					this.restActions.updateAttachment(this.controller.selectedAttachments[0].data, function(json){
						if (this.currentFolder){
							this.currentFolder.clickNode();
						}else{
							this.topTreeNode.clickNode();
	...
	shareFile: function(){
		...
		var inputNode = new Element("input", {"type": "text", "readonly": true, "styles": this.css.shareFileInputNode}).inject(inputAreaNode);
		...
		okButton.addEvent("click", function(){
            ...
            this.controller.selectedAttachments.each(function(att){
                if (att.type!="folder"){
                    ...
                    this.restActions.updateAttachment(att.data, function(json){
                        current++;
                        callback.apply(this);
                    }.bind(this));
                }else{
    ...
    sendFile: function(){
        ...
        okButton.addEvent("click", function(){
            ...
            var callback = function(){
                if (current == count){
                    if (inputNode.get("value")) this.notice(this.lp.fileSendSuccess+inputNode.get("value"), "success", this.content);
                    shareNode.destroy();
                    this.content.unmask();
                }
            };
				...
				if (files.length){
					...
					for (var i = 0; i < files.length; i++) {
						...
						this.restActions.addAttachment(function(){
                            current++;
                            callback.apply(this);
						}.bind(this), null, formData, folderId, file);
					}
				}
            ...
            this.fileReplaceNode.addEvent("change", function(){
                ...
                if (files.length){
                    ...
                    for (var i = 0; i < files.length; i++) {
                        ...
                        this.restActions.updateAttachmentData(function(){
                            current++;
                            callback.apply(this);
                        }.bind(this), null, formData, fileId, file);
                    }
                }
		...
		okButton.addEvent("click", function(){
			if (inputNode.get("value")){
				...
				this.restActions.saveFolder(data, function(json){
                    ...
                    this.restActions.getFolder(function(folderJson){
                        if (this.currentFolder){
