...
MWF.xScript.PageEnvironment = function(ev){
    ...
    this.org = {
        ...
        listPersonWithIdentity: function(name){
            getOrgActions();
            var data = {"dentityList": getNameFlag(name)};
            var v = null;
    ...
    this.page = this.form = {
        ...
        "toPage": function(name, par){
            _form.app.toPage(name, par);
        },
        "toPortal": function(portal, page, par){
            _form.app.toPortal(portal, page, par);
        },
        "openWork": function(id, completedId, options){
            var op = options || {};
            op.workId = id;
            op.workCompletedId = completedId;
            layout.desktop.openApplication(this.event, "process.Work", options);
        },
        "openDocument": function(id, options){
            var op = options || {};
            op.documentId = id;
            layout.desktop.openApplication(this.event, "cms.Document", options);
        },
        ...
        "startProcess": function(app, process){
            debugger;
