...
MWF.widget.Combox = new Class({
	...
	initialize: function(options){
		...
		this.path = MWF.defaultPath+"/widget/$Combox/";
		this.cssPath = MWF.defaultPath+"/widget/$Combox/"+this.options.style+"/css.wcss";
		this._loadCss();
		this.values = [];
		//this.lastValue();
		this.load();
    ...
    clear: function(){
	    if (this.input) MWF.release(this.input);
	    this.input = null;
	    this.node.empty();
        this.values = [];
    },
    ...
    commitInput: function(data){
        ...
        if (valueStr){
            ...
            //     this.values.push(new MWF.widget.Combox.Value(this, value, data));
            //
            // }.bind(this));
            //
    ...
    createItem: function(values, i, data, callback){
        if (values[i]){
            ...
            this.values.push(new MWF.widget.Combox.Value(this, itemText, itemData, {
                "onLoad": function(){
                    i++;
                    if (values[i]){
...
MWF.widget.Combox.Value = new Class({
    Implements: [Options, Events],
	...
	load: function(){
        this.checkData(function(){
            ...
            if (!this.data) this.node.setStyle("color", "#bd0000");
    ...
    edit: function(where){
        ...
        if (!this.input){
            this.input = new MWF.widget.Combox.Input(this.combox, this, this.value);
    ...
    commitInput: function(data){
        ...
        if (valueStr){
            ...
            if (values.length>1){
                ...
                if (!this.data){
                    this.node.setStyle("color", "#bd0000");
                }else{
...
MWF.widget.Combox.Input = new Class({
