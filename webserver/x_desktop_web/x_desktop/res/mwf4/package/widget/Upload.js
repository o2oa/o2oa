...
MWF.widget.Upload = new Class({
	...
	initialize: function(container, options){
		...
		this.path = MWF.defaultPath+"/widget/$Upload/";
		this.cssPath = MWF.defaultPath+"/widget/$Upload/"+this.options.style+"/css.wcss";
		this._loadCss();
    ...
    inputFile_CreateInputNode: function(url){
        ...
        var formNode = new Element("form", {
            "method": "POST",
            //"action": url+"/callback/window.frameElement.ownerDocument.defaultView.MWF.O2UploadCallbackFun",
    ...
    inputFile_UploadCallback: function(str){
        ...
        if (json.type==="success"){
            ...
            this.setMessageTitle(this.messageItem, MWF.LP.desktop.action.uploadComplete);
            this.setMessageText(this.messageItem, MWF.LP.desktop.action.uploadComplete);
            this.fireEvent("every", [json]);
            this.fireEvent("completed", [json]);
        }else{
            this.setMessageTitle(this.messageItem, MWF.LP.desktop.action.sendError);
            this.setMessageText(this.messageItem, MWF.LP.desktop.action.sendError+": "+json.message);
    ...
    doUpload_InputFile: function(){
        this.action.getActions(function(){
            ...
            //MWF.O2UploadCallbackFun = this.inputFile_UploadCallback.bind(this);
