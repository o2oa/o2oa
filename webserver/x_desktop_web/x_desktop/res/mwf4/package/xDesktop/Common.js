...
MWF.org = {
    parseOrgData: function(data){
        if (data.distinguishedName){
            var flag = data.distinguishedName.substr(data.distinguishedName.length-1, 1);
            switch (flag.toLowerCase()){
                case "i":
                    return this.parseIdentityData(data);
                    break;
                case "p":
                    return this.parsePersonData(data);
                    break;
                case "u":
                    return this.parseUnitData(data);
                    break;
                case "g":
                    return this.parseGroupData(data);
                    break;
                case "r":
                    return this.parseRoleData(data);
                    break;
                case "a":
                    return this.parseAttributeData(data);
                    break;
                default:
                    return data;
            }
    ...
    parseIdentityData: function(data){
        var rData = {
            "id": data.id,
            "name": data.name,
            "unique": data.unique,
