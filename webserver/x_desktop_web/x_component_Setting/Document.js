...
MWF.xApplication.Setting.Document.List.ItemEditor = new Class({
    ...
    load: function(){
        ...
        MWF.require("MWF.xDesktop.Dialog", function(){
            ...
            Object.each(this.data, function(v, k){
                if ((this.list.data.readonly && this.list.data.readonly.indexOf(k)==-1) || !this.list.data.readonly){
                    new Element("div", {"styles": this.css.explorerContentListEditTitleNode, "text": this.lp.list[k]}).inject(this.content);
                    if (typeOf(v)=="string"){
    ...
    save: function(dlg){
        ...
        Object.each(this.inputs, function(input, k){
            if ((this.list.data.readonly && this.list.data.readonly.indexOf(k)==-1) || !this.list.data.readonly){
                var value = (typeOf(input)=="element") ? input.get("value") : input.getValue();
                if (!value){
                    flag = false;
                    this.app.notice(this.lp.pleaseInput+this.lp.list[keys[i]], "error");
                    return false;
                }else{
                    values[k] = value;
                }
