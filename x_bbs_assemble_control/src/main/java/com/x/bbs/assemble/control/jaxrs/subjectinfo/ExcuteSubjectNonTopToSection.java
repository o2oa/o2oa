...
public class ExcuteSubjectNonTopToSection extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectNonTopToSection.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToSection( id, false, effectivePerson.getName() );
