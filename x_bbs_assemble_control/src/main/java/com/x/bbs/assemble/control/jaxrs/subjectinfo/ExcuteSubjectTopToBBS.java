...
public class ExcuteSubjectTopToBBS extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectTopToBBS.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToBBS( id, true, effectivePerson.getName() );
