...
public class ExcuteSubjectNonTopToBBS extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectNonTopToBBS.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToBBS( id, false, effectivePerson.getName() );
