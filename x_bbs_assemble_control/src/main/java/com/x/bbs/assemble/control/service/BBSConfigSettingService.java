...
public class BBSConfigSettingService{
	...
	private Logger logger = LoggerFactory.getLogger( BBSConfigSettingService.class );
	...
	public BBSConfigSetting save( BBSConfigSetting bbsConfigSetting ) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if( _bbsConfigSetting != null ){
				...
				bbsConfigSetting.copyTo( _bbsConfigSetting );
	...
	public void initAllSystemConfig() {
		...
		selectContent = null;
		isMultiple = false;
		description = "论坛系统名称:可以为系统指定名称,比如以公司名称作为论坛名称等等。";
		try {
