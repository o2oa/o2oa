...
public class ExcuteSubjectNonTopToForum extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectNonTopToForum.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToForum( id, false, effectivePerson.getName() );
