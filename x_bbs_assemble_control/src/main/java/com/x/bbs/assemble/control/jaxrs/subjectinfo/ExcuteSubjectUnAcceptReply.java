...
public class ExcuteSubjectUnAcceptReply extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectUnAcceptReply.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.acceptReply( id, "", effectivePerson.getName() );
