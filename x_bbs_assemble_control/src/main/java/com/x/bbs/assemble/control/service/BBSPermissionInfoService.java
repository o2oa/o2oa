...
public class BBSPermissionInfoService {
	...
	private Logger logger = LoggerFactory.getLogger( BBSPermissionInfoService.class );
	...
	public List<BBSPermissionInfo> listPermissionBySection( String sectionId ) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			business = new Business(emc);
			ids = business.permissionInfoFactory().listPermissionBySectionId( sectionId );
			if( ids != null && !ids.isEmpty() ){
				return business.permissionInfoFactory().list(ids);
			}else{
				return null;
	...
	public List<BBSPermissionInfo> listAllPermissionForUser( String userName ) throws Exception {
		...
		List<WrapGroup> groups = null;
		List<WrapDepartment> departments = null;
		List<WrapDepartment> user_departments = new ArrayList<WrapDepartment>();
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			//查询用户所属的组织以及上级组织等等
			departments = business.organization().department().listWithPerson( userName );
			if( departments != null && !departments.isEmpty() ){
				for( WrapDepartment department : departments ){
					user_departments = listSuperDepartment( department, user_departments );
				}
			}
			if( user_departments != null && !user_departments.isEmpty() ){
				for(  WrapDepartment department : user_departments ){
					if( uniqueIds.contains( department.getName() ) ){
						uniqueIds.add( department.getName() );
					}
					//查询各组织所属的公司是否已经存在于列表里，如果不存在，则进行添加
					company = business.organization().company().getWithDepartment( department.getName() );
					if( company != null && !uniqueIds.contains( company.getName() ) ){
						uniqueIds.add( company.getName() );
					}
				}
			}			
			//查询用户所属的群组
			groups = business.organization().group().listWithPersonSupNested( userName );
			if( groups != null && !groups.isEmpty() ){
				for( WrapGroup group : groups){
					if( !uniqueIds.contains( group.getName() ) ){
						uniqueIds.add( group.getName() );
					}
				}
			}			
	...
	/**
	 * 递归将部门对象以及上级部门对象放入部门列表里
	 * @param department
	 * @param departmentList
	 * @return
	 * @throws Exception
	 */
	public List<WrapDepartment> listSuperDepartment( WrapDepartment department,  List<WrapDepartment> departmentList ) throws Exception {
		if( departmentList == null ){
			departmentList = new ArrayList<WrapDepartment>();
		}
		if( department != null ){
			//将部门信息放入部门信息列表里
			...
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				business = new Business(emc);
				departments = business.organization().department().listSupNested( department.getName() );
				if( departments != null && !departments.isEmpty() ){
					for( WrapDepartment department_super : departments ){
						departmentList = listSuperDepartment( department_super, departmentList );
					}
				}
			}catch( Exception e ){
