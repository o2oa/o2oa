...
public class ExcuteSubjectUnLock extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectUnLock.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.lock( id, false, effectivePerson.getName() );
