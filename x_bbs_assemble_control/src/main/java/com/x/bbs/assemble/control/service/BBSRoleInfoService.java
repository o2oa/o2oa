...
public class BBSRoleInfoService {
	...
	public void createForumRole( String forumId ) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void createForumRole( BBSForumInfo _bBSForumInfo ) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			checkAndSaveBBSRoleInfo( emc, _bBSForumInfo, "FORUM_GUEST_" + forumId, forumName + "-论坛访客", "系统角色，论坛["+ forumName +"]对指定范围用户可见。", permissionCodeList );
...
//			permissionCodeList.add( "FORUM_REPLY_MANAGEMENT_" + forumId );
	...
	public void createSectionRole( String sectionId ) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void createSectionRole( BBSSectionInfo _sectionInfo ) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			checkAndSaveBBSRoleInfo( emc, _sectionInfo, "SECTION_GUEST_" + sectionId, forumName + "-" + sectionName + "-版块访问", "系统角色，版块[" + forumName + "-"+ sectionName +"]对指定范围内的用户可见，并且用户可发布可回复。", permissionCodeList );
			...
			checkAndSaveBBSRoleInfo( emc, _sectionInfo, "SECTION_MANAGER_" + sectionId, forumName + "-" + sectionName + "-版主", "系统角色，用户可以在版块[" + forumName + "-"+ sectionName +"]中拥有版块及版块内容管理的最大权限。", permissionCodeList );
			...
			checkAndSaveBBSRoleInfo( emc, _sectionInfo, "SECTION_SUBJECT_PUBLISHER_" + sectionId, forumName + "-" + sectionName + "-主题发布者", "系统角色，用户可以在版块[" + forumName + "-"+ sectionName +"]发布主题。", permissionCodeList );
			...
			checkAndSaveBBSRoleInfo( emc, _sectionInfo, "SECTION_REPLY_PUBLISHER_" + sectionId, forumName + "-" + sectionName + "-回复发布者", "系统角色，用户可以在版块[" + forumName + "-"+ sectionName +"]发表回复。", permissionCodeList );
			...
			checkAndSaveBBSRoleInfo( emc, _sectionInfo, "SECTION_RECOMMENDER_" + sectionId, forumName + "-" + sectionName + "-主题推荐者", "系统角色，用户可以在版块[" + forumName + "-"+ sectionName +"]对主题进行推荐到主页操作。", permissionCodeList );
			...
			checkAndSaveBBSRoleInfo( emc, _sectionInfo, "SECTION_SUBJECT_AUDITOR_" + sectionId, forumName + "-" + sectionName + "-主题审核者", "系统角色，用户可以在版块[" + forumName + "-"+ sectionName +"]对发布的主题进行审核。", permissionCodeList );
	 ...
	 * @throws Exception
	 */
	private void checkAndSaveBBSRoleInfo( EntityManagerContainer emc, BBSForumInfo _bBSForumInfo, String roleCode, String roleName, String description, List<String> permissionCodeList ) throws Exception{
        ...
        if( roleInfo == null ){
        	roleInfo = new BBSRoleInfo( _bBSForumInfo.getId(), _bBSForumInfo.getForumName(), null, null, null, null, 
	 ...
	 * @throws Exception
	 */
	private void checkAndSaveBBSRoleInfo( EntityManagerContainer emc, BBSSectionInfo _sectionInfo, String roleCode, String roleName, String description, List<String> permissionCodeList ) throws Exception{
		Business business = new Business( emc );
		BBSPermissionInfo permissionInfo  = null;
		BBSPermissionRole permissionRole = null;
		BBSRoleInfoFactory roleInfoFactory = business.roleInfoFactory();
		...
		BBSRoleInfo roleInfo = roleInfoFactory.getRoleByCode( roleCode );
        if( roleInfo == null ){
        	roleInfo = new BBSRoleInfo( _sectionInfo.getForumId(), _sectionInfo.getForumName(), _sectionInfo.getId(), _sectionInfo.getSectionName(), _sectionInfo.getMainSectionId(), _sectionInfo.getMainSectionName(), 
	...
	public BBSRoleInfo save( BBSRoleInfo _roleInfo, List<String> permissionCodes ) throws Exception {
		...
		try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create() ) {
			...
			if( _roleInfo_tmp == null ){
				...
				//更新一条记录
				_roleInfo.copyTo( _roleInfo_tmp );
	...
	public List<BBSRoleInfo> listAllRoleForUser( String userName ) throws Exception {
		...
		//1、查询用户所在的部门信息，需要递归查询上级组织信息
		Business business = null;
		List<String> objectUniqueIds = new ArrayList<String>();
		List<WrapDepartment> departmentList = new ArrayList<WrapDepartment>();
		List<WrapCompany> companyList = new ArrayList<WrapCompany>();
		List<String> ids = null;
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			business = new Business(emc);
			identities = business.organization().identity().listWithPerson( userName );
			if ( identities != null && identities.size() > 0 ) {
				for( WrapIdentity identity : identities ){
					departmentList = composeUserDepartments( identity.getDepartment(), departmentList );
					companyList = composeUserCompanys( getCompanyNameByIdentity(identity.getName()), companyList );
				}
			}
			groupList = business.organization().group().listWithPersonSupNested( userName );
			//然后把部门名称,公司名称,群组名称放到同一个LIST里供查询使用
			if( companyList != null && !companyList.isEmpty() ){
				for( WrapCompany company : companyList ){
					objectUniqueIds.add( company.getName() );
				}
			}
			if( departmentList != null && !departmentList.isEmpty() ){
				for( WrapDepartment department : departmentList ){
					objectUniqueIds.add( department.getName() );
				}
			}
			if( groupList != null && !groupList.isEmpty() ){
				for( WrapGroup group : groupList ){
					objectUniqueIds.add( group.getName() );
				}
			}
	...
	/**
	 * 递归根据部门将部门信息放入List，如果上有级部门，则上级部门也需要放入同一个List
	 * @param departmentName
	 * @param departmentList
	 * @throws Exception 
	 */
	private List<WrapDepartment> composeUserDepartments( String departmentName, List<WrapDepartment> departmentList ) throws Exception {
		Business business = null;
		WrapDepartment department = null;
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			business = new Business(emc);
			department = business.organization().department().getWithName( departmentName );
			if( department.getSuperior() != null && !department.getSuperior().isEmpty() && !department.getSuperior().equals( departmentName ) ){
				composeUserDepartments( department.getSuperior(), departmentList );
			}
	...
	/**
	 * 递归根据公司名称将公司信息放入List，如果上有级公司，则上级公司也需要放入同一个List
	 * @param companyName
	 * @param companyList
	 * @throws Exception 
	 */
	private List<WrapCompany> composeUserCompanys( String companyName, List<WrapCompany> companyList ) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			business = new Business(emc);
			company = business.organization().company().getWithName( companyName );
