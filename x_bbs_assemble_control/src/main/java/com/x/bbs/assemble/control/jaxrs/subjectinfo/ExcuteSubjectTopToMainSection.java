...
public class ExcuteSubjectTopToMainSection extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectTopToMainSection.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToMainSection( id, true, effectivePerson.getName() );
