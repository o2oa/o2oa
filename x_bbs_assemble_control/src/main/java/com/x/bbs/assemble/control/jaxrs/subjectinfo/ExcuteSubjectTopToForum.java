...
public class ExcuteSubjectTopToForum extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectTopToForum.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToForum( id, true, effectivePerson.getName() );
