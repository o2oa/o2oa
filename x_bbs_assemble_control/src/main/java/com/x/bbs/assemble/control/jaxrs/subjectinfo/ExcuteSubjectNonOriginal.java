...
public class ExcuteSubjectNonOriginal extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectNonOriginal.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setOriginal( id, false, effectivePerson.getName() );
