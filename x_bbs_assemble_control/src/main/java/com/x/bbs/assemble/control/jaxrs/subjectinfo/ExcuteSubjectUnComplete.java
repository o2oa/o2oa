...
public class ExcuteSubjectUnComplete extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectUnComplete.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.complete( id, true, effectivePerson.getName() );
