...
public class ExcuteSubjectTopToSection extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectTopToSection.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToSection( id, true, effectivePerson.getName() );
