...
public class ExcuteSubjectNonRecommendToBBSIndex extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectNonRecommendToBBSIndex.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.recommendToBBSIndex( id, false, effectivePerson.getName() );
