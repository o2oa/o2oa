...
public class ExcuteSubjectSetOriginal extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectSetOriginal.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setOriginal( id, true, effectivePerson.getName() );
