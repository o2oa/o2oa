...
public class BBSSubjectInfoService {
	...
	private Logger logger = LoggerFactory.getLogger( BBSSubjectInfoService.class );
	...
	public void delete( EntityManagerContainer emc, String subjectId ) throws Exception {
		...
		if( voteOptionList != null && voteOptionList.size() > 0 ){
			for( BBSVoteOption voteOption : voteOptionList ){
				voteOptionBinary = emc.find( voteOptionBinary.getId(), BBSVoteOptionBinary.class );
				if( voteOptionBinary != null ){
					emc.remove( voteOptionBinary, CheckRemoveType.all );
				}
