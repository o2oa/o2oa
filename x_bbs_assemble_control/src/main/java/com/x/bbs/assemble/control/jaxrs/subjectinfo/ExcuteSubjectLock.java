...
public class ExcuteSubjectLock extends ExcuteBase {
	
	private Logger logger = LoggerFactory.getLogger( ExcuteSubjectLock.class );
	...
	protected ActionResult<WrapOutId> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.lock( id, true, effectivePerson.getName() );
