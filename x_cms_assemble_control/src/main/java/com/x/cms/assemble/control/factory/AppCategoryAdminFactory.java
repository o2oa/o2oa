 ...
 * @author liyi
 */
public class AppCategoryAdminFactory extends AbstractFactory {

	public AppCategoryAdminFactory( Business business ) throws Exception {
		super(business);
	}
	...
	public List<String> listAppInfoIdsByAdminName( String personName ) throws Exception {
		...
		CriteriaQuery<String> cq = cb.createQuery( String.class );
		Root<AppCategoryAdmin> root = cq.from( AppCategoryAdmin.class );
		Predicate p = cb.equal( root.get( AppCategoryAdmin_.adminName ), personName );
		p = cb.and( p, cb.equal( root.get( AppCategoryAdmin_.objectType ), "APPINFO" ) );
