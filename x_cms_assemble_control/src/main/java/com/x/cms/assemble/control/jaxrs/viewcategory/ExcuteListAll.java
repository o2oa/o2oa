...
public class ExcuteListAll extends ExcuteBase {
	...
	protected ActionResult<List<WrapOutViewCategory>> execute( HttpServletRequest request, EffectivePerson effectivePerson ) throws Exception {
		...
		if ((null != element) && ( null != element.getObjectValue()) ) {
			...
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {			
				...
				if (!business.viewEditAvailable( request, effectivePerson )) {
					throw new Exception("person{name:" + effectivePerson.getName() + "} 用户没有查询全部视图分类关联信息配置的权限！");
				}			
				//如果有权限，继续操作
				...
				List<ViewCategory> viewCategoryList = viewCategoryFactory.list( ids );//查询ID IN ids 的所有视图分类关联信息信息列表
				wraps = WrapTools.viewCategory_wrapout_copier.copy( viewCategoryList );//将所有查询出来的有状态的对象转换为可以输出的过滤过属性的对象
