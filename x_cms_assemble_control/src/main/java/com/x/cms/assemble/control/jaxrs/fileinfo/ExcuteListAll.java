...
public class ExcuteListAll extends ExcuteBase {
	...
	protected ActionResult<List<WrapOutFileInfo>> execute( HttpServletRequest request, EffectivePerson effectivePerson ) throws Exception {
		...
		if ((null != element) && ( null != element.getObjectValue()) ) {
			...
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {			
				...
				if (!business.fileInfoEditAvailable( request, effectivePerson )) {
					throw new Exception("person{name:" + effectivePerson.getName() + "} 用户没有查询全部文件或者附件的权限！");
				}			
				//如果有权限，继续操作
