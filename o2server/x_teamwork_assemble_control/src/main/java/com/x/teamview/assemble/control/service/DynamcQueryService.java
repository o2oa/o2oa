 ...
 * @author O2LEE
 */
public class DynamcQueryService {
	...
	public Long countWithFilter( EffectivePerson currentPerson, List<String> projectIds, List<String> taskIds ) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			return dynamicService.countWithFilter(emc, projectIds, taskIds);
		} catch (Exception e) {
			throw e;
		}
	}
	 ...
	 * @throws Exception
	 */
	public List<Dynamic> listWithFilterNext( EffectivePerson currentPerson, Integer pageSize, String lastId, List<String> projectIds, List<String> taskIds ) throws Exception {
		List<Dynamic> dynamicList = null;
		Integer maxCount = 20;
		String orderField = "createTime";
		String orderType = "desc";
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			dynamicList = dynamicService.listWithFilterNext(emc, maxCount, dynamic.getId(), orderField, orderType, projectIds, taskIds );
		} catch (Exception e) {
			throw e;
		}
		return dynamicList;
