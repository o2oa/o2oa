...
class TaskGroupService {
	 ...
	 * @throws Exception 
	 */
	protected TaskGroup refreshTaskGroupCount(EntityManagerContainer emc, String taskGroupId ) throws Exception {
		Business business = new Business( emc );
		TaskGroup taskGroup = emc.find( taskGroupId, TaskGroup.class );
		if( taskGroup == null ) {
			throw new Exception("taskGroup not exists. ID=" + taskGroupId );
		}
		List<String> taskIds = business.taskGroupReleFactory().listTaskIdByGroup( taskGroupId );
