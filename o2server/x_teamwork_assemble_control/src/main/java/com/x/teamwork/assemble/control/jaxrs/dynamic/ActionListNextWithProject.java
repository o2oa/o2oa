...
public class ActionListNextWithProject extends BaseAction {
	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request, EffectivePerson effectivePerson, String flag, Integer count, String projectId ) throws Exception {
		...
		if( check ) {
			cacheKey = ApplicationCache.concreteCacheKey( "ActionListNextWithProject", projectId  );
			...
			if ((null != element) && (null != element.getObjectValue())) {
				...
				try {
					...
					if( total > 0 ) {
						System.out.println(">>>>>search dynamics ..." );
