...
public class ActionListNextWithFilter extends BaseAction {
	...
	protected ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, Integer page, Integer count, JsonElement jsonElement ) throws Exception {
		...
		Long total = dynamicQueryService.countWithCondition(effectivePerson.getDistinguishedName(), queryFilter );

		if( total > 0 ) {
			List<Dynamic> dynamicList = dynamicQueryService.listPagingWithCondition(effectivePerson.getDistinguishedName(),
	...
	public static class Wo extends Dynamic {
		...
		private static final long serialVersionUID = -8312431550627566773L;
