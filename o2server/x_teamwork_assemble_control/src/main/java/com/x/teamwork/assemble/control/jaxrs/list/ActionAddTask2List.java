...
public class ActionAddTask2List extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(ActionAddTask2List.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id, String taskId ) throws Exception {
		...
		if ( StringUtils.isEmpty( taskId ) ) {
			check = false;
			Exception exception = new TaskIdForAddEmptyException();;
			result.error( exception );
		}
		...
		if (check) {
			try {
				taskListPersistService.addTaskToList(taskId, id, effectivePerson);
				// 更新缓存
