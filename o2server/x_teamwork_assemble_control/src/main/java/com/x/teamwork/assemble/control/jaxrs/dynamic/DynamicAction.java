...
public class DynamicAction extends StandardJaxrsAction {
	...
	public void listNextWithFilter(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request,
			@JaxrsParameterDescribe("当前页数") @PathParam( "page" ) Integer page,
		...
		try {
			result = new ActionListNextWithFilter().execute(effectivePerson, page, count, jsonElement );
		} catch (Exception e) {
	...
	@JaxrsMethodDescribe(value = "根据项目分页列示工作动态信息.", action = ActionListNextWithProject.class)
	@PUT
	@Path("list/{page}/next/{count}/project/{projectId}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void listNextWithProject(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request,
		...
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionListNextWithProject().execute(effectivePerson, page, count, projectId, jsonElement );
		} catch (Exception e) {
			logger.error(e, effectivePerson, request, null);
			result.error(e);
		}
	...
	@JaxrsMethodDescribe(value = "根据工作任务分页列示工作动态信息.", action = ActionListNextWithTask.class)
	@PUT
	@Path("list/{page}/next/{count}/task/{taskId}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void listNextWithTask(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request,
		...
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionListNextWithTask().execute(effectivePerson, page, count, taskId, jsonElement );
		} catch (Exception e) {
			logger.error(e, effectivePerson, request, null);
			result.error(e);
		}
