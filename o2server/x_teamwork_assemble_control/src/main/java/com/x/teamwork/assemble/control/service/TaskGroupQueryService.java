...
public class TaskGroupQueryService {
	...
	public List<String> listGroupIdsByPersonAndProject( EffectivePerson effectivePerson, String project ) throws Exception {
		...
		List<String> taskGroupListIds = new ArrayList<>();
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			taskGroupListIds.addAll(taskGroupService.listGroupIdsByPersonAndProject(emc, effectivePerson.getDistinguishedName(), project ));
			//如果用户在该项目中没有工作任务组，则需要创建一个默认的工作任务组
