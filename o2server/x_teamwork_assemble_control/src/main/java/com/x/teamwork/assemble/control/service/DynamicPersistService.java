...
public class DynamicPersistService {
	...
	private static final Logger logger = LoggerFactory.getLogger( DynamicPersistService.class );
	 ...
	 * @throws Exception
	 */
	public Dynamic projectUpdateStatusDynamic( Project object, EffectivePerson effectivePerson, String status) {
		Dynamic dynamic = null;
		try {
			dynamic = dynamicService.getUpdateStatusDynamic( object, effectivePerson ,status);
			try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				dynamic = dynamicService.save( emc, dynamic, null );
			}
		} catch (Exception e) {
	...
	public List<Dynamic> taskSaveDynamic( Task object_old, Task object_new, TaskDetail oldDetail , EffectivePerson effectivePerson, String content ) throws Exception {
		...
		try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			dynamics = dynamicService.getTaskDynamic( object_old, object_new, effectivePerson );
			if( ListTools.isNotEmpty( dynamics )) {
				 for( Dynamic dynamic : dynamics ) {
					 dynamic = dynamicService.save( emc, dynamic, content );
					 result.add( dynamic );
	...
	public List<Dynamic> taskUpdatePropertyDynamic(Task task, String dynamicTitle, String dynamicOptType, String dynamicDescription,  EffectivePerson effectivePerson) {
		List<Dynamic> dynamics = null;
		try {
			...
			dynamics = dynamicService.getTaskPropertyUpdateDynamic( task, dynamicTitle, dynamicOptType, dynamicDescription, effectivePerson);
			try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				if( ListTools.isNotEmpty( dynamics )) {
					for( Dynamic dynamic : dynamics ) {
						dynamicService.save( emc, dynamic, null );
