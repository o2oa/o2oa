...
public class ProjectQueryService {
	...
	private Cache.CacheCategory cache = new Cache.CacheCategory(Project.class);
	...
	public Project getFromCache( String id ) throws Exception {
		if ( StringUtils.isEmpty( id )) {
			return null;
		}
		Project project;
		...
		if (optional.isPresent()) {
			...
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				project = emc.find(id, Project.class);
			}
			if(project != null) {
				CacheManager.put(cache, cacheKey, project);
			}
		}
		return project;
	}
	...
	public List<String> listNotArchiveProject() throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			return business.projectFactory().listNotArchiveProject();
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public Boolean isProjectParticipant( String projectId, String personName ) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if( project.getParticipantList().contains( personName )) {
				return true;
			}
			project.getParticipantList().retainAll( identityNames );
			if( ListTools.isNotEmpty( project.getParticipantList() )) {
				return true;
			}
			project.getParticipantList().retainAll( unitNames );
			if( ListTools.isNotEmpty( project.getParticipantList() )) {
				return true;
			}
			project.getParticipantList().retainAll( groupNames );
			if( ListTools.isNotEmpty( project.getParticipantList() )) {
				return true;
			}
		} catch (Exception e) {
	...
	public List<Project> listPagingWithCondition( String personName, String orderField, String orderType, QueryFilter queryFilter, Integer adjustPage,
		...
		try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create() ) {
			Business business = new Business(emc);
			return business.projectFactory().listPagingWithCondition(personName, orderField, orderType, queryFilter, adjustPage, pageSize);
		} catch ( Exception e ) {
			throw e;
		}
	}
	...
	public List<Project> listWithCondition( String personName, String orderField, String orderType, QueryFilter queryFilter) throws Exception {
		try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create() ) {
			Business business = new Business(emc);
			return business.projectFactory().listWithCondition(personName, orderField, orderType, queryFilter);
		} catch ( Exception e ) {
			throw e;
		}
	}
	...
	public Long countWithCondition( String personName, QueryFilter queryFilter) throws Exception {
		...
		try ( EntityManagerContainer emc = EntityManagerContainerFactory.instance().create() ) {
			Business business = new Business(emc);
			return business.projectFactory().countWithCondition(personName, queryFilter);
		} catch ( Exception e ) {
			throw e;
		}
	}
