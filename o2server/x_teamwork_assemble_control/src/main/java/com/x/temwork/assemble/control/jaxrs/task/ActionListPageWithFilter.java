...
public class ActionListPageWithFilter extends BaseAction {
	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request, EffectivePerson effectivePerson, Integer pageNum, Integer count, JsonElement jsonElement ) throws Exception {
		...
		try {
			wrapIn = this.convertToWrapIn(jsonElement, WrapInQueryTask.class);
		} catch (Exception e) {
			check = false;
		...
		if( check ) {
			...
			if ((null != element) && (null != element.getObjectValue())) {
				...
				try {
					...
					List<Task> taskList = taskQueryService.listWithFilter( effectivePerson, count, pageNum, wrapIn.getOrderField(), wrapIn.getOrderType(), queryFilter );
					
					if( ListTools.isNotEmpty( taskList )) {
						for( Task task : taskList ) {
							wos.add( Wo.copier.copy(task));
						}
					}
