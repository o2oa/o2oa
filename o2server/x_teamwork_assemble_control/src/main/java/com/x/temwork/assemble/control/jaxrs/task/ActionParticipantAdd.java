...
public class ActionParticipantAdd extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionParticipantAdd.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson,  String id,  JsonElement jsonElement ) throws Exception {
		...
		if (check) {
			if( ListTools.isNotEmpty( wi.getParticipants() ) ) {
				try {					
					task = taskPersistService.addParticipants( id, wi.getParticipantPersons(), wi.getParticipantIdentitys(), wi.getParticipantUnits(), wi.getParticipantGroups(), effectivePerson );		
