...
public class ActionManagerRemove extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionManagerRemove.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson,  String id,  JsonElement jsonElement ) throws Exception {
		...
		if (check) {
			if( ListTools.isNotEmpty( wi.getManagers() ) ) {
				try {					
					task = taskPersistService.removeParticipants( id, wi.getManagers(), effectivePerson );		
					...
					check = false;
					Exception exception = new TaskPersistException(e, "移除工作任务管理者时发生异常。");
					result.error(exception);
					logger.error(e, effectivePerson, request, null);
				}
			}
