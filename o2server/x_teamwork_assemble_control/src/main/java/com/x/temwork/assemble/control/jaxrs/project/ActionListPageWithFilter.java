	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request, EffectivePerson effectivePerson, Integer pageNum, Integer count, JsonElement jsonElement ) throws Exception {
		...
		if( check ) {
			if ((null != element) && (null != element.getObjectValue())) {
				...
				try {					
					Long total = projectQueryService.countWithFilter(effectivePerson, queryFilter );
					List<Project> projectList = projectQueryService.listWithFilter( effectivePerson, count, pageNum, wrapIn.getOrderField(), wrapIn.getOrderType(), queryFilter);
					
					if( ListTools.isNotEmpty( projectList )) {
						WrapOutControl control = null;
						for( Project project : projectList ) {
							Wo wo = Wo.copier.copy(project);
							wo.setProjectDetail( projectQueryService.getDetail( project.getId() ));
