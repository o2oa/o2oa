	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String flag) throws Exception {
		...
		if (check) {
			try {
				...
				check = false;
				Exception exception = new ProjectQueryException(e, "根据指定flag查询项目信息对象时发生异常。flag:" + flag);
				result.error(exception);
		...
		if (check) {
			try {
				projectPersistService.delete(flag, effectivePerson );				
				// 更新缓存
				...
				check = false;
				Exception exception = new ProjectQueryException(e, "根据指定flag删除项目信息对象时发生异常。flag:" + flag);
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
		if (check) {
			try {					
				dynamicPersistService.save( project, "DELETE", effectivePerson, null );
			} catch (Exception e) {
				logger.error(e, effectivePerson, request, null);
			}	
		}
