...
public class GeneralFileAction extends StandardJaxrsAction {
	...
	@JaxrsMethodDescribe(value = "发送html文本到webServer下。", action = ActionHtmlToWeb.class)
	@POST
	@Path("{id}/html/to/web")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void htmlToWeb(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request,
						  @JaxrsParameterDescribe("证件ID") @PathParam("id") String id,JsonElement jsonElement) {
		ActionResult<ActionHtmlToWeb.Wo> result = new ActionResult<>();
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionHtmlToWeb().execute(effectivePerson,id,jsonElement);
		} catch (Exception e) {
			LOGGER.error(e, effectivePerson, request, null);
			result.error(e);
		}
