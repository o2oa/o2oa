...
public class ScriptingFactory2 {

	private static final Logger LOGGER = LoggerFactory.getLogger(ScriptingFactory2.class);

	private ScriptingFactory2() {
	...
	public static ScriptContext scriptContextEvalInitialServiceScript() {
		...
		try {
			ScriptingFactory2.initialServiceScript().eval(scriptContext);
	...
	public static ScriptContext scriptContextEvalInitialScript() {
		ScriptContext scriptContext = new SimpleScriptContext();
		try {
			ScriptingFactory2.initialScript().eval(scriptContext);
		} catch (ScriptException | URISyntaxException | IOException e) {
			LOGGER.error(e);
		}
		return scriptContext;
