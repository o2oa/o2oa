...
public class Query extends GsonPropertyObject {
	...
	public Query() {
		this.scopeType = ScopeType.workCompleted;
	...
	public void query() throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			/* 先进行字段调整 */
			this.adjust();
			List<String> jobs = this.listJob(emc);
			Table table = this.concreteTable(jobs);
			if (this.selectEntryListAvailable()) {
				this.fillSelectEntries(emc, jobs, table);
			}
	...
	private List<String> listJob(EntityManagerContainer emc) throws Exception {
		...
		if (Objects.equals(this.scopeType, ScopeType.workCompleted)) {
			jobs.addAll(this.listJob_onWorkCompleted(emc));
		} else {
			jobs.addAll(this.listJob_onWork(emc));
		}
		/** 针对DataItem进行判断 */
		if (ListTools.isNotEmpty(this.getRestrictFilterEntryList())
			...
			if (ListTools.isNotEmpty(this.getRestrictFilterEntryList())) {
				List<String> partJobs = new ArrayList<>();
	...
	private List<String> listJob_onWork(EntityManagerContainer emc) throws Exception {
		EntityManager em = emc.get(Work.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<String> cq = cb.createQuery(String.class);
		Root<Work> root = cq.from(Work.class);
		Predicate pd = cb.and(DateRangeEntryTools.toWorkPredicate(cb, root, this.getRestrictDateRangeEntry()),
				DateRangeEntryTools.toWorkPredicate(cb, root, this.getDateRangeEntry()));
		Predicate pw = cb.and(WhereEntryTools.toWorkPredicate(cb, root, this.getRestrictWhereEntry()),
				WhereEntryTools.toWorkPredicate(cb, root, this.getWhereEntry()));
		cq.select(root.get(Work_.job)).distinct(true).where(cb.and(pd, pw));
		List<String> jobs = em.createQuery(cq).getResultList();
		return jobs;
	}

	private List<String> listJob_onWorkCompleted(EntityManagerContainer emc) throws Exception {
		EntityManager em = emc.get(WorkCompleted.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
		CriteriaQuery<String> cq = cb.createQuery(String.class);
		Root<WorkCompleted> root = cq.from(WorkCompleted.class);
		Predicate pd = cb.and(DateRangeEntryTools.toWorkCompletedPredicate(cb, root, this.getRestrictDateRangeEntry()),
				DateRangeEntryTools.toWorkCompletedPredicate(cb, root, this.getDateRangeEntry()));
		Predicate pw = cb.and(WhereEntryTools.toWorkCompletedPredicate(cb, root, this.getRestrictWhereEntry()),
				WhereEntryTools.toWorkCompletedPredicate(cb, root, this.getWhereEntry()));
		cq.select(root.get(WorkCompleted_.job)).distinct(true).where(cb.and(pd, pw));
		List<String> jobs = em.createQuery(cq).getResultList();
		return jobs;
	}
	...
	private void fillSelectEntries(EntityManagerContainer emc, List<String> jobs, Table table) throws Exception {
		...
		this.fillAttributeSelectEntries(emc, jobs, attributeSelectEntries, table);
		this.fillPathSelectEntries(emc, jobs, pathSelectEntries, table);
	}

	private void fillAttributeSelectEntries(EntityManagerContainer emc, List<String> jobs,
			List<SelectEntry> selectEntries, Table table) throws Exception {
		if (this.scopeType.equals(ScopeType.workCompleted)) {
			this.fillAttributeSelectEntriesWorkCompleted(emc, jobs, selectEntries, table);
		} else {
			this.fillAttributeSelectEntriesWork(emc, jobs, selectEntries, table);
		}
	}

	private void fillAttributeSelectEntriesWork(EntityManagerContainer emc, List<String> jobs,
			List<SelectEntry> selectEntries, Table table) throws Exception {
		EntityManager em = emc.get(Work.class);
		CriteriaBuilder cb = em.getCriteriaBuilder();
		...
		for (SelectEntry en : selectEntries) {
			if (en.available()) {
				if (StringUtils.equals(Work.job_FIELDNAME, en.getAttribute())) {
					/* 单独处理一下job */
					selectionJob = root.get(Work_.job).alias(en.getColumn());
					selections.add(selectionJob);
				} else {
	...
	private void fillAttributeSelectEntriesWorkCompleted(EntityManagerContainer emc, List<String> jobs,
		...
		for (SelectEntry en : selectEntries) {
			if (en.available()) {
				if (StringUtils.equals(WorkCompleted.job_FIELDNAME, en.getAttribute())) {
					/* 单独处理一下job */
	...
	private void fillPathSelectEntries(EntityManagerContainer emc, List<String> jobs, List<SelectEntry> selectEntries,
