...
class ActionListAll extends BaseAction {
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			List<Wo> wos = Wo.copier.copy(this.list(business));
			List<WoView> views = WoView.copier.copy(this.listView(business));
			List<WoStat> stats = WoStat.copier.copy(this.listStat(business));
			List<WoReveal> reveals = WoReveal.copier.copy(this.listReveal(business));
	...
	private List<Query> list(Business business) throws Exception {
		...
		List<Query> os = em.createQuery(cq.select(root)).getResultList();
		return os;
	}

	private List<View> listView(Business business) throws Exception {
		...
		List<View> os = em.createQuery(cq.select(root)).getResultList();
		return os;
	}

	private List<Stat> listStat(Business business) throws Exception {
		...
		List<Stat> os = em.createQuery(cq.select(root)).getResultList();
		return os;
	}

	private List<Reveal> listReveal(Business business) throws Exception {
		...
		List<Reveal> os = em.createQuery(cq.select(root)).getResultList();
		return os;
	}
