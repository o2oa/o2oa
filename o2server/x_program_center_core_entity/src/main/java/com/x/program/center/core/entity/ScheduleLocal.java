...
@Entity
@ContainerEntity
@Table(name = PersistenceProperties.ScheduleLocal.table, uniqueConstraints = {
		@UniqueConstraint(name = PersistenceProperties.ScheduleLocal.table + JpaObject.IndexNameMiddle
				+ JpaObject.DefaultUniqueConstraintSuffix, columnNames = { JpaObject.IDCOLUMN,
						JpaObject.CREATETIMECOLUMN, JpaObject.UPDATETIMECOLUMN, JpaObject.SEQUENCECOLUMN }) })
@Inheritance(strategy = InheritanceType.TABLE_PER_CLASS)
public class ScheduleLocal extends SliceJpaObject {

	public ScheduleLocal() {

	}

	public ScheduleLocal(String className, String application, String node, Integer delay, Integer interval,
			String cron) {
		this.node = node;
	...
	private static final long serialVersionUID = 2682376909473629057L;

	private static final String TABLE = PersistenceProperties.ScheduleLocal.table;
	...
	public static final String delay_FIELDNAME = "delay";
	@FieldDescribe("第一次运行延时(秒).")
	@CheckPersist(allowEmpty = true)
	@Column(name = ColumnNamePrefix + delay_FIELDNAME)
	private Integer delay;

	public static final String interval_FIELDNAME = "interval";
	@FieldDescribe("每次运行延时(秒).")
	@CheckPersist(allowEmpty = true)
	@Column(name = ColumnNamePrefix + interval_FIELDNAME)
