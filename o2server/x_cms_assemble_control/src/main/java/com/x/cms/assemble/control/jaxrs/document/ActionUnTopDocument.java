...
public class ActionUnTopDocument extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionUnTopDocument.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, String id, EffectivePerson effectivePerson ) throws Exception {
		...
		if (check) {
			try {				
				documentInfoServiceAdv.unTopDocument( document.getId() );				
				Wo wo = new Wo();
