...
class V2Search extends V2Base {
	...
	private Predicate toPredicate(Business business, String person, String query) throws Exception {
		...
		if (StringUtils.isNotEmpty(person)) {
			p = cb.or(cb.equal(root.get(Review_.permissionObj), person),
					cb.equal(root.get(Review_.permissionObj), "*"));
		}
		query = "%" + query + "%";
		return cb.and(p, cb.like(root.get(Review_.title), query));
	}
	...
	private List<Review> list(Business business, Predicate predicate, Integer max, Integer startPosition)
		...
		CriteriaQuery<Review> cq = cb.createQuery(Review.class);
		Root<Review> root = cq.from(Review.class);
		return em.createQuery(cq.where(predicate).orderBy(cb.desc(root.get(JpaObject_.sequence)))).setMaxResults(max)
				.setFirstResult(startPosition).getResultList();
	}
	...
	@Schema(name = "com.x.cms.assemble.control.jaxrs.review.ActionSearch$Wi")
	...
	@Schema(name = "com.x.cms.assemble.control.jaxrs.review.ActionSearch$Wo")
