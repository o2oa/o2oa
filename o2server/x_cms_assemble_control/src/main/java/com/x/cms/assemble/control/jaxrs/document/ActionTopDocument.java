...
public class ActionTopDocument extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionTopDocument.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, String id, EffectivePerson effectivePerson ) throws Exception {
		...
		if (check) {
			try {				
				documentInfoServiceAdv.topDocument( document.getId() );
