...
public class ActionListCategoryInfoPublishers extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger( ActionListCategoryInfoPublishers.class );
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, String categoryId ) throws Exception {
		...
		if( check ){
			Wo wo = new Wo();
			wo.setPersonList( categoryInfo.getPublishablePersonList() );
			wo.setUnitList( categoryInfo.getPublishableUnitList() );
			wo.setGroupList( categoryInfo.getPublishableGroupList() );
			result.setData( wo );
		}
		return result;
	}
