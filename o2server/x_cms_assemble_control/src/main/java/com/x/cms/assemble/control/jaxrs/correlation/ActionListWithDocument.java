...
class ActionListWithDocument extends BaseAction {

	private static final Logger LOGGER = LoggerFactory.getLogger(ActionListWithDocument.class);

	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, String docId) throws Exception {
		...
		List<Wo> wos = ThisApplication.context().applications()
				.getQuery(effectivePerson.getDebugger(), x_correlation_service_processing.class,
						Applications.joinQueryUri("correlation", "list", "type", "cms", "document", docId), docId)
