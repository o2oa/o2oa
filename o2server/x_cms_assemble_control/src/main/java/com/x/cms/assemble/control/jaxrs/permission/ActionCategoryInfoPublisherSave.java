...
public class ActionCategoryInfoPublisherSave extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger( ActionCategoryInfoPublisherSave.class );
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, String categoryId, JsonElement jsonElement ) throws Exception {
		...
		if( check ){
			try {
				categoryInfoServiceAdv.updatePublisherPermission( categoryId, wi.getPersonList(), wi.getUnitList(), wi.getGroupList() );
			} catch (Exception e) {
				check = false;
		...
		if( check ) {
			try {
				new CmsBatchOperationPersistService().addOperation( 
						...
						CmsBatchOperationProcessService.OPT_TYPE_PERMISSION,  categoryId,  categoryId, "分类发布权限变更：ID=" +  categoryId );
			}catch( Exception e ) {
				e.printStackTrace();
			}
		}
