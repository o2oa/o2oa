		...
		if ((null != element) && (null != element.getObjectValue())) {
			...
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				...
				if (!business.fileInfoEditAvailable(request, effectivePerson)) {
					throw new Exception("[listFileInfoByDocumentId]person{name:"
							+ effectivePerson.getDistinguishedName() + "} 用户没有查询全部文件或者附件的权限！");
				}
				// 如果有权限，继续操作
				FileInfoFactory fileInfoFactory = business.getFileInfoFactory();
				List<String> ids = fileInfoFactory.listAttachmentByDocument(documentId);// 获取指定文档的所有附件列表
