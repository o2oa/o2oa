...
@Path("design/appdict")
@JaxrsDescribe("数据字典	")
public class AppDictDesignAction extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(AppDictDesignAction.class);

	@JaxrsMethodDescribe(value = "获取AppInfo的数据字典列表.", action = ActionListWithAppInfo.class)
	@GET
	@Path("list/appInfo/{appId}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response listWithAppInfo(@Context HttpServletRequest request,
			@JaxrsParameterDescribe("栏目标识") @PathParam("appId") String appId) {
		ActionResult<List<ActionListWithAppInfo.Wo>> result = new ActionResult<>();
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionListWithAppInfo().execute(effectivePerson, appId);
		} catch (Exception e) {
	...
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response put(@Context HttpServletRequest request, @JaxrsParameterDescribe("标识") @PathParam("id") String id,
