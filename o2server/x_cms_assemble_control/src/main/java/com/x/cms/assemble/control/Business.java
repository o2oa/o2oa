...
public class Business {
    ...
    public boolean isManager(EffectivePerson person) throws Exception {
        ...
        if (person.isManager()) {
            ...
            return BooleanUtils.isTrue(
                    organization().person().hasRole(person, OrganizationDefinition.Manager,
    ...
    public boolean isAppInfoManager(EffectivePerson person, AppInfo appInfo) throws Exception {
        ...
        if (appInfo != null) {
            ...
            if (ListTools.isNotEmpty(appInfo.getManageableGroupList())) {
                List<String> groupNames = this.organization().group()
                        .listWithPerson(person.getDistinguishedName());
            ...
            if (ListTools.isNotEmpty(appInfo.getManageableRoleList())) {
                List<String> roleNames = this.organization().role()
                        .listWithPerson(person.getDistinguishedName());
     ...
     * @throws Exception
     */
    public boolean isDocumentEditor(EffectivePerson person, AppInfo appInfo,
            CategoryInfo categoryInfo, Document document) throws Exception {
        if (isManager(person)) {
            return true;
        }
        List<String> unitNames = this.organization().unit()
        ...
        if (document != null) {
            ...
            if (ListTools.isNotEmpty(document.getManagerList()) && (document.getManagerList()
                    .contains(getShortTargetFlag(person.getDistinguishedName())))) {
                return true;
            }
            if (ListTools.isNotEmpty(document.getAuthorPersonList())
                    && (document.getAuthorPersonList()
                    .contains(getShortTargetFlag(person.getDistinguishedName())))) {
                return true;
            }
            if (ListTools.isNotEmpty(document.getAuthorUnitList()) && (ListTools.containsAny(
                    getShortTargetFlag(unitNames), document.getAuthorUnitList()))) {
                return true;
            }
            if (ListTools.isNotEmpty(document.getAuthorGroupList()) && (ListTools.containsAny(
                    getShortTargetFlag(groupNames), document.getAuthorGroupList()))) {
                return true;
            }
            if (appInfo == null) {
        ...
        boolean publishFlag = document == null;
        Set<String> personList = new HashSet<>();
        Set<String> unitList = new HashSet<>();
        Set<String> groupList = new HashSet<>();
        Set<String> roleList = new HashSet<>();
        if (categoryInfo != null) {
            personList.addAll(categoryInfo.getManageablePersonList());
            unitList.addAll(categoryInfo.getManageableUnitList());
            groupList.addAll(categoryInfo.getManageableGroupList());
            if (document == null) {
                personList.addAll(categoryInfo.getPublishablePersonList());
                unitList.addAll(categoryInfo.getPublishableUnitList());
                groupList.addAll(categoryInfo.getPublishableGroupList());
        ...
        if (appInfo != null) {
            personList.addAll(appInfo.getManageablePersonList());
            unitList.addAll(appInfo.getManageableUnitList());
            groupList.addAll(appInfo.getManageableGroupList());
            roleList.addAll(appInfo.getManageableRoleList());
            if (document == null) {
                personList.addAll(appInfo.getPublishablePersonList());
                unitList.addAll(appInfo.getPublishableUnitList());
                groupList.addAll(appInfo.getPublishableGroupList());
                roleList.addAll(appInfo.getPublishableRoleList());
                if (!appInfo.getPublishablePersonList().isEmpty()
                        || !appInfo.getPublishableUnitList().isEmpty()
                        || !appInfo.getPublishableGroupList().isEmpty()
                        || !appInfo.getPublishableRoleList().isEmpty()) {
                    publishFlag = false;
                }
            }
        }
        if (!personList.isEmpty() && personList.contains(
                person.getDistinguishedName())) {
            return true;
        }
        if (!unitList.isEmpty() && ListTools.containsAny(unitNames,
                new ArrayList<>(unitList))) {
            return true;
        }
        if (!groupList.isEmpty() && ListTools.containsAny(groupNames,
                new ArrayList<>(groupList))) {
            return true;
        }
        List<String> roleNames = this.organization().role()
                .listWithPerson(person.getDistinguishedName());
        if (!roleList.isEmpty() && ListTools.containsAny(roleNames,
    ...
    public boolean isDocumentReader(EffectivePerson person, Document document) throws Exception {
        ...
        Long count = this.reviewFactory()
                .countByDocumentAndPerson(document.getId(), person.getDistinguishedName());
        if (count > 0) {
            return true;
        }
     ...
     * @return
     */
    public boolean ifPersonHasSufficientSecurityClearance(String person,
            Integer objectSecurityClearance) {
        try {
            if (BooleanUtils.isFalse(Config.ternaryManagement().getSecurityClearanceEnable())) {
                return true;
            }
            ...
            if (null == subjectSecurityClearance) {
                subjectSecurityClearance = Config.ternaryManagement()
                        .getDefaultSubjectSecurityClearance();
            }
    ...
    public boolean ifDocumentHasBeenCorrelation(String person, String docId) throws Exception {
        ...
        ActionReadableTypeProcessPlatformWo resp = ThisApplication.context().applications()
                .postQuery(x_correlation_service_processing.class,
                        Applications.joinQueryUri("correlation", "readable", "type", "cms"), req,
                        docId)
    ...
    public boolean editable(EffectivePerson effectivePerson, AppInfo appInfo) throws Exception {
        if ((StringUtils.equals(appInfo.getCreatorPerson(), effectivePerson.getDistinguishedName()))
                ...
                || organization().person()
                .hasRole(effectivePerson, OrganizationDefinition.CMSManager)) {
            return true;
        }
        ...
        List<String> groupList = this.organization().group()
        ...
        if (ListTools.isNotEmpty(appInfo.getManageablePersonList())) {
            return appInfo.getManageablePersonList()
                    .contains(effectivePerson.getDistinguishedName());
        }

        if (ListTools.isNotEmpty(appInfo.getManageableGroupList()) && ListTools.isNotEmpty(
                groupNameList)) {
            groupNameList.retainAll(appInfo.getManageableGroupList());
            return ListTools.isNotEmpty(groupNameList);
        }

        if (ListTools.isNotEmpty(appInfo.getManageableUnitList()) && ListTools.isNotEmpty(
                unitNameList)) {
            unitNameList.retainAll(appInfo.getManageableUnitList());
            return ListTools.isNotEmpty(unitNameList);
        }

        List<String> roleList = this.organization().role()
                .listWithPerson(effectivePerson.getDistinguishedName());
        List<String> roleNameList = new ArrayList<>(roleList);
        if (ListTools.isNotEmpty(appInfo.getManageableRoleList()) && ListTools.isNotEmpty(
                roleNameList)) {
     ...
     * @throws Exception
     */
    public void downToZip(List<FileInfo> attachmentList, OutputStream os,
            Map<String, byte[]> otherAttMap)
            throws Exception {
        ...
        try (ZipOutputStream zos = new ZipOutputStream(os)) {
            for (Map.Entry<String, FileInfo> entry : filePathMap.entrySet()) {
                zos.putNextEntry(new ZipEntry(
                        StringUtils.replaceEach(entry.getKey(), FILENAME_SENSITIVES_KEY,
