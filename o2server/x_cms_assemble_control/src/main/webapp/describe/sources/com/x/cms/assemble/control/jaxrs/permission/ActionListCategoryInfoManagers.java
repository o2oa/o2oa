...
public class ActionListCategoryInfoManagers extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger( ActionListCategoryInfoManagers.class );
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, String categoryId ) throws Exception {
		...
		if( check ){
			Wo wo = new Wo();
			wo.setPersonList( categoryInfo.getManageablePersonList() );
			wo.setUnitList( categoryInfo.getManageableUnitList() );
			wo.setGroupList( categoryInfo.getManageableGroupList() );
			result.setData( wo );
		}
		return result;
	}
