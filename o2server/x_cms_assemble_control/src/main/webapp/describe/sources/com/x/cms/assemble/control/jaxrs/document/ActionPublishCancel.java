...
public class ActionPublishCancel extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionPublishCancel.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, String id, EffectivePerson effectivePerson) throws Exception {
		...
		if (check) {
			try {
				documentInfoServiceAdv.refreshDocumentPermission( id, null, null );
			} catch (Exception e) {
				check = false;
