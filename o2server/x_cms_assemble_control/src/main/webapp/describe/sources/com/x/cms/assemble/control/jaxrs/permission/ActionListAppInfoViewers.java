...
public class ActionListAppInfoViewers extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger( ActionListAppInfoViewers.class );
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, String appId ) throws Exception {
		...
		if( check ){
			Wo wo = new Wo();
			wo.setPersonList( appInfo.getViewablePersonList() );
			wo.setUnitList( appInfo.getViewableUnitList() );
			wo.setGroupList( appInfo.getViewableGroupList() );
			result.setData( wo );
		}
		return result;
	}
