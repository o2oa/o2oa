...
import com.x.okr.assemble.control.jaxrs.okrworkchat.ActionSave;

@Path("okrworkdetailinfo")
@JaxrsDescribe("工作详细信息管理服务")
public class OkrWorkDetailInfoAction extends StandardJaxrsAction {
	private static Logger logger = LoggerFactory.getLogger(OkrWorkDetailInfoAction.class);

	@JaxrsMethodDescribe(value = "新建或者更新工作详细信息", action = ActionSave.class)
	@POST
	...
	@JaxrsMethodDescribe(value = "根据ID删除工作详细信息", action = ActionDelete.class)
	@DELETE
	@Path("{id}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	...
	@JaxrsMethodDescribe(value = "根据ID获取工作详细信息", action = ActionDelete.class)
	@GET
	@Path("{id}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
