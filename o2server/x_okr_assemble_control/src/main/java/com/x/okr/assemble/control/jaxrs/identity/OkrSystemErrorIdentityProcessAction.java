...
public class OkrSystemErrorIdentityProcessAction extends StandardJaxrsAction {
	...
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response testReplaceAll( @Context HttpServletRequest request ) {
		EffectivePerson effectivePerson = this.effectivePerson( request );
		ActionResult<WrapOutString> result = new ActionResult<>();
		try {
	...
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response errorInfoFilterListNextWithFilter(@Context HttpServletRequest request, 
			@JaxrsParameterDescribe("上一页最后一条信息的ID") @PathParam("id") String id,
			@JaxrsParameterDescribe("每页显示的条目数量") @PathParam("count") Integer count, 
			JsonElement jsonElement) {
		EffectivePerson effectivePerson = this.effectivePerson( request );
		ActionResult<List<ActionErrorInfoFilterListNextWithFilter.Wo>> result = new ActionResult<>();
		try {
			result = new ActionErrorInfoFilterListNextWithFilter().execute( request, effectivePerson, id, count, jsonElement );
		} catch (Exception e) {
			result = new ActionResult<>();
	...
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response getOkrErrorIdentityRecords(@Context HttpServletRequest request, 
			@JaxrsParameterDescribe("上一页最后一条信息的ID") @PathParam("id") String id,
			@JaxrsParameterDescribe("每页显示的条目数量") @PathParam("count") Integer count,
			JsonElement jsonElement) {
		EffectivePerson effectivePerson = this.effectivePerson( request );
