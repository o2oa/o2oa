...
public class ActionListMyTaskPrevWithFilter extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionListMyTaskPrevWithFilter.class);
	...
	protected ActionResult<List<Wo>> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id,
		...
		Boolean check = true;
		try {
			wrapIn = this.convertToWrapIn(jsonElement, com.x.okr.assemble.control.jaxrs.WorkPersonSearchFilter.class);
		} catch (Exception e) {
			check = false;
	...
	public static class Wo extends OkrWorkReportPersonLink {
		...
		public static WrapCopier<OkrWorkReportPersonLink, Wo> copier = WrapCopierFactory
				.wo(OkrWorkReportPersonLink.class, Wo.class, null, JpaObject.FieldsInvisible);
