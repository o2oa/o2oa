...
public class ActionStReportContentListByFilter extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionStReportContentListByFilter.class);
	...
	protected ActionResult<List<WoOkrStatisticReportContentCenter>> execute( HttpServletRequest request, EffectivePerson effectivePerson, JsonElement jsonElement ) throws Exception {
		...
		if( check ){
			try {
				result = new ActionFilterList().execute( request, effectivePerson, wrapIn, false );
			} catch (Exception e) {
