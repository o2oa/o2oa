...
public class ActionListWithFilterNext extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger( ActionListWithFilterNext.class );
	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request,EffectivePerson effectivePerson, String id, Integer count, JsonElement jsonElement ) throws Exception {
		...
		if( check ){
			try{
				chatList = okrWorkChatService.listChatNextWithFilter( id, count, wrapIn.getWorkId(), wrapIn.getSequenceField(), wrapIn.getOrder() );
