...
public class OkrCenterWorkInfoAction extends StandardJaxrsAction{
	...
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public Response draftNew(@Context HttpServletRequest request ) {
		EffectivePerson effectivePerson = this.effectivePerson( request );
	...
	public Response listDraftNextWithFilter( @Context HttpServletRequest request, 
		...
		if( check ){
			try {
				...
				result = new ActionListByProcessIdentityNextWithFilter().execute( request, effectivePerson, id, count, wrapIn );
			} catch (Exception e) {
				result = new ActionResult<>();
				logger.error( e, effectivePerson, request, null);
			}
	...
	public Response listDraftPrevWithFilter(@Context HttpServletRequest request, 
		...
		if( check ){
			try {
				...
				wrapIn.addQueryWorkProcessStatus( "草稿" );
				wrapIn.addQueryProcessIdentity( "部署者" );
				result = new ActionListByProcessIdentityPrevWithFilter().execute( request, effectivePerson, id, count, wrapIn );
			} catch (Exception e) {
				result = new ActionResult<>();
				logger.error( e, effectivePerson, request, null);
			}
