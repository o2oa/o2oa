...
class ActionSync extends BaseAction {
	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson) throws Exception {
		...
		for(String appId : aiConfig.getKnowledgeIndexAppList()){
			appId = appId.contains("|") ? StringUtils.substringBefore(appId, "|") : appId;
			List<String> docIds = listByAppId(appId);
			for (String docId : docIds) {
				ThisApplication.queueDocumentIndex.send(docId);
			}
		}
		for(String appId : aiConfig.getQuestionsIndexAppList()){
			appId = appId.contains("|") ? StringUtils.substringBefore(appId, "|") : appId;
			List<String> docIds = listByAppId(appId);
			for (String docId : docIds) {
				ThisApplication.queueDocumentIndex.send(docId);
