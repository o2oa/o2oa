...
public class IndexAction extends StandardJaxrsAction {
	...
	@Operation(summary = "获取检索目录列表.", operationId = OPERATIONID_PREFIX + "available", responses = {
			@ApiResponse(content = { @Content(schema = @Schema(implementation = ActionAvailable.Wo.class)) }) })
	@JaxrsMethodDescribe(value = "获取检索目录列表.", action = ActionAvailable.class)
	@GET
	@Path("available")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void available(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request) {
		...
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionAvailable().execute(effectivePerson);
		} catch (Exception e) {
			LOGGER.error(e, effectivePerson, request, null);
			result.error(e);
		}
