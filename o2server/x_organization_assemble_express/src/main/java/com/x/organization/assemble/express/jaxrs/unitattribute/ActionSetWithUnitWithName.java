...
class ActionSetWithUnitWithName extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(ActionSetWithUnitWithName.class);
	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, JsonElement jsonElement) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (null == unit) {
				wo.setValue(false);
				logger.warn("user:{} set unitAttribute:{} fail, unit:{} not exist.",
						effectivePerson.getDistinguishedName(), StringUtils.join(wi.getAttributeList(), ","),
						wi.getUnit());
			} else if ((!effectivePerson.isManager())) {
				wo.setValue(false);
				logger.warn("user:{} set unitAttribute unit:{}, value:{} fail, permission denied.",
