...
class ActionAppendWithPersonWithName extends BaseAction {
	...
	private static Logger logger = LoggerFactory.getLogger(ActionAppendWithPersonWithName.class);
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (null == person) {
				...
				if (null == personAttribute) {
					...
					list.addAll(personAttribute.getAttributeList());
					list.addAll(wi.getAttributeList());
					personAttribute.setAttributeList(ListTools.trim(list, true, false));
					personAttribute.setName(wi.getName());
					personAttribute.setPerson(person.getId());
