	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, String key) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (!keys.isEmpty()) {
				...
				ids = this.sort(ids);
				Role searchPrivilege = business.organization().role().getObject(OrganizationDefinition.SearchPrivilege);
				if (searchPrivilege != null) {
					if (!searchPrivilege.getPersonList().isEmpty() || (!searchPrivilege.getGroupList().isEmpty())) {
