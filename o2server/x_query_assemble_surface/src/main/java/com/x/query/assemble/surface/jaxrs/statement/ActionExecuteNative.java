...
class ActionExecuteNative extends BaseAction {
	...
	ActionResult<Object> execute(EffectivePerson effectivePerson, String flag, Integer page, Integer size,
		...
		if (StringUtils.equals(statement.getFormat(), Statement.FORMAT_SCRIPT)) {
			data = this.script(effectivePerson, statement, runtime, page, size);
		} else {
			data = this.jpql(statement, runtime, page, size);
		}
		result.setData(data);
		return result;
	}
	...
	private Object jpql(Statement statement, Runtime runtime, Integer page, Integer size) throws Exception {
		Object data = null;
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			Business business = new Business(emc);
			Class<? extends JpaObject> cls = this.clazz(business, statement);
			...
			Query query = em.createNativeQuery(statement.getData());
			for (Parameter<?> p : query.getParameters()) {
				if (runtime.hasParameter(p.getName())) {
					query.setParameter(p.getName(), runtime.getParameter(p.getName()));
				}
