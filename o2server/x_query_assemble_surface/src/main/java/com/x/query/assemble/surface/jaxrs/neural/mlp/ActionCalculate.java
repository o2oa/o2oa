...
class ActionCalculate extends BaseAction {
	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, String projectFlag, String workId) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			Wo wo = ThisApplication.context().applications().getQuery(x_query_service_processing.class, Applications
					.joinQueryUri("neural", "mlp", "calculate", "project", project.getId(), "work", work.getId()))
					.getData(Wo.class);
			result.setData(wo);
			return result;
		}
