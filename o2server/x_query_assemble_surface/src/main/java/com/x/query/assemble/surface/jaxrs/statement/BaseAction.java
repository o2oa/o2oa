...
//        }
		...
		if (keys.contains(Runtime.PARAMETER_UNITLIST)) {
			runtime.parameters.put(Runtime.PARAMETER_UNITLIST, organization.unit().listWithPerson(effectivePerson));
		}
		...
		if (keys.contains(Runtime.PARAMETER_GROUPLIST)) {
			runtime.parameters.put(Runtime.PARAMETER_GROUPLIST, organization.group().listWithPerson(effectivePerson));
		}
		if (keys.contains(Runtime.PARAMETER_ROLELIST)) {
			runtime.parameters.put(Runtime.PARAMETER_ROLELIST, organization.role().listWithPerson(effectivePerson));
		}
