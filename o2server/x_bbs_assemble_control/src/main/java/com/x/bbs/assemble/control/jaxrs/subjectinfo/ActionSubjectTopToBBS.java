...
public class ActionSubjectTopToBBS extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectTopToBBS.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToBBS(id, true, effectivePerson.getDistinguishedName());
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation(e, "用户在设置全局置顶主题信息时发生异常！");
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
