...
public class ActionSubjectSetRecommendToBBSIndex extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectSetRecommendToBBSIndex.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.recommendToBBSIndex(id, true,
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation(e, "用户在设置推荐主题到论坛首页时发生异常！");
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
