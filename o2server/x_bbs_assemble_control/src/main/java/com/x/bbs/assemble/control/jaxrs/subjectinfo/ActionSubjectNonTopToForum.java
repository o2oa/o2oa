...
public class ActionSubjectNonTopToForum extends BaseAction {
	
	private static  Logger logger = LoggerFactory.getLogger( ActionSubjectNonTopToForum.class );
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToForum( id, false, effectivePerson.getDistinguishedName() );
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation( e, "用户在取消论坛置顶主题信息时发生异常！" );
				result.error( exception );
				logger.error( e, effectivePerson, request, null);
			}
		}
