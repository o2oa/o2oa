...
public class ActionSubjectCompleted extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectCompleted.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.complete(id, true, effectivePerson.getDistinguishedName());
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation(e, "用户完成主题时发生异常！");
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
