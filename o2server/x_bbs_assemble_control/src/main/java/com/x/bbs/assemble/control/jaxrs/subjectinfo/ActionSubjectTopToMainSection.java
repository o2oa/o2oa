...
public class ActionSubjectTopToMainSection extends BaseAction {
	
	private static  Logger logger = LoggerFactory.getLogger( ActionSubjectTopToMainSection.class );
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id ) throws Exception {
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setTopToMainSection( id, true, effectivePerson.getDistinguishedName() );
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation( e, "用户在设置主版块置顶主题信息时发生异常！" );
				result.error( exception );
				logger.error( e, effectivePerson, request, null);
			}
		}
