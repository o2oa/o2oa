...
public class ActionSubjectNonRecommendToBBSIndex extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectNonRecommendToBBSIndex.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.recommendToBBSIndex(id, false,
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation(e, "用户在取消推荐主题信息时发生异常！");
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
