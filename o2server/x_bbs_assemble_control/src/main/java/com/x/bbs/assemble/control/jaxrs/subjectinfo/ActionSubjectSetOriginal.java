...
public class ActionSubjectSetOriginal extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectSetOriginal.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setOriginal(id, true, effectivePerson.getDistinguishedName());
