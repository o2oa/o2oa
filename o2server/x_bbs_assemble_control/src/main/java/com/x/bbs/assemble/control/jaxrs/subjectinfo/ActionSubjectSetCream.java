...
public class ActionSubjectSetCream extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectSetCream.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setCream(id, true, effectivePerson.getDistinguishedName());
