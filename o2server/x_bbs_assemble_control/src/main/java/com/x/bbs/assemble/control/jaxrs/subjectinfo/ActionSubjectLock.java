...
public class ActionSubjectLock extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectLock.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.lock(id, true, effectivePerson.getDistinguishedName());
