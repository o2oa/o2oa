...
public class ActionSubjectNonOriginal extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectNonOriginal.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setOriginal(id, false, effectivePerson.getDistinguishedName());
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation(e, "用户在取消原创主题信息时发生异常！");
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
