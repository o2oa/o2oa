...
public class ActionSubjectNonCream extends BaseAction {

	private static  Logger logger = LoggerFactory.getLogger(ActionSubjectNonCream.class);
	...
	protected ActionResult<Wo> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id)
		...
		if (check) {
			try {
				subjectInfo = subjectInfoServiceAdv.setCream(id, false, effectivePerson.getDistinguishedName());
				...
				check = false;
				Exception exception = new ExceptionSubjectOperation(e, "用户在取消精华主题信息时发生异常！");
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
