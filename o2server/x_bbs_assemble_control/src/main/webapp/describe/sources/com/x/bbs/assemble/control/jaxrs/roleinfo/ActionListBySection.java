...
public class ActionListBySection extends BaseAction {
	
	private static  Logger logger = LoggerFactory.getLogger( ActionListBySection.class );
	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request, EffectivePerson effectivePerson, JsonElement jsonElement ) throws Exception {
		...
		if (check) {
			try {
				...
				check = false;
				Exception exception = new ExceptionRoleInfoProcess( e, "根据指定ID查询版块信息时发生异常.ID:" + wrapIn.getSectionId() );
				result.error( exception );
				logger.error( e, effectivePerson, request, null);
			}
		}
		...
		if (check) {
			try {
				...
				check = false;
				result.error(e);
				Exception exception = new ExceptionRoleInfoProcess( e, "系统在根据版块ID查询角色信息列表时发生异常.Section:" + wrapIn.getSectionId() );
				result.error( exception );
				logger.error( e, effectivePerson, request, null);
			}
		}
