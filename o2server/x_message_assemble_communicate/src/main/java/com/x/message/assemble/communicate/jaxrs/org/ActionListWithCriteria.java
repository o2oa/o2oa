...
public class ActionListWithCriteria extends BaseAction {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ActionListWithCriteria.class);
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, JsonElement jsonElement) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			List<Wo> wos = this.list(business, wi.getConsume(), NumberUtils.min(200, NumberUtils.max(1, count)),
					receiveSystem, consumedModule, operType, orgType);
			result.setData(wos);
			return result;
		}
	}

	private List<Wo> list(Business business, boolean consume, Integer count, String receiveSystem,
			String consumedModule, String operType, String orgType) throws Exception {
	...
	public static class Wi extends GsonPropertyObject {
		...
		private static final long serialVersionUID = 8100841404900371033L;
