...
public class Business {
    ...
    public boolean ifPersonHasTaskReadTaskCompletedReadCompletedReviewWithJob(String person,
            String job) {
        Optional<Triple<Class<? extends JpaObject>, String, String>> opt = Stream
                .<Triple<Class<? extends JpaObject>, String, String>>of(
                        Triple.of(Review.class, person, job),
                        Triple.of(TaskCompleted.class, person, job),
                        Triple.of(ReadCompleted.class, person, job),
                        Triple.of(Task.class, person, job), Triple.of(Read.class, person, job))
                .filter(this::hasTaskOrReadOrTaskCompletedOrReadCompletedOrReviewWithPersonWithJob)
                .findFirst();
     ...
     * @return
     */
    public boolean ifPersonHasPermissionWriteReviewWithJob(EffectivePerson effectivePerson,
            String job) {
        try {
            return emc.countEqualAndEqualAndEqual(Review.class, Review.person_FIELDNAME,
                    ...
                    Review.PERMISSIONWRITE_FIELDNAME,
                    true) > 0;
        } catch (Exception e) {
            LOGGER.error(e);
        }
    ...
    public boolean ifPersonCanManageApplicationOrProcess(EffectivePerson effectivePerson,
        ...
        if (BooleanUtils.isTrue(
                organization().person().hasRole(effectivePerson, OrganizationDefinition.Manager,
    ...
    public boolean ifPersonCanManageApplicationOrProcess(String person, String applicationId,
            String processId)
            throws Exception {
        if (BooleanUtils.isTrue(
    ...
    private boolean hasTaskOrReadOrTaskCompletedOrReadCompletedOrReviewWithPersonWithJob(
        ...
        try {
            return emc.countEqualAndEqual(param.first(), Task.person_FIELDNAME, param.second(),
                    Task.job_FIELDNAME,
                    param.third()) > 0;
        } catch (Exception e) {
            LOGGER.error(e);
    ...
    public boolean ifJobHasBeenCorrelation(String person, String job) throws Exception {
        ...
        ActionReadableTypeProcessPlatformWo resp = ThisApplication.context().applications()
                .postQuery(x_correlation_service_processing.class,
                        Applications.joinQueryUri("correlation", "readable", "type",
                                "processplatform"), req, job)
    ...
    public boolean ifPersonHasReadWithJob(String person, String job) throws Exception {
        return emc.countEqualAndEqual(Read.class, Read.person_FIELDNAME, person, Read.job_FIELDNAME,
                job) > 0;
    }

    public Optional<Task> ifPersonHasTaskWithWork(String person, String workId) throws Exception {
        Task task = emc.firstEqualAndEqual(Task.class, Task.person_FIELDNAME, person,
                Task.work_FIELDNAME, workId);
    ...
    public boolean ifPersonHasPauseTaskWithWork(String person, String work) throws Exception {
        return emc.countEqualAndEqualAndEqual(Task.class, Task.person_FIELDNAME, person,
                Task.work_FIELDNAME, work,
