...
class V2GetWorkOrWorkCompleted extends BaseAction {
    ...
    ActionResult<Wo> execute(EffectivePerson effectivePerson, String workOrWorkCompleted)
        ...
        CompletableFuture<Boolean> checkControlFuture = this.checkControlVisitFuture(
                effectivePerson,
                workOrWorkCompleted);
        ...
        if (null != work) {
            ...
            CompletableFuture<Void> dataFuture = this.dataFuture(work, wo, effectivePerson);
            CompletableFuture<Void> taskFuture = this.taskFuture(effectivePerson, work.getJob(),
                    work.getId(), wo);
            CompletableFuture<Void> readFuture = this.readFuture(effectivePerson, work.getJob(),
                    wo);
            CompletableFuture<Void> creatorIdentityFuture = this.creatorIdentityFuture(
                    work.getCreatorIdentity(), wo);
            CompletableFuture<Void> creatorPersonFuture = this.creatorPersonFuture(
                    work.getCreatorPerson(), wo);
            CompletableFuture<Void> creatorUnitFuture = this.creatorUnitFuture(
                    work.getCreatorUnit(), wo);
            CompletableFuture<Void> attachmentFuture = this.attachmentFuture(effectivePerson,
                    work.getJob(), wo);
            CompletableFuture<Void> recordFuture = this.recordFuture(effectivePerson, work.getJob(),
                    wo);
            workJsonFuture.get(Config.processPlatform().getAsynchronousTimeout(), TimeUnit.SECONDS);
            activityRouteFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            ...
            creatorIdentityFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            creatorPersonFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            creatorUnitFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            attachmentFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            ...
            CompletableFuture<Void> workCompletedJsonFuture = this.workCompletedJsonFuture(
                    workCompleted, wo);
            CompletableFuture<Void> activityFuture = this.activityFuture(workCompleted, wo);
            CompletableFuture<Void> workCompletedDataFuture = this.workCompletedDataFuture(
                    workCompleted, wo, effectivePerson);
            CompletableFuture<Void> readFuture = readFuture(effectivePerson, workCompleted.getJob(),
                    wo);
            CompletableFuture<Void> creatorIdentityFuture = creatorIdentityFuture(
                    workCompleted.getCreatorIdentity(),
                    wo);
            CompletableFuture<Void> creatorPersonFuture = creatorPersonFuture(
                    workCompleted.getCreatorPerson(), wo);
            CompletableFuture<Void> creatorUnitFuture = creatorUnitFuture(
                    workCompleted.getCreatorUnit(), wo);
            CompletableFuture<Void> attachmentFuture = attachmentFuture(effectivePerson,
                    workCompleted.getJob(), wo);
            CompletableFuture<Void> workCompletedRecordFuture = this.workCompletedRecordFuture(
                    effectivePerson,
                    workCompleted, wo);
            workCompletedJsonFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            activityFuture.get(Config.processPlatform().getAsynchronousTimeout(), TimeUnit.SECONDS);
            workCompletedDataFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            readFuture.get(Config.processPlatform().getAsynchronousTimeout(), TimeUnit.SECONDS);
            creatorIdentityFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            ...
            attachmentFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            workCompletedRecordFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                    TimeUnit.SECONDS);
            jobView(effectivePerson, workCompleted.getJob());
        }

        if (BooleanUtils
                .isFalse(checkControlFuture.get(Config.processPlatform().getAsynchronousTimeout(),
                        TimeUnit.SECONDS))) {
    ...
    private void jobView(EffectivePerson effectivePerson, String job) throws Exception {
        ThisApplication.context().applications()
                .getQuery(x_processplatform_service_processing.class,
                        Applications.joinQueryUri("job", "v2", job, "person",
    ...
    private CompletableFuture<Void> taskFuture(EffectivePerson effectivePerson, String job,
            String workId, Wo wo) {
        return CompletableFuture.runAsync(() -> {
            try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
                wo.setTaskList(
                        WoTask.copier.copy(emc.listEqual(Task.class, Task.job_FIELDNAME, job)));
                wo.setCurrentTaskIndex(ListUtils.indexOf(wo.getTaskList(),
                        e -> effectivePerson.isPerson(e.getPerson()) && (StringUtils.equals(
                                e.getWork(), workId))));
            } catch (Exception e) {
                LOGGER.error(e);
            }
    ...
    private CompletableFuture<Void> attachmentFuture(EffectivePerson effectivePerson, String job,
            Wo wo) {
        return CompletableFuture.runAsync(() -> {
            try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
                Business business = new Business(emc);
                ...
                for (Attachment attachment : business.entityManagerContainer()
                        .listEqual(Attachment.class,
                                Attachment.job_FIELDNAME, job)) {
                    boolean canControl = attachmentControl(attachment, effectivePerson, identities,
                            units);
                    boolean canEdit = attachmentEdit(attachment, effectivePerson, identities, units)
                            || canControl;
                    boolean canRead = attachmentRead(attachment, effectivePerson, identities, units)
                            || canEdit;
                ...
                wos = wos.stream()
                        .sorted(Comparator
                                .comparing(WoAttachment::getOrderNumber,
    ...
    private CompletableFuture<Void> readFuture(EffectivePerson effectivePerson, String job, Wo wo) {
        return CompletableFuture.runAsync(() -> {
            try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
                wo.setReadList(
                        WoRead.copier.copy(emc.listEqual(Read.class, Read.job_FIELDNAME, job)));
                wo.setCurrentReadIndex(
                        ListUtils.indexOf(wo.getReadList(),
                                e -> effectivePerson.isPerson(e.getPerson())));
            } catch (Exception e) {
                LOGGER.error(e);
            }
        }, ThisApplication.forkJoinPool());
    ...
    private CompletableFuture<Void> activityFuture(WorkCompleted workCompleted, Wo wo) {
        return CompletableFuture.runAsync(() -> {
            ...
            try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
                Business business = new Business(emc);
                Activity activity = business.getActivity(workCompleted.getActivity(),
                        ActivityType.end);
                if (null != activity) {
                    activity.copyTo(woActivity);
                    wo.setActivity(woActivity);
    ...
    private CompletableFuture<Void> creatorIdentityFuture(String creatorIdentity, Wo wo) {
        return CompletableFuture.runAsync(() -> {
            try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
                Business business = new Business(emc);
                wo.setCreatorIdentity(
                        business.organization().identity().getObject(creatorIdentity));
            } catch (Exception e) {
                LOGGER.error(e);
            }
        }, ThisApplication.forkJoinPool());
    ...
    private CompletableFuture<Void> recordFuture(EffectivePerson effectivePerson, String job,
            Wo wo) {
        return CompletableFuture.runAsync(() -> {
            try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ...
            try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
                if (ListTools.isNotEmpty(workCompleted.getRecordList())) {
                    wo.setRecordList(WoRecord.copier.copy(workCompleted.getRecordList()).stream()
                            .sorted(Comparator.comparing(WoRecord::getOrder))
                            .collect(Collectors.toList()));
                } else {
                    wo.setRecordList(emc.listEqual(Record.class, Record.job_FIELDNAME,
                                    workCompleted.getJob()).stream()
                            .sorted(Comparator.comparing(Record::getOrder))
                            .map(WoRecord.copier::copy)
                            .collect(Collectors.toList()));
                }
            } catch (Exception e) {
                LOGGER.error(e);
    ...
    private boolean attachmentRead(Attachment attachment, EffectivePerson effectivePerson,
    ...
    private boolean attachmentEdit(Attachment attachment, EffectivePerson effectivePerson,
            List<String> identities,
            List<String> units) throws Exception {
        boolean value = false;
        if (effectivePerson.isManager()) {
    ...
    private boolean attachmentControl(Attachment attachment, EffectivePerson effectivePerson,
            List<String> identities,
            List<String> units) throws Exception {
        boolean value = false;
        if (effectivePerson.isManager()) {
    ...
    public static class WoWorkCompleted extends WorkCompleted {
        ...
        static WrapCopier<WorkCompleted, WoWorkCompleted> copier = WrapCopierFactory.wo(
                WorkCompleted.class,
    ...
    public static class WoActivity extends GsonPropertyObject {
        ...
        static WrapCopier<Activity, WoActivity> copier = WrapCopierFactory.wo(Activity.class,
                WoActivity.class,
        ...
        public void setProcessingTaskOnceUnderSamePerson(
                Boolean processingTaskOnceUnderSamePerson) {
    ...
    public static class WoRoute extends Route {
        ...
        static WrapCopier<Route, WoRoute> copier = WrapCopierFactory.wo(Route.class, WoRoute.class,
                null,
                ListTools.toList(Route.createTime_FIELDNAME, Route.edition_FIELDNAME,
                        Route.position_FIELDNAME,
                        Route.process_FIELDNAME, Route.updateTime_FIELDNAME,
                        Route.track_FIELDNAME));
    }
    ...
    public static class WoAttachment extends Attachment {
        ...
        static WrapCopier<Attachment, WoAttachment> copier = WrapCopierFactory.wo(Attachment.class,
                WoAttachment.class,
    ...
    public static class WoRecord extends Record {
        ...
        static WrapCopier<Record, WoRecord> copier = WrapCopierFactory.wo(Record.class,
                WoRecord.class, null,
                JpaObject.FieldsInvisible);
