...
class ActionCreateDataPath3 extends BaseAction {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ActionCreateDataPath3.class);

	ActionResult<Wo> execute(EffectivePerson effectivePerson, String applicationDictFlag, String applicationFlag,
			String path0, String path1, String path2, String path3, JsonElement jsonElement) throws Exception {
		...
		Wo wo = ThisApplication.context().applications().postQuery(x_processplatform_service_processing.class,
				Applications.joinQueryUri("applicationdict", id, path0, path1, path2, path3, "data"), jsonElement, id)
				.getData(Wo.class);
		result.setData(wo);
		return result;
	}

	@Schema(name = "com.x.processplatform.assemble.surface.jaxrs.applicationdict.ActionCreateDataPath3$Wo")
	public static class Wo extends WoId {

		private static final long serialVersionUID = -6753039349551369477L;
