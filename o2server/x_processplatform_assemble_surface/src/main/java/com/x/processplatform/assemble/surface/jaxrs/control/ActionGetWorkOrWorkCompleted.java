...
class ActionGetWorkOrWorkCompleted extends BaseAction {
	...
	private void setAllowGoBack(Business business, Work work, Wo wo, Activity activity) throws Exception {
		if (activity.getClass().isAssignableFrom(Manual.class)) {
			...
			if (BooleanUtils.isNotFalse(manual.getAllowGoBack())
					&& (BooleanUtils.isNotFalse(manual.getGoBackConfig().getMultiTaskEnable())
							|| taskCountWithWork(business, work) <= 1)) {
				wo.setAllowGoBack(true);
			}
		}

	}

	private Long taskCountWithWork(Business business, Work work) throws Exception {
