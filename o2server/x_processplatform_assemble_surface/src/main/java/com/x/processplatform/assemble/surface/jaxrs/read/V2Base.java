...
abstract class V2Base extends StandardJaxrsAction {
	...
	public static abstract class AbstractWo extends Read {
	...
	protected Predicate toFilterPredicate(EffectivePerson effectivePerson, Business business, FilterWi wi)
		...
		if (StringUtils.isNotEmpty(wi.getTitle())) {
			...
			if (StringUtils.isNotEmpty(title)) {
				p = cb.and(cb.like(root.get(Read_.title), "%" + title + "%"));
			}
		}
		return p;
	}
	...
	private <O extends AbstractWo> void relateReadCompleted(Business business, List<O> wos, List<String> jobs)
			throws Exception {
	...
	public <O extends AbstractWo, W extends RelateFilterWi> void relate(Business business, List<O> wos, W wi)
			throws Exception {
		if (!wi.isEmptyRelate()) {
			List<String> jobs = ListTools.extractProperty(wos, Read.job_FIELDNAME, String.class, true, true);
