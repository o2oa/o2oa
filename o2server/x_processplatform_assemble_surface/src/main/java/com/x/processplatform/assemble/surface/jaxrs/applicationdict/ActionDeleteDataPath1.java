...
class ActionDeleteDataPath1 extends BaseAction {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ActionDeleteDataPath1.class);

	ActionResult<Wo> execute(EffectivePerson effectivePerson, String applicationDictFlag, String applicationFlag,
			String path0, String path1) throws Exception {
		...
		Wo wo = ThisApplication.context().applications().deleteQuery(x_processplatform_service_processing.class,
				Applications.joinQueryUri("applicationdict", id, path0, path1, "data"), id).getData(Wo.class);
		result.setData(wo);
		return result;
	}

	@Schema(name = "com.x.processplatform.assemble.surface.jaxrs.applicationdict.ActionDeleteDataPath1$Wo")
	public static class Wo extends WoId {

		private static final long serialVersionUID = -347289598468490285L;
