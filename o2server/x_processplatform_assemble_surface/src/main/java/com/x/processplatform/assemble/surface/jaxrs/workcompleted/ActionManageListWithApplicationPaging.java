...
class ActionManageListWithApplicationPaging extends BaseAction {
	...
	private List<Wo> list(EffectivePerson effectivePerson, Integer page, Integer size, WorkCriteria workCriteria,
		...
		subQuery.where(reviewCriteria.cb.and(reviewCriteria.cb.equal(root.get(Review_.permissionWrite), true),
				reviewCriteria.cb.equal(root.get(Review_.job), workCriteria.root.get(WorkCompleted_.job)),
		...
		if (controllableProcesses.isEmpty()) {
			...
			p = workCriteria.cb.and(p, (workCriteria.cb.or(workCriteria.cb.exists(subQuery),
					workCriteria.root.get(WorkCompleted_.process).in(controllableProcesses))));
		}
		workCriteria.cq.multiselect(selections).where(p)
				.orderBy(workCriteria.cb.desc(workCriteria.root.get(WorkCompleted.startTime_FIELDNAME)));
	...
	private Long count(EffectivePerson effectivePerson, CountCriteria countCriteria, ReviewCriteria reviewCriteria,
		...
		subQuery.select(root.get(Review_.job));
		subQuery.where(reviewCriteria.cb.and(reviewCriteria.cb.equal(root.get(Review_.permissionWrite), true),
				countCriteria.cb.equal(root.get(Review_.job), countCriteria.root.get(WorkCompleted_.job)),
		...
		if (!controllableProcesses.isEmpty()) {
			p = countCriteria.cb.and(p, (countCriteria.cb.or(countCriteria.cb.exists(subQuery),
					countCriteria.root.get(WorkCompleted_.process).in(controllableProcesses))));
		} else {
	...
	private void fetch(Business business, Integer page, Integer size, Predicate p, WorkCriteria workCriteria,
		...
		if (!controllableProcesses.isEmpty()) {
			p = workCriteria.cb.and(p, workCriteria.root.get(WorkCompleted_.process).in(controllableProcesses));
		}
		List<Wo> wos = business.entityManagerContainer().fetchDescPaging(WorkCompleted.class, Wo.copier, p, page, size,
				WorkCompleted.startTime_FIELDNAME);
		result.setData(wos);
		result.setCount(business.entityManagerContainer().count(WorkCompleted.class, p));
	}
	...
	private Predicate predicateTitle(Wi wi, Predicate p, WorkCriteria workCriteria) {
		if (StringUtils.isNotEmpty(wi.getTitle())) {
			...
			p = workCriteria.cb.and(p, workCriteria.cb.like(workCriteria.root.get(WorkCompleted_.title),
					"%" + title + "%", StringTools.SQL_ESCAPE_CHAR));
		}
		return p;
	}

	private Predicate predicateApplication(Application application, WorkCriteria workCriteria) {
		return workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.application), application.getId());
	}
	...
	private Predicate predicateCreatorPersonCreatorUnitActivityName(Business business, Wi wi, Predicate p,
		...
		if (ListTools.isNotEmpty(wi.getCredentialList())) {
			p = workCriteria.cb.and(p, workCriteria.root.get(WorkCompleted_.creatorPerson)
					.in(business.organization().person().list(wi.getCredentialList())));
		}
		if (ListTools.isNotEmpty(wi.getCreatorUnitList())) {
			p = workCriteria.cb.and(p, workCriteria.root.get(WorkCompleted_.creatorUnit).in(wi.getCreatorUnitList()));
		}
		if (ListTools.isNotEmpty(wi.getActivityNameList())) {
			p = workCriteria.cb.and(p, workCriteria.root.get(WorkCompleted_.activityName).in(wi.getActivityNameList()));
		}
		return p;
	}
	...
	private Predicate predicateStartTimeEndTime(Wi wi, Predicate p, WorkCriteria workCriteria) throws Exception {
		if (BooleanUtils.isTrue(DateTools.isDateTimeOrDate(wi.getStartTime()))) {
			p = workCriteria.cb.and(p, workCriteria.cb.greaterThan(workCriteria.root.get(WorkCompleted_.startTime),
					DateTools.parse(wi.getStartTime())));
		}
		if (BooleanUtils.isTrue(DateTools.isDateTimeOrDate(wi.getEndTime()))) {
			p = workCriteria.cb.and(p, workCriteria.cb.lessThan(workCriteria.root.get(WorkCompleted_.startTime),
					DateTools.parse(wi.getEndTime())));
		}
		return p;
	}
	...
	private Predicate predicateWorkJob(Wi wi, Predicate p, WorkCriteria workCriteria) {
		if (ListTools.isNotEmpty(wi.getWorkList())) {
			p = workCriteria.cb.and(p, workCriteria.root.get(WorkCompleted_.id).in(wi.getWorkList()));
		}
		if (ListTools.isNotEmpty(wi.getJobList())) {
			p = workCriteria.cb.and(p, workCriteria.root.get(WorkCompleted_.job).in(wi.getJobList()));
		}
		return p;
	}
...
//					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.workThroughManual), wi.getWorkThroughManual()));
//		}
//		if (StringUtils.isNotBlank(wi.getWorkCreateType())) {
//			p = workCriteria.cb.and(p,
//					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.workCreateType), wi.getWorkCreateType()));
//		}
//		if (StringUtils.isNotBlank(wi.getWorkStatus())) {
//			if (wi.getWorkStatus().equalsIgnoreCase(WorkStatus.start.name())) {
//				p = workCriteria.cb.and(p,
//						workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.workStatus), WorkStatus.start));
//			} else if (wi.getWorkStatus().equalsIgnoreCase(WorkStatus.processing.name())) {
//				p = workCriteria.cb.and(p,
//						workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.workStatus), WorkStatus.processing));
//			} else if (wi.getWorkStatus().equalsIgnoreCase(WorkStatus.hanging.name())) {
//				p = workCriteria.cb.and(p,
//						workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.workStatus), WorkStatus.hanging));
//			}
//		}
//		return p;
//	}
	...
	private Predicate predicateStringValue(Wi wi, Predicate p, WorkCriteria workCriteria) {
		if (StringUtils.isNotBlank(wi.getStringValue01())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue01), wi.getStringValue01()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue02())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue02), wi.getStringValue02()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue03())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue03), wi.getStringValue03()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue04())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue04), wi.getStringValue04()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue05())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue05), wi.getStringValue05()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue06())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue06), wi.getStringValue06()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue07())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue07), wi.getStringValue07()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue08())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue08), wi.getStringValue08()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue09())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue09), wi.getStringValue09()));
		}
		if (StringUtils.isNotBlank(wi.getStringValue10())) {
			p = workCriteria.cb.and(p,
					workCriteria.cb.equal(workCriteria.root.get(WorkCompleted_.stringValue10), wi.getStringValue10()));
		}
		return p;
	}
	...
	private WorkCriteria initWorkCriteria(Business business) throws Exception {
		...
		criteria.em = business.entityManagerContainer().get(WorkCompleted.class);
		criteria.cb = criteria.em.getCriteriaBuilder();
		criteria.cq = criteria.cb.createQuery(Tuple.class);
		criteria.root = criteria.cq.from(WorkCompleted.class);
		return criteria;
	}
	...
	private class WorkCriteria {
		...
		private Root<WorkCompleted> root;
	}
