...
public class ProcessFactory extends ElementFactory {
    ...
    public List<Process> listProcessEditionObject(String application, String edition)
            throws Exception {
    ...
    public Process pickProcessEditionEnabled(Application application, String flag)
        ...
        if (optional.isPresent()) {
            ...
            p = this.entityManagerContainer().restrictFlagEqualAndNotEqual(flag, Process.class,
                    Process.application_FIELDNAME, application.getId(),
                    Process.editionEnable_FIELDNAME, false);
            if (p != null) {
    ...
    public List<String> listStartableWithApplication(EffectivePerson effectivePerson,
        ...
        if (BooleanUtils
                .isNotTrue(this.business()
                        .ifPersonCanManageApplicationOrProcess(effectivePerson, application,
                                null))) {
        ...
        if (StringUtils.isNotEmpty(terminal)) {
            p = cb.and(p, cb.or(cb.equal(root.get(Process_.startableTerminal),
    ...
    public List<String> listControllableProcess(EffectivePerson effectivePerson,
            Application application)
            throws Exception {
        List<String> ids = this.listWithApplication(application);
        boolean isManager = this.business()
                .ifPersonCanManageApplicationOrProcess(effectivePerson, application, null);
        ...
        for (String str : ids) {
            ...
            if (flag) {
                list.add(str);
            } else if (ListTools.isNotEmpty(o.getControllerList()) && ListTools.containsAny(
                    this.business().organization().person()
                            .getAuthInfo(effectivePerson.getDistinguishedName()),
                    o.getControllerList())) {
                list.add(str);
            }
        }
    ...
    public <T extends Process> List<T> sort(List<T> list) {
        list = list.stream()
                .sorted(Comparator.comparing(Process::getAlias,
    ...
    public List<Process> listObjectWithProcess(List<String> processList, boolean includeEdition)
            throws Exception {
        EntityManager em = this.entityManagerContainer().get(Process.class);
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Process> cq = cb.createQuery(Process.class);
        ...
        if (includeEdition) {
            ...
            subP = cb.and(subP,
                    cb.equal(root.get(Process_.edition), subRoot.get(Process_.edition)));
