...
public class ActionListNextFilter extends BaseAction {
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson, String id, Integer count, JsonElement jsonElement)
		...
		if (ListTools.isNotEmpty(wi.getApplicationList())) {
			ins.put(Review.application_FIELDNAME, wi.getApplicationList());
		}
		if (ListTools.isNotEmpty(wi.getProcessList())) {
			ins.put(Review.process_FIELDNAME, wi.getProcessList());
		}
		if (ListTools.isNotEmpty(wi.getCreatorUnitList())) {
			ins.put(Review.creatorUnit_FIELDNAME, wi.getCreatorUnitList());
		}
		if (ListTools.isNotEmpty(wi.getStartTimeMonthList())) {
			ins.put(Review.startTimeMonth_FIELDNAME, wi.getStartTimeMonthList());
		}
		if (ListTools.isNotEmpty(wi.getCompletedTimeMonthList())) {
			ins.put(Review.completedTimeMonth_FIELDNAME, wi.getCompletedTimeMonthList());
		}
		if (StringUtils.isNotEmpty(wi.getKey())) {
			String key = StringUtils.trim(StringUtils.replace(wi.getKey(), "\u3000", " "));
			if (StringUtils.isNotEmpty(key)) {
				likes.put(Review.title_FIELDNAME, key);
				likes.put(Review.serial_FIELDNAME, key);
				likes.put(Review.creatorPerson_FIELDNAME, key);
				likes.put(Review.creatorUnit_FIELDNAME, key);
			}
		}
