...
public class ActionFilterAttribute extends BaseAction {
	...
	public static class Wo extends GsonPropertyObject {
		...
		@FieldDescribe("可选开始时间范围")
		private List<NameValueCountPair> startTimeMonthList = new ArrayList<NameValueCountPair>();
		@FieldDescribe("可选结束时间范围")
		private List<NameValueCountPair> completedTimeMonthList = new ArrayList<NameValueCountPair>();
	...
	private List<NameValueCountPair> listCreatorUnitPair(Business business, EffectivePerson effectivePerson)
		...
		Root<Review> root = cq.from(Review.class);
		Predicate p = cb.equal(root.get(Review_.person), effectivePerson.getDistinguishedName());
		cq.select(root.get(Review_.creatorUnit)).where(p).distinct(true);
		List<String> os = em.createQuery(cq).getResultList();
		List<NameValueCountPair> wos = new ArrayList<>();
		for (String str : os) {
			if (StringUtils.isNotEmpty(str)) {
	...
	private List<NameValueCountPair> listCompletedTimeMonthPair(Business business, EffectivePerson effectivePerson)
		...
		Root<Review> root = cq.from(Review.class);
		Predicate p = cb.equal(root.get(Review_.person), effectivePerson.getDistinguishedName());
		cq.select(root.get(Review_.completedTimeMonth)).where(p).distinct(true);
	...
	private List<NameValueCountPair> listStartTimeMonthPair(Business business, EffectivePerson effectivePerson)
		...
		Root<Review> root = cq.from(Review.class);
		Predicate p = cb.equal(root.get(Review_.person), effectivePerson.getDistinguishedName());
		cq.select(root.get(Review_.startTimeMonth)).where(p).distinct(true);
		List<String> list = em.createQuery(cq).getResultList();
		List<NameValueCountPair> wos = new ArrayList<>();
		for (String str : list) {
			if (StringUtils.isNotEmpty(str)) {
