	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, JsonElement jsonElement) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			List<Triple<String, String, String>> targets = this.readTarget(effectivePerson, business,
					wi.getTargetList());
List<Correlation> exists = this.exists(business, wi.getFromType(), wi.getFromBundle());
	...
	private Map<String, Correlation> exists(Business business, String fromType, String fromBundle) throws Exception {
		return business.entityManagerContainer().listEqualAndEqual(Correlation.class, Correlation.FROMTYPE_FIELDNAME,
				fromType, Correlation.FROMTYPE_FIELDNAME, fromBundle).stream();
	}
