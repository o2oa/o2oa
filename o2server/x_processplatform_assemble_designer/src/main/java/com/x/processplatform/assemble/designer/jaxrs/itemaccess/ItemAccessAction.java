...
public class ItemAccessAction extends StandardJaxrsAction {
	...
	@JaxrsMethodDescribe(value = "获取流程内容.含所有节点和路由信息", action = ActionGetWithProcessWithPath.class)
	@GET
	...
	@JaxrsMethodDescribe(value = "保存访问控制字段，基于流程ID和字段路径查找存在则更新，不存在则新增.", action = ActionSave.class)
	@POST
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	...
	@JaxrsMethodDescribe(value = "基于字段路径列示访问控制字段", action = ActionListWithPath.class)
	@GET
	@Path("path/{path}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	...
	@JaxrsMethodDescribe(value = "基于流程列示访问控制字段", action = ActionListWithProcess.class)
	@GET
