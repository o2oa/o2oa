...
public class TemplateFormAction extends StandardJaxrsAction {
	...
	@JaxrsMethodDescribe(value = "列示所有分类名称以及分类数量.", action = ActionListCategory.class)
	@GET
	@Path("list/category")
	...
	@JaxrsMethodDescribe(value = "按分类名称列示模版表单.", action = ActionListWithCategory.class)
	@PUT
	@Path("list/category")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
