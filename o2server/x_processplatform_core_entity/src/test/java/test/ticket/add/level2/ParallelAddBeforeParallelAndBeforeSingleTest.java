...
class ParallelAddBeforeParallelAndBeforeSingleTest {

	@DisplayName("B前加签EFG,FEG并行处理,F前加签IJK,IJK单人处理,即并行-前加签并行-前加签单人,混合处理,IJK任意处理需到F,EFG处理完需到B")
	@Test
	@Order(1)
	void test01() {
	...
	@DisplayName("B前加签EFG,FEG并行处理,F前加签IJK,IJK单人处理,即并行-前加签并行-前加签单人,AC先处理,IJK任意处理需到F,EFG处理完需到B")
	@Test
	@Order(2)
	void test02() {
		List<Ticket> p1 = Arrays.asList("A${LA}", "B${LB}", "C${LC}").stream().map(Ticket::new).collect(Collectors.toList());
		...
		tickets.completed("LI");
		value = tickets.bubble().stream().<String>map(Ticket::distinguishedName).sorted().collect(Collectors.joining(","));
		Assertions.assertEquals("E,F", value);
		tickets.completed("LF");
		value = tickets.bubble().stream().<String>map(Ticket::distinguishedName).sorted().collect(Collectors.joining(","));
	...
	@DisplayName("B前加签EFG,FEG并行处理,F前加签IJK,IJK单人处理,即并行-前加签并行-前加签单人,IJK任意处理到F,EFG处理完到ABC")
	@Test
	@Order(3)
	void test03() {
		List<Ticket> p1 = Arrays.asList("A${LA}", "B${LB}", "C${LC}").stream().map(Ticket::new).collect(Collectors.toList());
