...
class SingleAddTest2 {
	...
	void test01() {
		...
		value = tickets.bubble().stream().<String>map(Ticket::distinguishedName).collect(Collectors.joining(","));
		Assertions.assertEquals("a", value);
		opt = tickets.findTicketWithLabel("a1");
		tickets.add(opt.get(), Arrays.asList("b${b2}", "c${c2}", "d${d2}"), false, Tickets.MODE_SINGLE);
		value = tickets.bubble().stream().<String>map(Ticket::distinguishedName).collect(Collectors.joining(","));
		Assertions.assertEquals("b,c,d", value);
	}
