...
class ParallelAddAfterQueueAndBeforeParallelTest {

	@DisplayName("B后加签EFG,EFG串行处理，E前加签IJK，IJK并行处理,即并行-后加签串行-前加签并行,IJK先处理到ACE,E处理到F,F处理到G")
	@Test
	@Order(1)
	void test01() {
	...
	@DisplayName("B后加签EFG,EFG串行处理，E前加签IJK，IJK并行处理,即并行-后加签串行-前加签并行,AC先处理到IJK,IJK处理到E,E处理到F,F处理到G")
	@Test
	@Order(2)
	void test02() {
		List<Ticket> p1 = Arrays.asList("A${LA}", "B${LB}", "C${LC}").stream().map(Ticket::new).collect(Collectors.toList());
	...
	@DisplayName("B后加签EFG,EFG串行处理，E前加签IJK，IJK并行处理,即并行-后加签串行-前加签并行,混合处理,IJK处理到E,E处理到F,F处理到G")
	@Test
	@Order(3)
	void test03() {
		List<Ticket> p1 = Arrays.asList("A${LA}", "B${LB}", "C${LC}").stream().map(Ticket::new).collect(Collectors.toList());
		...
		tickets.completed("LJ");
		value = tickets.bubble().stream().<String>map(Ticket::distinguishedName).sorted().collect(Collectors.joining(","));
		Assertions.assertEquals("C,I,K", value);
		tickets.completed("LK");
		value = tickets.bubble().stream().<String>map(Ticket::distinguishedName).sorted().collect(Collectors.joining(","));
