...
class ActionInfo extends BaseAction {
	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, JsonElement jsonElement) throws Exception {
		...
		String nonceStr = Config.zhengwuDingding().getNonce();
		long timeStamp = System.currentTimeMillis() / 1000;
		String jsticket = Config.zhengwuDingding().getJsapiTicket();
		String signature = this.getSignature(effectivePerson, jsticket, nonceStr, timeStamp, wi.getUrl());
		String agentid = Config.zhengwuDingding().getAgentId();
		Wo wo = new Wo();
		wo.setJsticket(jsticket);
		wo.setAgentid(agentid);
		wo.setCorpId(Config.zhengwuDingding().getCorpId());
