...
class ActionCodeLogin extends BaseAction {
    ...
    ActionResult<Wo> execute(HttpServletRequest request, HttpServletResponse response,
        ...
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ...
            if (BooleanUtils.isFalse(Config.person().getTwoFactorLogin()) && BooleanUtils.isFalse(
                    Config.person().getCodeLogin())) {
    ...
    private Person validatePerson(EntityManagerContainer emc, Business business, String credential,
            String codeAnswer)
            throws Exception {
        // 普通用户登录,也有可能拥有管理员角色
        ...
        if (BooleanUtils.isTrue(Config.person().getSuperPermission())
            ...
            if (BooleanUtils.isNotTrue(
                    business.instrument().code().validateCascade(o.getMobile(), codeAnswer))) {
                throw new ExceptionPersonNotExistOrInvalidPassword();
            }
        }
        return o;
