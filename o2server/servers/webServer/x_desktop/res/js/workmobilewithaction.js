        ...
        MWF.require("MWF.xDesktop.Layout", function(){
            ...
            (function(){
                ...
                layout.loadWork = function(options){
                    ...
                    if (method && id){
                        this.action[method](id, function(json){
                            if (json.data.workCompleted){
                                workMethod = "getJobByWorkCompletedMobile";
                                workid = json.data.workCompleted
                            }else{
                                workMethod = "getJobByWorkMobile";
                    ...
                    if (workMethod && workid){
                        this.action[workMethod](function(json){
                            ...
                            if( this.control.allowSave || this.control.allowProcessing ){
                                $("MWF_form_action_save").getParent().setStyle("display","");
                            }else{
                                $("MWF_form_action_save").getParent().getPrevious().setStyle("height","100%");
                            }
                            if( this.control.allowSave ){
                                var action = $("MWF_form_action_save");
                                ...
                                $("MWF_form_action_save").setStyle("display","none")
                            }
                            if( this.control.allowProcessing ){
                                var action = $("MWF_form_action_process");
                                action.setStyle("display","");
                                action.addEvents({
                                    "click": function(){
                                ...
                                $("MWF_form_action_process").setStyle("display","none")
                            }
                    ...
                    //
                    // if (options.taskid){
                    //     method = "getJobByTaskMobile";
                    //     id = options.taskid;
                    // }else if (options.workid) {
                    //     method = "getJobByWorkMobile";
                    //     id = options.workid;
                    // }else if (options.workCompletedId) {
                    //     method = "getJobByWorkCompletedMobile";
                ...
                layout.addEvent = window.addEvent.bind(window);
                layout.addEvents = window.addEvents.bind(window);

                layout.errorWork = function(xhr){
                    if (this.mask) this.mask.hide();
                    var json = JSON.decode(xhr.responseText);
                    if (json.prompt.indexOf("ExceptionWorkAccessDenied")!=-1){
                        if (this.authentication) this.authentication.loadLogin(this.node);
                    }else{
                ...
                layout.openWork = function(){
                    if (this.form){
                        MWF.xDesktop.requireApp("process.Xform", "Form", function(){
                            this.appForm = new MWF.APPForm(this.node, this.form, {"readonly": (this.currentTask) ? false : true});
                            this.appForm.businessData = {
                                ...
                                "status": {
                                    "readonly": (this.currentTask) ? false : true
                                }
                            };
