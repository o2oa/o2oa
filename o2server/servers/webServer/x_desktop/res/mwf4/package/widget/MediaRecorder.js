...
MWF.widget.MediaRecorder = MWF.MediaRecorder = new Class({
	Implements: [Options, Events],
    Extends: MWF.widget.Common,
	options: {
		"style": "default",
		"path": MWF.defaultPath+"/widget/$MediaRecorder/",
		"videoHeight" : "232px",
		"constraints" : {
			audio: true,
			video: true
	...
	initialize: function(node, options){
		...
		this.path = this.options.path || (MWF.defaultPath+"/widget/$MediaRecorder/");
	...
	load: function(){
		...
		this.loadResource( function(){
			//this.checkDevice( function(){
				this._load();
			//	this.uuidInstance = new MWF.widget.UUID();
			//	this.uuid = this.uuidInstance.valueOf();
			//	MWF.widget.MediaRecorder.instanceObject[ this.uuid ] = this;
			//}.bind(this));
		}.bind(this))
	},
	//checkDevice : function( callback ){
	//	var flag;
	//	if( MWF.MediaRecorder.hasActiveInstance() ){
	//		MWF.xDesktop.confirm("warn", null, "设备已经被占用", "设备已经被占用,是否释放被占用的设备？", 350, 120, function () {
	//			MWF.MediaRecorder.stopActiveInstance();
	//			this.close();
	//			callback();
	//			flag = true;
	//		}, function () {
	...
	checkBroswer : function(){
		...
		var unavailableFeatures = MWF.MediaRecorder.getUnavailableBrowerFeatures();
...
MWF.MediaRecorder.getUnavailableBrowerFeatures = function(){
...
MWF.MediaRecorder.isAvailable = function(){
	// window.isSecureContext could be used for Chrome
	var isSecureOrigin = location.protocol === 'https:' ||
		location.hostname === 'localhost';
	if (!isSecureOrigin) {
	...
	if( MWF.MediaRecorder.getUnavailableBrowerFeatures().length > 0 ){
		return false;
	}
	return true;
};

//MWF.MediaRecorder.instanceObject = {};
//
//MWF.MediaRecorder.hasActiveInstance = function(){
//	for( var id in MWF.MediaRecorder.instanceObject ){
//		var instance = MWF.MediaRecorder.instanceObject[id];
...
//};
//
//MWF.MediaRecorder.stopActiveInstance = function(){
//	for( var id in MWF.MediaRecorder.instanceObject ){
//		var instance = MWF.MediaRecorder.instanceObject[id];
//		instance.stopDevice();
//	}
