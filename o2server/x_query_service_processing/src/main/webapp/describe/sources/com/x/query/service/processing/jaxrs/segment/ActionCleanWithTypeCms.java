...
class ActionCleanWithTypeCms extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(ActionCleanWithTypeCms.class);
	...
	public class Job extends Thread {
		...
		public void run() {
			try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
				List<String> ids = emc.idsEqual(Entry.class, Entry.type_FIELDNAME, Entry.TYPE_CMS);
				int batch = 1;
				for (List<String> list : ListTools.batch(ids, batchSize)) {
					logger.debug("开始清空分词数据, 数据类型: {}, 数据数量: {},  批次大小: {}, 批次: {}.", Entry.TYPE_CMS, ids.size(),
							batchSize, batch++);
