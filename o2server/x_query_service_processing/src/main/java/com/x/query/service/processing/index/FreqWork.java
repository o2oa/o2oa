...
public abstract class FreqWork extends AbstractJob {

    private static final Logger LOGGER = LoggerFactory.getLogger(FreqWork.class);
        ...
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ...
            if (StringUtils.isNotEmpty(state.getLatestId())
                ...
                p = cb.and(p, cb.not(cb.and(cb.equal(root.get(JpaObject_.updateTime), state.getLatestUpdateTime()),
                        cb.equal(root.get(Work_.id), state.getLatestId()))));
            } else {
                p = cb.conjunction();
            }
    ...
    protected void index(List<String> ids) {
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ...
            List<Doc> list = ids.stream().map(o -> Pair.of(business, o)).map(DocFunction.wrapWork)
