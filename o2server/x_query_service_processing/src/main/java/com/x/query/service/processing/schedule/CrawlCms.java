...
public class CrawlCms extends Crawl {
	...
	public void schedule(JobExecutionContext jobExecutionContext) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			List<String> ids = business.entityManagerContainer().ids(Document.class);
			...
			emc.flush();
			this.crawl(business);
			logger.print("内容管理索引器条目总数:{}, 内容总数:{}, 需要新增:{}, 需要删除:{}, 更新数量:{}, 数量限制:{}, 耗时{}.", entry_references.size(),
					ids.size(), add_references.size(), remove_references.size(), update_references.size(),
					Config.query().getCrawlCms().getCount(), stamp.consumingMilliseconds());
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e);
	...
	private void remove(Business business, List<String> references) throws Exception {
		...
		for (String reference : references) {
			...
			for (Entry en : emc.listEqualAndEqual(Entry.class, Entry.type_FIELDNAME, Entry.TYPE_CMS,
	...
	private void update(Business business, List<String> references) throws Exception {
		...
		for (List<String> list : ListTools.batch(references, BATCH_SIZE)) {
			emc.beginTransaction(Entry.class);
			for (Entry o : emc.listEqualAndIn(Entry.class, Entry.type_FIELDNAME, Entry.TYPE_CMS,
	...
	private void add(Business business, List<String> references) throws Exception {
		EntityManagerContainer emc = business.entityManagerContainer();
		if (references.size() > Config.query().getCrawlCms().getCount()) {
			references = references.subList(0, Config.query().getCrawlCms().getCount());
		}
		for (List<String> list : ListTools.batch(references, 500)) {
			emc.beginTransaction(Entry.class);
			for (Document document : emc.list(Document.class, list)) {
				Entry entry = new Entry();
				entry.setLastUpdateTime(document.getUpdateTime());
	...
	private void crawl(Business business) throws Exception {
		...
		for (Entry entry : this.listWait(business)) {
			Document document = emc.find(entry.getReference(), Document.class);
			if (null != document) {
				try {
					try {
						logger.debug("正在处理:{}.", document.getTitle());
						...
						String body = converter.text(emc.listEqualAndEqual(Item.class, Item.itemCategory_FIELDNAME,
								ItemCategory.cms, Item.bundle_FIELDNAME, document.getId()), true, true, true, true,
								true, ",");
						String attachment = this.attachmentToText(business, document);
						...
						throw new ExceptionCrawlCms(e, document);
					}
				} catch (Exception e) {
					logger.error(e);
				}
	...
	private List<Entry> listWait(Business business) throws Exception {
		...
		p = cb.and(p, cb.equal(root.get(Entry_.type), Entry.TYPE_CMS));
		cq.select(root).where(p);
		List<Entry> os = em.createQuery(cq).setMaxResults(Config.query().getCrawlCms().getCount()).getResultList();
		return os;
	}
	...
	private List<String> listOldToUpdateReference(Business business) throws Exception {
		...
		List<Entry> os = em.createQuery(cq).setMaxResults(Config.query().getCrawlCms().getCount() / 7).getResultList();
	...
	private String attachmentToText(Business business, Document document) throws Exception {
		StringBuffer buffer = new StringBuffer();
		EntityManagerContainer emc = business.entityManagerContainer();
		for (FileInfo o : emc.listEqual(FileInfo.class, FileInfo.documentId_FIELDNAME, document.getId())) {
