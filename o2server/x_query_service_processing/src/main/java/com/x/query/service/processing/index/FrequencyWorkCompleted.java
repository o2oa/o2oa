...
public abstract class FrequencyWorkCompleted extends AbstractJob {

    private static final Logger LOGGER = LoggerFactory.getLogger(FrequencyWorkCompleted.class);
    ...
    protected List<Pair<String, String>> list(State state, int size) throws Exception {
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ...
            if (StringUtils.isNotEmpty(state.getLatestUpdateId())
                ...
                p = cb.notEqual(root.get(WorkCompleted_.id), state.getLatestUpdateId());
    ...
    protected State getState(String frequency) throws Exception {
        ...
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            state = emc.firstEqualAndEqualAndEqual(State.class, State.NODE_FIELDNAME, Config.node(),
                    State.FREQUENCY_FIELDNAME, frequency, State.TYPE_FIELDNAME,
