...
public class CrawlWorkCompleted extends Crawl {
	...
	public void schedule(JobExecutionContext jobExecutionContext) throws Exception {
		...
		Long workCompletedTotal = null;
		Long entryTotal = null;
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			Business business = new Business(emc);
			workCompletedTotal = emc.count(WorkCompleted.class);
			entryTotal = emc.countEqual(Entry.class, Entry.type_FIELDNAME, Entry.TYPE_WORKCOMPLETED);
			...
			updates = ListUtils.union(ListUtils.union(add_workCompleteds, update_workCompleteds), update_references);
		} catch (Exception e) {
			logger.error(e);
		...
		logger.print(
				...
				workCompletedTotal, add_workCompleteds.size(), update_workCompleteds.size(), entryTotal,
		...
		for (String reference : references) {
			try {
				try {
					...
					throw new ExceptionCrawlWorkCompleted(e, reference);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private List<String> listUpdateEntryReference(Business business) throws Exception {
		...
		cq.select(root.get(Entry_.reference)).where(p).orderBy(cb.asc(root.get(WorkCompleted_.sequence)));
		Integer count = Config.query().getCrawlWorkCompleted().getCount() / 7;
		List<String> os = em.createQuery(cq).setMaxResults(count).getResultList();
		return os;
	}
