...
public class DocFunction {
	...
	public static final Function<Pair<Business, String>, Pair<String, Optional<Doc>>> wrapDocument = param -> {
		try {
			...
			CategoryInfo categoryInfo = param.first().entityManagerContainer().find(document.getCategoryId(),
					CategoryInfo.class);
			if ((null != document) && (!StringUtils.equals(document.getDocumentType(), Document.DOCUMENT_TYPE_DATA))
					&& (null == categoryInfo || BooleanUtils.isNotFalse(categoryInfo.getIndexAble()))) {
				...
				doc.addString(Indexs.PREFIX_FIELD_CMS_STRING, Document.appId_FIELDNAME, document.getAppId());
				doc.addString(Indexs.PREFIX_FIELD_CMS_STRING, Document.appName_FIELDNAME, document.getAppName());
	...
	private static List<String> readers(Business business, Work work) throws Exception {
		List<String> list = business.entityManagerContainer()
				.fetchEqualAndEqual(Review.class, PROCESSPLATFORM_REVIEW_FIELDS, Review.job_FIELDNAME, work.getJob(),
	...
	private static void workAddTaskPersonNames(Doc doc, Business business, Work work) throws Exception {
		...
		doc.addStrings(Indexs.PREFIX_FIELD_PROCESSPLATFORM_STRINGS, Indexs.FIELD_ROCESSPLATFORM_TASKPERSONNAMES,
				list.stream().map(Task::getPerson).filter(StringUtils::isNotBlank).map(OrganizationDefinition::name)
	...
	public static void updatePrimitive(Doc doc, JsonPrimitive jsonPrimitive, String name, Integer dataStringThreshold) {
	...
	private static String storageObjectToText(Tika tika, StorageObject storageObject) throws Exception {
		...
		try {
			...
			LOGGER.warn("error extract attachment text, storageObject:{}, name:{}, exception class:{}, message:{}.",
					storageObject.getId(), storageObject.getName(), th.getClass().getName(), th.getMessage());
		}
