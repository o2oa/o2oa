...
public class HighFrequencyWorkCompleted extends FrequencyWorkCompleted {

    private static final Logger LOGGER = LoggerFactory.getLogger(HighFrequencyWorkCompleted.class);

    @Override
    public void schedule(JobExecutionContext jobExecutionContext) throws Exception {
        if (BooleanUtils.isNotTrue(Config.query().index().getHighFrequencyWorkCompletedEnable())) {
            return;
        }
        ...
        int count = 0;
        do {
            list = this.list(state, Config.query().index().getHighFrequencyWorkCompletedMaxCount());
            if (!list.isEmpty()) {
                ...
                && ((new Date().getTime() - startAt.getTime()) < Config.query().index()
                        .getHighFrequencyWorkCompletedMaxMinutes() * 1000 * 60)
                && count < Config.query().index().getHighFrequencyWorkCompletedMaxCount());
