...
public abstract class FreqDocument extends AbstractJob {

    private static final Logger LOGGER = LoggerFactory.getLogger(FreqDocument.class);
        ...
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ...
            if (StringUtils.isNotEmpty(state.getLatestId())
                ...
                p = cb.and(p, cb.not(cb.and(cb.equal(root.get(JpaObject_.updateTime), state.getLatestUpdateTime()),
                        cb.equal(root.get(Document_.id), state.getLatestId()))));
            } else {
                p = cb.conjunction();
            }
    ...
    protected State getState(String freq) throws Exception {
        ...
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            state = emc.firstEqualAndEqualAndEqual(State.class, State.NODE_FIELDNAME, Config.node(),
                    State.FREQ_FIELDNAME, freq, State.TYPE_FIELDNAME, State.TYPE_DOCUMENT);
            if (null != state) {
