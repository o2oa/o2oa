...
public class CrawlWork extends Crawl {
	...
	public void schedule(JobExecutionContext jobExecutionContext) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			List<String> ids = business.entityManagerContainer().ids(Work.class);
			...
			emc.flush();
			this.crawl(business);
			logger.print("工作索引器条目总数:{}, 工作总数:{}, 需要新增:{}, 需要删除:{}, 更新数量:{}, 数量限制:{}, 耗时{}.", entry_references.size(),
	...
	private void remove(Business business, List<String> references) throws Exception {
		...
		for (String reference : references) {
			...
			for (Entry en : emc.listEqualAndEqual(Entry.class, Entry.type_FIELDNAME, Entry.TYPE_WORK,
	...
	private void update(Business business, List<String> references) throws Exception {
		...
		for (List<String> list : ListTools.batch(references, BATCH_SIZE)) {
			emc.beginTransaction(Entry.class);
			for (Entry o : emc.listEqualAndIn(Entry.class, Entry.type_FIELDNAME, Entry.TYPE_WORK,
	...
	private void add(Business business, List<String> references) throws Exception {
		EntityManagerContainer emc = business.entityManagerContainer();
		if (references.size() > Config.query().getCrawlWork().getCount()) {
			references = references.subList(0, Config.query().getCrawlWork().getCount());
		}
		for (List<String> list : ListTools.batch(references, 500)) {
			emc.beginTransaction(Entry.class);
			for (Work work : emc.list(Work.class, list)) {
				Entry entry = new Entry();
				entry.setLastUpdateTime(work.getUpdateTime());
	...
	private List<Entry> listWait(Business business) throws Exception {
		...
		p = cb.and(p, cb.equal(root.get(Entry_.type), Entry.TYPE_WORK));
		cq.select(root).where(p);
		List<Entry> os = em.createQuery(cq).setMaxResults(Config.query().getCrawlWork().getCount()).getResultList();
		return os;
	}
	...
	private List<String> listOldToUpdateReference(Business business) throws Exception {
		...
		List<Entry> os = em.createQuery(cq).setMaxResults(Config.query().getCrawlWork().getCount() / 7).getResultList();
	...
	private String attachmentToText(Business business, Work work) throws Exception {
