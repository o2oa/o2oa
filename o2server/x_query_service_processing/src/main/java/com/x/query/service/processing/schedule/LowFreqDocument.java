...
public class LowFreqDocument extends LowFreq {
    ...
    public void schedule(JobExecutionContext jobExecutionContext) throws Exception {
        ...
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ...
            state = business.state().getState(Config.query().index().getMode(), Indexs.TYPE_DOCUMENT, State.FREQ_LOW,
                    Config.node());
        }
        List<Pair<String, Date>> list;
        int count = 0;
        do {
            list = this.list(state, Document.class, Config.query().index().getLowFreqDocumentBatchSize());
            if (!list.isEmpty()) {
                ...
                pairs.stream().collect(Collectors.groupingBy(Pair::first)).entrySet().stream().forEach(o -> {
                    try {
                        ...
                        ThisApplication.indexWriteQueue.send(new IndexWriteQueue.UpdateMessage(
                                docs, Indexs.CATEGORY_CMS, o.getKey(), true));
                        ThisApplication.indexWriteQueue.send(new IndexWriteQueue.UpdateMessage(
                                docs,
        ...
        LOGGER.info("low freq index document start at:{}, elapsed:{} minutes, count:{}.",
