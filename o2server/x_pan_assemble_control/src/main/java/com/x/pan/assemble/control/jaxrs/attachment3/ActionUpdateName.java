...
class ActionUpdateName extends BaseAction {
	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, String id, JsonElement jsonElement) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if(!wi.getName().equals(attachment.getName())){
				if (business.attachment3().exist(wi.getName(), attachment.getFolder(), FileStatusEnum.VALID.getName())) {
					throw new ExceptionNameExist(wi.getName());
				}
				emc.beginTransaction(Attachment3.class);
				attachment.setName(wi.getName());
	...
	public static class Wi extends Attachment3 {
		...
		static WrapCopier<Wi, Attachment3> copier = WrapCopierFactory.wi(Wi.class, Attachment3.class,
				ListTools.toList(Attachment3.name_FIELDNAME), null);
