...
public class ActionCreate extends BaseAction {
	...
	ActionResult<Wo> execute(EffectivePerson effectivePerson, JsonElement jsonElement) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (share == null) {
				...
				Set<String> set = new HashSet<>(share.getShareUserList());
				if(wi.getShareUserList() != null){
					set.addAll(wi.getShareUserList());
				}
				share.setShareUserList(new ArrayList<>(set));
				set.clear();
				set.addAll(share.getShareOrgList());
				if(wi.getShareOrgList() != null){
					set.addAll(wi.getShareOrgList());
				}
				share.setShareOrgList(new ArrayList<>(set));
				set.clear();
				set.addAll(share.getShareGroupList());
				if(wi.getShareGroupList() != null){
					set.addAll(wi.getShareGroupList());
				}
				share.setShareGroupList(new ArrayList<>(set));
		...
		if (!isExist && !Share.SHARE_TYPE_PASSWORD.equals(wi.getShareType())) {
			CompletableFuture.runAsync(() -> sendSms(business, tempShare),
	...
	private void sendSms(Business business, Share share) {
		try {
