...
abstract class BaseAction extends StandardJaxrsAction {
	...
	protected void verifyConstraint(Business business, String person, long size, String fileName) throws Exception{
		...
		Optional<?> optional = CacheManager.get(business.configCacheCategory, cacheKey);
		FileConfig3 config = null;
		if(optional.isPresent()){
			config = (FileConfig3)optional.get();
		}else{
			config = business.entityManagerContainer().firstEqual(FileConfig3.class, FileConfig3.person_FIELDNAME, Business.SYSTEM_CONFIG);
			if(config != null){
				business.entityManagerContainer().get(FileConfig3.class).detach(config);
			}else{
				config = new FileConfig3();
				config.setProperties(null);
			}
			CacheManager.put(business.configCacheCategory, cacheKey, config);
		}

		cacheKey = new Cache.CacheKey(FileConfig3.class, person, Business.SYSTEM_CONFIG);
		optional = CacheManager.get(business.configCacheCategory, cacheKey);
		FileConfig3 personConfig = null;
		if(optional.isPresent()){
			personConfig = (FileConfig3)optional.get();
		}else{
			personConfig = business.entityManagerContainer().firstEqual(FileConfig3.class, FileConfig3.person_FIELDNAME, person);
			if(personConfig != null){
				business.entityManagerContainer().get(FileConfig3.class).detach(config);
			}else{
				personConfig = config;
			}
			CacheManager.put(business.configCacheCategory, cacheKey, personConfig);
		}

		if (personConfig != null) {
			if (personConfig.getCapacity() != null && personConfig.getCapacity() > 0) {
				long usedCapacity = (business.attachment2().getUseCapacity(person) + size) / (1024 * 1024);
				if (usedCapacity > personConfig.getCapacity()) {
					throw new ExceptionCapacityOut(usedCapacity, personConfig.getCapacity());
				}
			}
		}
		if (config != null){
