...
public class Folder2Action extends StandardJaxrsAction {
	...
	@GET
	@Path("list/top/order/by/{orderBy}/desc/{desc}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void listTop(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request,
		...
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionListTop().execute(effectivePerson, orderBy, desc);
		} catch (Exception e) {
			logger.error(e, effectivePerson, request, null);
			result.error(e);
		}
	...
	@JaxrsMethodDescribe(value = "保存到共享区", action = ActionSaveToZone.class)
	@POST
	@Path("save/to/zone/{zoneFolder}")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void saveToZone(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request,
						   @JaxrsParameterDescribe("共享区目录标志") @PathParam("zoneFolder") String zoneFolder, JsonElement jsonElement) {
		ActionResult<ActionSaveToZone.Wo> result = new ActionResult<>();
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionSaveToZone().execute(effectivePerson, zoneFolder, jsonElement);
		} catch (Exception e) {
			logger.error(e, effectivePerson, request, jsonElement);
			result.error(e);
		}
