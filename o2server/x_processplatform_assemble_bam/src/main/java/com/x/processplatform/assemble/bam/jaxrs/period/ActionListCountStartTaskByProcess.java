...
class ActionListCountStartTaskByProcess extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(ActionListCountStartTaskByProcess.class);
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ProcessStub stub : this.listStub(applicationId)) {
					...
					w.setCount(this.countStartTask(business, o, applicationId, stub.getValue(), StandardJaxrsAction.EMPTY_SYMBOL,
							unit, person));
					list.add(w);
				}
			}
	...
	public static class Wo extends LinkedHashMap<String, List<WoProcess>> {
		...
		private static final long serialVersionUID = 8553319300009532954L;
