...
class ActionListCountCompletedTaskByApplication extends BaseAction {
	
	private static Logger logger = LoggerFactory.getLogger(ActionListCountCompletedTaskByApplication.class);
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ApplicationStub stub : this.listStub()) {
					...
					Long count = this.countCompletedTask(business, o, stub.getValue(), StandardJaxrsAction.EMPTY_SYMBOL,
							StandardJaxrsAction.EMPTY_SYMBOL, unit, person);
					w.setCount(count);
					Long duration = this.durationCompletedTask(business, o, stub.getValue(),
							StandardJaxrsAction.EMPTY_SYMBOL, StandardJaxrsAction.EMPTY_SYMBOL, unit, person);
