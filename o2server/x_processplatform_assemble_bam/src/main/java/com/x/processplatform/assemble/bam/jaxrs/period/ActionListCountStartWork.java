...
class ActionListCountStartWork extends BaseAction {
	
	private static Logger logger = LoggerFactory.getLogger(ActionListCountStartWork.class);
	...
	ActionResult<List<Wo>> execute(String applicationId, String processId, String unit, String person)
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				w.setName(str);
				w.setValue(str);
				w.setCount(this.countStartWork(business, o, applicationId, processId, unit, person));
				list.add(w);
			}
			result.setData(list);
			return result;
