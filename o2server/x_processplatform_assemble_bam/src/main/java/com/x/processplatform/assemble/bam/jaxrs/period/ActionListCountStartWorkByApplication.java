...
class ActionListCountStartWorkByApplication extends BaseAction {
	
	private static Logger logger = LoggerFactory.getLogger(ActionListCountStartWorkByApplication.class);
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ApplicationStub stub : this.listStub()) {
					...
					w.setCount(this.countStartWork(business, o, stub.getValue(), StandardJaxrsAction.EMPTY_SYMBOL, unit,
							person));
					list.add(w);
				}
			}
	...
	public static class Wo extends LinkedHashMap<String, List<WoApplication>> {
		...
		private static final long serialVersionUID = -5756519900545476884L;
