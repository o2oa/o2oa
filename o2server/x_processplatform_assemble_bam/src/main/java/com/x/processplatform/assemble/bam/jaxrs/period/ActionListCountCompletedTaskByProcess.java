...
class ActionListCountCompletedTaskByProcess extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(ActionListCountCompletedTaskByProcess.class);
	
	ActionResult<Wo> execute(String applicatonId, String unit, String person) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ProcessStub stub : this.listStub(applicatonId)) {
					...
					Long count = this.countCompletedTask(business, o, applicatonId, stub.getValue(),
							StandardJaxrsAction.EMPTY_SYMBOL, unit, person);
					w.setCount(count);
					Long duration = this.durationCompletedTask(business, o, applicatonId, stub.getValue(),
							StandardJaxrsAction.EMPTY_SYMBOL, unit, person);
