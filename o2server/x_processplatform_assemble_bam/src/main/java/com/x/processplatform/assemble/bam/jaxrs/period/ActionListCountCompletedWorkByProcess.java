...
class ActionListCountCompletedWorkByProcess extends BaseAction {
	
	private static Logger logger = LoggerFactory.getLogger(ActionListCountCompletedWorkByProcess.class);
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ProcessStub stub : this.listStub(applicationId)) {
					...
					Long count = this.countCompletedWork(business, o, applicationId, stub.getValue(), unit, person);
					w.setCount(count);
					Long duration = this.durationCompletedWork(business, o, applicationId, stub.getValue(), unit,
							person);
					w.setDuration(duration);
					Long times = this.timesCompletedWork(business, o, applicationId, stub.getValue(), unit, person);
					w.setTimes(times);
					list.add(w);
				}
			}
	...
	public static class Wo extends LinkedHashMap<String, List<WoProcess>> {
		...
		private static final long serialVersionUID = 7915523797120105853L;
	}
