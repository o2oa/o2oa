...
class ActionListCountStartTaskByUnit extends BaseAction {
	
	private static Logger logger = LoggerFactory.getLogger(ActionListCountStartTaskByUnit.class);
	...
	ActionResult<Wo> execute(String applicationId, String processId, String activityId) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				wo.put(str, list);
				for (UnitStub stub : ThisApplication.period.getStartTaskUnitStubs()) {
					WoUnit w = new WoUnit();
					w.setName(stub.getName());
					w.setValue(stub.getValue());
					w.setLevel(stub.getLevel());
					w.setCount(this.countStartTask(business, o, applicationId, processId, activityId, stub.getValue(),
	...
	public static class Wo extends LinkedHashMap<String, List<WoUnit>> {
		...
		private static final long serialVersionUID = -4514124280431181727L;
