...
class ActionListCountStartWorkByProcess extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(ActionListCountStartWorkByProcess.class);
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ProcessStub stub : this.listStub(applicationId)) {
					...
					w.setCount(this.countStartWork(business, o, applicationId, stub.getValue(), unit, person));
					list.add(w);
				}
			}
			result.setData(wo);
	...
	public static class Wo extends LinkedHashMap<String, List<WoProcess>> {
		...
		private static final long serialVersionUID = -1721734358355963956L;
