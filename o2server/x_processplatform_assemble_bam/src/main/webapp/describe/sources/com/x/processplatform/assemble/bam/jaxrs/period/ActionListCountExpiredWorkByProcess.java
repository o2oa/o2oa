...
class ActionListCountExpiredWorkByProcess extends BaseAction {
	
	private static Logger logger = LoggerFactory.getLogger(ActionListCountExpiredWorkByProcess.class);
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			for (DateRange o : os) {
				...
				for (ProcessStub stub : this.listStub(applicationId)) {
					...
					w.setCount(this.countExpiredWork(business, o, StandardJaxrsAction.EMPTY_SYMBOL, stub.getValue(),
							unit, person));
					list.add(w);
				}
			}
	...
	public static class Wo extends LinkedHashMap<String, List<WoProcess>> {
		...
		private static final long serialVersionUID = 6476242733735121613L;
