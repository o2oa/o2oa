...
public class ActionListIncludeMeasuresByStrategyYear extends BaseAction {
	private static Logger logger = LoggerFactory.getLogger(ActionListIncludeMeasuresByStrategyYear.class);
	...
	public ActionResult<List<WoIncludeMeasures>> execute(WiAppendMeasuresDept wi, EffectivePerson effectivePerson) throws Exception {
		...
		if (ispass) {
			...
			for (StrategyDeploy o : objs) {
				...
				_includeObjs = ActionListIncludeMeasuresByStrategyYear.getMeasuresInfoListByParentid(o.getId());
