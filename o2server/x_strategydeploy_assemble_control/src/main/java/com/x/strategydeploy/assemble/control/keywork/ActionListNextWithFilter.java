...
public class ActionListNextWithFilter extends BaseAction {
	...
	protected ActionResult<List<Wo>> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id,
		...
		// WrapCopier<KeyworkInfo, BaseAction.Wo> wrapout_copier =
		// WrapCopierFactory.wo(KeyworkInfo.class, BaseAction.Wo.class, null,
		// JpaObject.FieldsInvisible);
		// WrapCopier<KeyworkInfo, BaseAction.Wo> wrapout_copier =
		// WrapCopierFactory.wo(KeyworkInfo.class, BaseAction.Wo.class,
		// JpaObject.singularAttributeField(KeyworkInfo.class, true, false), null);
		...
		// result = this.standardListPrev(wrapout_copier, id, count, sequenceField,
		// equals, notEquals, likes, ins, notIns, members, notMembers, andJoin, order);
			...
			if (reader_persons.indexOf(distinguishedName) >= 0 || writer_persons.indexOf(distinguishedName) >= 0) {
				...
				if (reader_persons.indexOf(distinguishedName) >= 0) {
					...
					for (Wo wo : wos) {
						...
						List<String> _titles = keyworkInfoQueryService.getMeasuresTitleListByIds(wo.getMeasureslist());
						wo.setMeasurestitlelist(_titles);
						List<MeasuresInfo> MeasuresInfoObjectList = keyworkInfoQueryService
								.getMeasuresObjectsListByIds(wo.getMeasureslist());
						wo.setMeasuresobjlist(MeasuresInfoObjectList);
					}
					result.setData(wos);
				}
