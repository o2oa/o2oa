...
import com.x.strategydeploy.assemble.control.measures.BaseAction.MeasuresInfoWithUnitML;
...
public class ActionListIncludeMeasuresByStrategyYearAndMeasuresDept2 extends BaseAction {
	private static Logger logger = LoggerFactory.getLogger(ActionListIncludeMeasuresByStrategyYearAndMeasuresDept2.class);
	...
	public static class WoIncludeMeasures extends Wo {
		...
		public List<MeasuresInfoWithUnitML> measureMLList = new ArrayList<MeasuresInfoWithUnitML>();

		public List<MeasuresInfoWithUnitML> getMeasureMLList() {
			return measureMLList;
		}

		public void setMeasureMLList(List<MeasuresInfoWithUnitML> measureMLList) {
			this.measureMLList = measureMLList;
		}
	...
	public ActionResult<List<WoIncludeMeasures>> execute(WiAppendMeasuresDept wi) throws Exception {
		...
		List<MeasuresInfoWithUnitML> _includeObjsML = new ArrayList<MeasuresInfoWithUnitML>();
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			if (ispass) {
				...
				logger.info("工作纲要数量：" + objs.size());
				Integer StrategyIndex = 0;
				for (StrategyDeploy o : objs) {

					WoIncludeMeasures _woincludemeasures = wrapout_copier.copy(o);
					List<MeasuresInfoWithUnitML> _resObjs = new ArrayList<MeasuresInfoWithUnitML>();
					_includeObjs = ActionListIncludeMeasuresByStrategyYearAndMeasuresDept2.getMeasuresInfoListByParentid(o.getId());
					_includeObjsML = coverToMeasuresInfoWithUnitMLList(_includeObjs);

					//logger.info("wi.getMeasuresdept:" + wi.getMeasuresdept());
					if (null == wi.getMeasuresdept() || StringUtils.isBlank(wi.getMeasuresdept())) {
						_woincludemeasures.setMeasureMLList(_includeObjsML);
						//_woincludemeasures.setMeasureList(_includeObjs);
					} else {
						for (MeasuresInfoWithUnitML measuresInfo : _includeObjsML) {
							//相关部门过滤
						...
						if (!_resObjs.isEmpty()) {
							...
							_woincludemeasures.setMeasureMLList(_resObjs);
						} else {
						}

					}

					//logger.info("StrategyIndex:" + StrategyIndex + "==" + jsonstr);
	...
	protected static List<MeasuresInfoWithUnitML> coverToMeasuresInfoWithUnitMLList(List<MeasuresInfo> _includeObjs) throws Exception {
		WrapCopier<MeasuresInfo, MeasuresInfoWithUnitML> _copier = WrapCopierFactory.wo(MeasuresInfo.class, MeasuresInfoWithUnitML.class, null, Wo.Excludes);
		List<MeasuresInfoWithUnitML> _includeObjsML = new ArrayList<MeasuresInfoWithUnitML>();
		Gson gson = new Gson();

		for (MeasuresInfo measuresInfo : _includeObjs) {

			MeasuresInfoWithUnitML MeasuresInfo_withUnitML = new MeasuresInfoWithUnitML();
			MeasuresInfo_withUnitML = _copier.copy(measuresInfo, MeasuresInfo_withUnitML);
			MeasuresInfo_withUnitML = MeasuresInfo_withUnitML.getObjectIncludUnitML(measuresInfo);
			_includeObjsML.add(MeasuresInfo_withUnitML);

		}
		return _includeObjsML;
	}
