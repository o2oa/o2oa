...
class ActionSelect1 extends BaseAction {

	private static final Logger LOGGER = LoggerFactory.getLogger(ActionSelect1.class);
	...
	private List<Bar> select(String field, String value, Integer count) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			Predicate p = cb.isMember(values, root.get(field));
