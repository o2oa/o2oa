...
class ActionListNextWithDate extends BaseAction {
	ActionResult<List<Wo>> execute(String id, Integer count, String date) throws Exception {
		...
		betweens.put(WarnLog.occurTime_FIELDNAME, ListTools.toList(floor, ceil));
		result = this.standardListNext(Wo.copier, id, count, WarnLog.sequence_FIELDNAME, null, null, null, null, null,
				null, null, betweens, true, DESC);
		return result;
	}
