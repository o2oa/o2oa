...
public class SyncOrganization {
    ...
    private Identity updateIdentity(Business business, PullResult result, Unit unit,
            ...
            Long order) throws Exception {
        if (null != order && (!StringUtils.equals(Objects.toString(identity.getOrderNumber(), ""),
                Math.abs(order.intValue()) + ""))) {
            EntityManagerContainer emc = business.entityManagerContainer();
            emc.beginTransaction(Identity.class);
            identity.setOrderNumber(Math.abs(order.intValue()));
            emc.commit();
    ...
    private void clean(Business business, PullResult result, List<Unit> units, List<Person> people,
        ...
        for (Identity identity : ListUtils.subtract(allIdentities, identities)) {
            ...
            if (null == person || StringUtils.isNotEmpty(person.getDingdingId())) {
                ...
                if (ListTools.isNotEmpty(uds)) {
                    emc.beginTransaction(UnitDuty.class);
                    uds.forEach(o -> o.getIdentityList().remove(identity.getId()));
                    emc.commit();
                }
                String distinguishedName = identity.getDistinguishedName();
                logger.info("删除身份：{}", distinguishedName);
                emc.beginTransaction(Identity.class);
                emc.remove(identity, CheckRemoveType.all);
                emc.commit();
                result.getRemoveIdentityList().add(distinguishedName);
            }
        }
        /* 组织单独方法删除 */
        ...
        for (Person person : ListUtils.subtract(allPeople, people)) {
            ...
            logger.info("删除用户：{}", distinguishedName);
            ThisApplication.context().applications()
                    .deleteQuery(x_organization_assemble_control.class,
                            Applications.joinQueryUri("person", person.getId()));
            result.getRemovePersonList().add(distinguishedName);
