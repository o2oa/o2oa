...
public class TestClient2 {
	public static void main(String[] args) throws IOException {
		try (Context context = Context.newBuilder("js").allowHostClassLookup(s -> true).allowHostAccess(HostAccess.ALL)
				.build()) {
			Source initialScriptSource = Source.create("js",
					Files.readString(Paths.get("/data/O2/o2oa/o2server/commons/initialScriptText.js")));
			context.eval(initialScriptSource);
			// 执行JavaScript代码
...
//					"Java.type('com.x.base.core.project.gson.XGsonBuilder').instance().toJsonTree(\"o\")");
			...
			System.out.println("类型1: " + result.isHostObject() + "->" + result.asHostObject());
