...
class ActionListMobile extends BaseAction {
	private static final Logger logger = LoggerFactory.getLogger(ActionListMobile.class);
	/**
	...
	ActionResult<List<Wo>> execute(EffectivePerson effectivePerson) throws Exception {
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			...
			result.setData(wos.stream().sorted(Comparator.comparing(Portal::getName, Comparator.nullsLast(String::compareTo)))
	...
	private List<String> list(Business business, EffectivePerson effectivePerson) throws Exception {
		...
		Predicate p = cb.or(cb.isNull(root.get(Portal_.mobileClient)), cb.isTrue(root.get(Portal_.mobileClient)));
	...
	public static class Wo extends Portal {
		...
		private static final long serialVersionUID = 7549117521073938989L;
		static WrapCopier<Portal, Wo> copier = WrapCopierFactory.wo(Portal.class, Wo.class,
				JpaObject.singularAttributeField(Portal.class, true, true), JpaObject.FieldsInvisible);
