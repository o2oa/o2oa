package com.x.server.console.server.admin;
...
import com.x.base.core.project.x_program_admin;
...
public class AdminServerTools extends JettySeverTools {

	private static final Logger LOGGER = LoggerFactory.getLogger(AdminServerTools.class);
	...
	public static Server start() throws Exception {
		...
		Path war = Paths.get(Config.dir_store().toString(), x_program_admin.class.getSimpleName() + PathTools.DOT_WAR);
		Path dir = Config.path_servers_adminServer_work(true).resolve(x_program_admin.class.getSimpleName());
	...
	public static QuickStartWebApp webContext() throws Exception {
		...
		webApp.setDisplayName(x_program_admin.class.getSimpleName());
		webApp.setContextPath("/");
		ResourceCollection resources = new ResourceCollection(new String[] { dir.toAbsolutePath().toString(),
				Config.path_servers_webServer_x_admin(true).toAbsolutePath().toString() });
		webApp.setBaseResource(resources);
		webApp.setDescriptor(dir.resolve(Paths.get(PathTools.WEB_INF_WEB_XML)).toString());
		webApp.setExtraClasspath(calculateExtraClassPath(x_program_admin.class));
		Path ext = dir.resolve("WEB-INF").resolve("ext");
		if (Files.exists(ext)) {
			webApp.setExtraClasspath(calculateExtraClassPath(x_program_admin.class, ext));
		} else {
			webApp.setExtraClasspath(calculateExtraClassPath(x_program_admin.class));
		}
		LOGGER.debug("{} extra class path:{}.", x_program_admin.class, webApp.getExtraClasspath());
	...
	private static void cleanWorkDirectory() throws IOException, URISyntaxException {
		try (Stream<Path> paths = Files.walk(Config.path_servers_adminServer_work(true))) {
			// 删除每个文件和子目录
