...
public class ActionListStdForTopUnitNextWithFilter extends BaseAction {

	private static Logger logger = LoggerFactory.getLogger(ActionListStdForTopUnitNextWithFilter.class);
	...
	protected ActionResult<List<Wo>> execute(HttpServletRequest request, EffectivePerson effectivePerson, String id,
		...
		if (check) {
			try {
				...
				List<String> unitNameList = getUnitNameList(wrapIn.getTopUnitName(), wrapIn.getUnitName(),
						effectivePerson.getDebugger());
				wrapIn.setUnitName(unitNameList);
				// 从数据库中查询符合条件的一页数据对象
				statisticList = business.getStatisticTopUnitForDayFactory().listIdsNextWithFilter(id, count, sequence,
						wrapIn);
	...
	public static class Wo extends StatisticTopUnitForDay {
		...
		public static WrapCopier<StatisticTopUnitForDay, Wo> copier = WrapCopierFactory.wo(StatisticTopUnitForDay.class,
