...
public class ActionListStdForUnitPrevWithFilter extends BaseAction {
	
	private static  Logger logger = LoggerFactory.getLogger( ActionListStdForUnitPrevWithFilter.class );
	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id, Integer count, JsonElement jsonElement ) throws Exception {
		...
		if(check ){
			try {
				...
				// 从数据库中查询符合条件的一页数据对象
				statisticList = business.getStatisticUnitForDayFactory().listIdsPrevWithFilter(id, count, sequence,
						wrapIn);
