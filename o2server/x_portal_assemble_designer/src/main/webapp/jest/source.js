...
function source_list_init(portal) {
    $('#content').html('');
    $('#result').html('');
...
function source_list(portal) {
	...
	source_list_grid(json);
    }).always(function(json) {
	$('#result').html(JSON.stringify(json, null, 4));
    });
}

function source_list_grid(json) {
    if (json.data) {
	var str = '';
	$.each(json.data, function(index, item) {
	    ...
	    str += '<td>' + item.id + '</td>';
	    str += '<td>';
	    str += '<a href="#" onclick="source_edit(\'' + item.id + '\')">edit</a>&nbsp;';
	    str += '<a href="#" onclick="source_delete(\'' + item.id + '\')">delete</a>';
	    str += '</td>';
	    str += '</tr>';
	});
    }
