...
public class Task extends SliceJpaObject {
	...
	public static final String TOP_TASK = "0";
	public static final String SERIAL_NUMBER_SEPARATOR = ".";
	...
	public void postLoad() {
		if(StringUtils.isNotBlank(serialNumber)){
			try {
				String[] numbers = StringUtils.split(serialNumber, SERIAL_NUMBER_SEPARATOR);
				List<Integer> numberList = new ArrayList<>();
				for (String number : numbers){
					numberList.add(Integer.valueOf(number));
				}
				this.serialNumberDisplay = StringUtils.join(numberList, SERIAL_NUMBER_SEPARATOR);
			} catch (NumberFormatException e) {
				this.serialNumberDisplay = this.serialNumber;
			}
		}
	}
	...
	@Column(length = length_255B, name = ColumnNamePrefix + projectName_FIELDNAME)
	...
	@FieldDescribe("工作状态：进行中-processing|已完成-completed|已搁置-delay|已取消-canceled")
	...
	@FieldDescribe("工作进度：记录0-100，显示的时候添加百分比")
	...
	public static final String important_FIELDNAME = "important";
	@FieldDescribe("重要程度")
	@Column( length = JpaObject.length_32B, name = ColumnNamePrefix + important_FIELDNAME)
	@CheckPersist( allowEmpty = true )
	private String important;

	public static final String urgency_FIELDNAME = "urgency";
	@FieldDescribe("紧急程度")
	@Column( length = JpaObject.length_32B, name = ColumnNamePrefix + urgency_FIELDNAME)
	@CheckPersist( allowEmpty = true )
	private String urgency;

	public static final String serialNumber_FIELDNAME = "serialNumber";
	@FieldDescribe("序列号")
	@Column( length = JpaObject.length_64B, name = ColumnNamePrefix + serialNumber_FIELDNAME)
	@Index( name = TABLE + IndexNameMiddle + serialNumber_FIELDNAME )
	@CheckPersist(allowEmpty = true )
	...
	@FieldDescribe("工作进展说明")
	@Lob
	@Basic(fetch = FetchType.EAGER)
	@Column( length = JpaObject.length_1M, name = ColumnNamePrefix + detail_FIELDNAME)
	...
	public static void main(String[] args) {
		System.out.println(StringUtils.leftPad("05", 3, "0"));
	}
}
