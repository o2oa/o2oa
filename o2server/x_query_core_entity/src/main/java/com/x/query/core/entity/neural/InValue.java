...
@Schema(name = "InValue", description = "数据中心神经网络短语值.")
@Entity
@ContainerEntity(dumpSize = 1000, type = ContainerEntity.Type.content, reference = ContainerEntity.Reference.strong)
@Table(name = PersistenceProperties.Neural.InValue.table, uniqueConstraints = {
		@UniqueConstraint(name = PersistenceProperties.Neural.InValue.table + JpaObject.IndexNameMiddle
				+ JpaObject.DefaultUniqueConstraintSuffix, columnNames = { JpaObject.IDCOLUMN,
						JpaObject.CREATETIMECOLUMN, JpaObject.UPDATETIMECOLUMN, JpaObject.SEQUENCECOLUMN }) })
@Inheritance(strategy = InheritanceType.TABLE_PER_CLASS)
public class InValue extends SliceJpaObject {

	private static final long serialVersionUID = -5610293696763235753L;

	private static final String TABLE = PersistenceProperties.Neural.InValue.table;
	...
	public static final String inTextSerial_FIELDNAME = "inTextSerial";
	@FieldDescribe("原始编号")
	@Column(name = ColumnNamePrefix + inTextSerial_FIELDNAME)
	@Index(name = TABLE + IndexNameMiddle + inTextSerial_FIELDNAME)
	@CheckPersist(allowEmpty = false)
	private Integer inTextSerial;
	...
	public Integer getInTextSerial() {
		return inTextSerial;
	}

	public void setInTextSerial(Integer inTextSerial) {
