...
public class H2Action extends StandardJaxrsAction {
	...
	@JaxrsMethodDescribe(value = "检查本地H2数据库是否要升级.", action = ActionCheck.class)
	@GET
	@Path("check")
	...
	@JaxrsMethodDescribe(value = "设置H2数据库进行升级.", action = ActionUpgrade.class)
	@GET
	@Path("upgrade")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void upgrade(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request) {
		...
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionUpgrade().execute(effectivePerson);
		} catch (Exception e) {
			LOGGER.error(e, effectivePerson, request, null);
			result.error(e);
		}
	...
	@JaxrsMethodDescribe(value = "取消H2数据库进行升级.", action = ActionUpgradeCancel.class)
	@GET
	@Path("upgrade/cancel")
	@Produces(HttpMediaType.APPLICATION_JSON_UTF_8)
	@Consumes(MediaType.APPLICATION_JSON)
	public void upgradeCancel(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request) {
		ActionResult<ActionUpgradeCancel.Wo> result = new ActionResult<>();
		EffectivePerson effectivePerson = this.effectivePerson(request);
		try {
			result = new ActionUpgradeCancel().execute(effectivePerson);
		} catch (Exception e) {
			LOGGER.error(e, effectivePerson, request, null);
			result.error(e);
		}
