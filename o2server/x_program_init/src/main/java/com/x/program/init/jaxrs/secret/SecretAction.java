...
public class SecretAction extends StandardJaxrsAction {
	...
	private static Logger logger = LoggerFactory.getLogger(SecretAction.class);

	@JaxrsMethodDescribe(value = "更新指定Work的Data数据.", action = ActionCreate.class)
	@PUT
	@Path("work/{id}")
	...
	public void updateWithWork(@Suspended final AsyncResponse asyncResponse, @Context HttpServletRequest request,
		...
		try {
			result = new ActionCreate().execute(effectivePerson);
		} catch (Exception e) {
