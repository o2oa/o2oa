 ...
 *
 */
public class Timertask_ReportCreateTask implements Job {

	private Logger logger = LoggerFactory.getLogger(Timertask_ReportCreateTask.class);
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println(">>>>>>>>>>>>工作汇报创建代理运行开始........");
		tryToCreateReport();
	}
	...
	private void tryToCreateReport() {
		...
		try {
			...
			nextReportTime = report_Caculater_MonthReportTime.getNextReportTime( EnumReportTypes.MONTHREPORT.toString(), lastReportTime );
			
			logger.info( "每月汇报最后一次发起时间：" + lastReportTime + ", 下一次发起时间：" + nextReportTime +", 当前时间：" + new Date() );
