 ...
 *
 */
public class ActionListWorkPlanWithReport extends BaseAction {
	
	private Logger logger = LoggerFactory.getLogger( ActionListWorkPlanWithReport.class);
	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request, EffectivePerson effectivePerson, String reportId ) throws Exception {
		...
		if( check ) {
			try {
				planList = report_C_WorkPlanServiceAdv.listWithReportId( reportId );
				if( planList != null ) {
