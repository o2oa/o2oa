...
import com.x.report.assemble.control.jaxrs.report.exception.ExceptionRefreshReportPermission;
...
public class ActionWorkAndPersonInfoSubmit extends BaseAction {
	
	private static Logger logger = LoggerFactory.getLogger( ActionWorkAndPersonInfoSubmit.class );
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, JsonElement jsonElement ) throws Exception {
		...
		if (check) {
			try { //将读者以及作者信息持久化到数据库中
				report_I_PermissionServiceAdv.refreshReportPermission( wrapIn.getId() );
			} catch (Exception e) {
				Exception exception = new ExceptionRefreshReportPermission( e, wrapIn.getId() );
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}
		}
