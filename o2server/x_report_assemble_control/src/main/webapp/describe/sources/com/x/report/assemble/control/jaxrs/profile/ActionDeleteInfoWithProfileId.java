 ...
 *
 */
public class ActionDeleteInfoWithProfileId extends BaseAction {
	
	private Logger logger = LoggerFactory.getLogger( ActionDeleteInfoWithProfileId.class );
	...
	protected ActionResult<WrapOutBoolean> execute( HttpServletRequest request, EffectivePerson effectivePerson, String profileId ) throws Exception {
		...
		if( check ){
			try {
				check = report_P_ProfileOperationServiceAdv.deleteInfo( profile );
