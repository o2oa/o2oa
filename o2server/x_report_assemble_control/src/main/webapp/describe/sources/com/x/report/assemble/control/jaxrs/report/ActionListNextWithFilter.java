...
public class ActionListNextWithFilter extends BaseAction {
	...
	protected ActionResult<List<Wo>> execute( HttpServletRequest request, String id, Integer count, JsonElement jsonElement, EffectivePerson effectivePerson ) {
		...
		if (check) {
			try {
				...
				check = false;
				Exception exception = new ExceptionReportInfoProcess( e, "系统在检查用户是否是平台管理员时发生异常。Name:" + effectivePerson.getDistinguishedName());
				result.error(exception);
				logger.error(e, effectivePerson, request, null);
			}	
		}
		...
		if (check && !isXAdmin) {
			try {
				isXAdmin = userManagerService.isHasPlatformRole(effectivePerson.getDistinguishedName(), "ReportManager");
			}catch( Exception e) {
				e.printStackTrace();
			}
		}
		...
		if (check) {
			if ( isXAdmin ) { // 如果是管理员，则可以查询所有的汇报信息，越过权限
				if( StringUtils.isNotEmpty( wi.getPermission() )) {
					// 需要带权限查询
					...
					try {
						viewAbleReportIds = report_I_PermissionServiceAdv.lisViewableReportIdsWithFilter(
								...
								wi.getReportStatus(), permissionObjectCodes, wi.getPermission(), 5000 );
					} catch (Exception e) {
						check = false;
						Exception exception = new ExceptionReportInfoProcess(e, "系统在根据过滤条件查询用户可访问的汇报信息文档ID列表时发生异常。");
						result.error(exception);
						logger.error(e, effectivePerson, request, null);
					}
				}else {
					try {
						viewAbleReportIds = report_I_ServiceAdv.lisViewableIdsWithFilter(
								wi.getTitle(), wi.getReportType(), wi.getReportObjType(),
								wi.getYear(), wi.getMonth(), wi.getWeek(),
								wi.getActivityList(), wi.getTargetList(), wi.getCurrentPersonList(), wi.getUnitList(), wi.getReportStatus(),
								5000, false  );
					} catch (Exception e) {
						check = false;
						Exception exception = new ExceptionReportInfoProcess(e, "系统在根据过滤条件查询用户可访问的汇报信息文档ID列表时发生异常。");
		...
		//如果传递了workId列表，那么根据workId，查询出与之有关系的汇报，取交集后再进行查询
		List<String> reportIdsWithWork = null;
		List<String> queryReportId = new ArrayList<>();
		if( viewAbleReportIds != null && !viewAbleReportIds.isEmpty() ) {
			for( String _id : viewAbleReportIds ) {
				queryReportId.add( _id );
			}
		}
		if (check) {
			if( wi.getWorkIds() != null && !wi.getWorkIds().isEmpty() ) {
				try {
					reportIdsWithWork = report_C_WorkProgServiceAdv.listReportIdsWithKeyWorkId( 
							wi.getYear(), wi.getMonth(), wi.getWeek(), wi.getReportDate(), wi.getReportType(), 
							wi.getWorkIds(), wi.getReportStatus() 
					);
					if( reportIdsWithWork != null && !reportIdsWithWork.isEmpty() ) {
						String _id = null;
						for ( int i = queryReportId.size() - 1; i >=0; i--) {//如果跟该工作无关，则删除  
							_id = queryReportId.get(i);  
						    if( !reportIdsWithWork.contains( _id )) {
								queryReportId.remove( _id );
							} 
						}
					}
				} catch (Exception e) {
					check = false;
					Exception exception = new ExceptionReportInfoProcess(e, "系统在根据工作ID列表过滤用户可访问的汇报ID列表时发生异常。");
					result.error(exception);
					logger.error(e, effectivePerson, request, null);
				}
			}
		...
		if (check) {
			// 从数据库中查询符合条件的对象总数
			try {
				total = report_I_ServiceAdv.countWithIds( queryReportId );
			} catch (Exception e) {
				check = false;
		...
		if (check) {
			try {
				reportBaseList = report_I_ServiceAdv.listNextWithDocIds( id, count, queryReportId, wi.getOrderField(), wi.getOrderType(), false  );
			} catch (Exception e) {
				check = false;
	...
	public static class Wi {
		...
		@FieldDescribe( "汇报信息状态：流转中|已完成" )
		private String reportStatus = "流转中";
