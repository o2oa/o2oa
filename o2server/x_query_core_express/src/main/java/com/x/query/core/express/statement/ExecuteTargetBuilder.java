...
public class ExecuteTargetBuilder {
	...
	private Pair<ExecuteTarget, Optional<ExecuteTarget>> concreteExecuteTargetSql(Context context,
		...
		Optional<String> additionFilter = runtime.additionFilter();
		if (additionFilter.isPresent()) {
			net.sf.jsqlparser.statement.Statement stmt = CCJSqlParserUtil.parse(sql);
			if (stmt instanceof Select) {
				sql = selectAdditionFilter(stmt, additionFilter.get());
				LOGGER.debug("sql after add addition filter:{}.", sql);
			}
		}
	...
	private Pair<ExecuteTarget, Optional<ExecuteTarget>> concreteExecuteTargetJpql(Context context,
		...
		if (additionFilter.isPresent()) {
			...
			if (stmt instanceof Select) {
				jpql = selectAdditionFilter(stmt, additionFilter.get());
				LOGGER.debug("jpql after add addition filter:{}.", jpql);
			}
		}
	 ...
	 */
	private String selectAdditionFilter(net.sf.jsqlparser.statement.Statement stmt, String addition)
			throws JSQLParserException {
		...
		String txt = addition;
		if (null != where) {
			txt = "(" + where.toString() + ") AND " + txt;
		}
		plainSelect.setWhere(CCJSqlParserUtil.parseCondExpression(txt));
