...
class ActionCreateDataPath4 extends BaseAction {
	...
	ActionResult<Wo> execute(String id, String path0, String path1, String path2, String path3, String path4,
			JsonElement jsonElement) throws InterruptedException, ExecutionException, TimeoutException {
		...
		Callable<ActionResult<Wo>> callable = new Callable<ActionResult<Wo>>() {
			public ActionResult<Wo> call() throws Exception {
				try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
					...
					create(business, dict, jsonElement, path0, path1, path2, path3, path4);
					emc.commit();
					Wo wo = new Wo();
					wo.setId(dict.getId());
					result.setData(wo);
	...
	public static class Wo extends WoId {
		...
		private static final long serialVersionUID = 4024960959646993730L;
