...
class ActionUpdateDataPath0 extends BaseAction {
	...
	ActionResult<Wo> execute(String id, String path0, JsonElement jsonElement) throws Exception {
		...
		Callable<ActionResult<Wo>> callable = new Callable<ActionResult<Wo>>() {
			public ActionResult<Wo> call() throws Exception {
				try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
					...
					update(business, dict, jsonElement, path0);
					emc.commit();
					Wo wo = new Wo();
					wo.setId(dict.getId());
					result.setData(wo);
	...
	public static class Wo extends WoId {
		...
		private static final long serialVersionUID = -7844250017265542218L;
